...
begin
  ...
  if m_listGateIP <> nil then begin
    for i := 0 to m_listGateIP.Count - 1 do
      Dispose(pTTablePubIP(m_listGateIP[i]));
    m_listGateIP.Free;
  end;

  if m_listGmIP <> nil then begin
    for i := 0 to m_listGmIP.Count - 1 do
      Dispose(pTTablePubIP(m_listGmIP[i]));
    m_listGmIP.Free;
  end;

  if m_listServerInfo <> nil then begin
    for i := 0 to m_listServerInfo.Count - 1 do begin
      pServerInfo := pTGameServerInfo(m_listServerInfo[i]);
      for j := 0 to pServerInfo.listSelectGate.Count - 1 do begin
