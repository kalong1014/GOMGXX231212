...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Dialogs, ScktComp, syncobjs, MudUtil, HUtil32, Grobal2, Math;
...
//   iProb := Trunc((Sqrt(10000 - Power(x+a, 2)) / (100 + Power(x, 2))) * 100);
   for i:=x downto 1 do begin
      if i > x div 2 then begin
        ...
        if Random(800) < iProb then begin        //ÉñÃØ×°±¸¼«Æ·¼¸ÂÊ£¬Êý×ÖÔ½Ð¡¼¸ÂÊÔ½´ó  Ä¬ÈÏ500
         Result := i{x} div 3;
         break;
      end;
   end;
...
//²ÎÊýpu±ØÐëÊÇÎäÆ÷¡£
//TUSERItem DescµÄÉý¼¶µØÍ¼0:DC 1:MC 2:SC
...
begin
   //¹¥»÷Ñ¡Ïî
   up := GetUpgrade (12, 15);
   if Random(25) = 0 then pu.Desc[0] := 1+up; //DC

   //¹¥»÷ËÙ¶È
   up := GetUpgrade (12, 15);
   if Random(30) = 0 then begin  //¹¥»÷ËÙ¶È
      incp := (1+up) div 3;  //Õ³²»ÀÎ
      if incp > 0 then begin
         if Random(3) <> 0 then  pu.Desc[6] := incp  //¹¥»÷ËÙ¶È£¨-£©
         else pu.Desc[6] := 10 + incp;  //¹¥»÷ËÙ¶È (+)
      end;
   end;
   ...
   up := GetUpgrade (12, 15);
   if Random(10) = 0 then
      pu.Desc[7] := 1 + (up div 2); //ÎäÆ÷µÄÇ¿¶È
...
begin
   //¹æ¾î
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random(50) = 0 then pu.Desc[0] := 1+up; //AC    ÐÒÔË¼«Æ·ÂÊ

   //¸¶Ç×
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random(50) = 0 then pu.Desc[1] := 1+up; //MAC

   //ÆÄ±«
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random(70) = 0 then pu.Desc[2] := 1+up; //DC

   //¸¶¹ý
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random(70) = 0 then pu.Desc[3] := 1+up; //MC

   //µµ·Â
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random(70) = 0 then pu.Desc[4] := 1+up; //SC
...
begin
   //Á¤È®
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random((jipingailv)) = 0 then pu.Desc[0] := 1+up; //AC(HIT)

   //¹ÎÃ¸
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random((jipingailv)) = 0 then pu.Desc[1] := 1+up; //MAC(SPEED)

   //ÆÄ±«
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random((jipingailv)) = 0 then pu.Desc[2] := 1+up; //DC

   //¸¶¹ý
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random((jipingailv)) = 0 then pu.Desc[3] := 1+up; //MC

   //µµ·Â
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random((jipingailv)) = 0 then pu.Desc[4] := 1+up; //SC
...
begin
   //¹æ¾î
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random((jipingailv)) = 0 then pu.Desc[0] := 1+up; //AC

   //¸¶Ç×
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random((jipingailv)) = 0 then pu.Desc[1] := 1+up; //MAC
...
begin
   //¼«Æ·±¬ÂÊ
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random(65) = 0 then pu.Desc[0] := 1+up; //Ä§·¨¶ã±Ü

   //Çà¿î
   up := GetUpgrade ((xingyunxlds), (dianshugailv));
   if Random((xingyunxl)) = 0 then pu.Desc[1] := 1+up; //ÐÒÔË  xingyun jingdian
...
begin
   //Áßµ¶ÀúÇ×
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random((jipingailv)) = 0 then pu.Desc[0] := 1+up; //Áßµ¶ÀúÇ×

   //Áßµ¶È¸º¹
   up := GetUpgrade ((jipindianshu), (dianshugailv));
   if Random((jipingailv)) = 0 then pu.Desc[1] := 1+up; //Áßµ¶È¸º¹
...
begin
   //¹æ¾î
   up := GetUpgrade ((jipingailv), (dianshugailv));
   if Random((jipingailv)) = 0 then pu.Desc[0] := 1+up; //AC
...
//   up := GetUpgrade (3, 15) + GetUpgrade (5, 30);
   ...
   //²»µôÂäµÄÎïÆ·
   if Random(30) = 0 then
      pu.Desc[7] := 1;  //²»µôÂäµÄÊôÐÔ
   pu.Desc[8] := 1;  //Î´ÖªÊôÐÔ

   //¸½¼Ó×°±¸ÐèÒªÖµ
   if sum >= 2 then begin
      if (pu.Desc[0] >= 3) then begin //·ÀÓù
         pu.Desc[5] := 1; //ÇÊÆÄ
         pu.Desc[6] := 25 + pu.Desc[0] * 3;
         exit;
      end;
      if (pu.Desc[2] >= 2) then begin //¹¥»÷
         pu.Desc[5] := 1; //ÇÊÆÄ
         pu.Desc[6] := 30 + pu.Desc[2] * 3;
         exit;
      end;
      if (pu.Desc[3] >= 2) then begin //Ä§·¨
         pu.Desc[5] := 2; //ÇÊ¸¶
         pu.Desc[6] := 20 + pu.Desc[3] * 2;
         exit;
      end;
      if (pu.Desc[4] >= 2) then begin //µÀÊõ
         pu.Desc[5] := 3; //ÇÊµµ
...
begin
   ...
   //¹¥»÷ËÙ¶È
   up := GetUpgrade (12, 15);
   if Random(20) = 0 then begin  //¹¥»÷ËÙ¶È
      incp := (1+up) div 3;  //Õ³²»ÀÎ
      if incp > 0 then begin
         if Random(3) <> 0 then  pu.Desc[6] := incp  //¹¥»÷ËÙ¶È£¨-£©
         else pu.Desc[6] := 10 + incp;  //¹¥»÷ËÙ¶È (+)
...
begin
   //¹æ¾î
   up := GetUpgrade (6, 20);
   if Random(50) = 0 then pu.Desc[0] := 1+up; //AC
   ...
   //ÆÄ±«
   up := GetUpgrade (6, 20);
   if Random(50) = 0 then pu.Desc[2] := 1+up; //DC

   //¸¶¹ý
   up := GetUpgrade (6, 20);
   if Random(50) = 0 then pu.Desc[3] := 1+up; //MC

   //µµ·Â
   up := GetUpgrade (6, 20);
   if Random(50) = 0 then pu.Desc[4] := 1+up; //SC
...
begin
   //¼«Æ·±¬ÂÊ
   up := GetUpgrade (6, 20);
   if Random(85) = 0 then pu.Desc[0] := 1+up; //Ä§·¨¶ã±Ü

   //Çà¿î
   up := GetUpgrade (6, 20);
   if Random(99) = 0 then pu.Desc[1] := 1+up; //ÐÒÔË  xingyun jingdian
...
begin
   //Áßµ¶ÀúÇ×
   up := GetUpgrade (6, 20);
   if Random(60) = 0 then pu.Desc[0] := 1+up; //Áßµ¶ÀúÇ×

   //Áßµ¶È¸º¹
   up := GetUpgrade (6, 20);
   if Random(60) = 0 then pu.Desc[1] := 1+up; //Áßµ¶È¸º¹

   //ÆÄ±«
   up := GetUpgrade (6, 20);
   if Random(60) = 0 then pu.Desc[2] := 1+up; //DC

   //¸¶·Â
   up := GetUpgrade (6, 20);
   if Random(60) = 0 then pu.Desc[3] := 1+up; //MC

   //µµ·Â
   up := GetUpgrade (6, 20);
   if Random(60) = 0 then pu.Desc[4] := 1+up; //SC
