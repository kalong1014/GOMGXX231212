...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Dialogs, ScktComp, syncobjs, MudUtil, HUtil32, Grobal2;
...

{
   ...
   public
      ...
      boStall,NoDropItem : Boolean;   //유저가 죽어도 아이템을 떨구거나 깨지지 않는 맵
      TimeMap:string;
      NoRevive : Boolean;
      MapQuestList: TList;

      NoUseMagic : Boolean;
      NoUseMagicList: TList;
      NoUseItem : Boolean;
      NoUseItemList: TList;

      MapQuestParams : array [0..99] of integer; //2004/08/27 맵 지역변수 추가(sonmg)
   ...
   public
      ...
      constructor Create;
      Destructor Destroy; override;
      function  AddEnvir (mapname, newmapname,
                     title: string;
                     serverindex, needlevel: integer;
                     lawful, fightzone, fight2zone, fight3zone, fight4zone, dark, dawn, sunny, quiz, norecon,
                     needhole, norecall, norandommove, NoEscapeMove, NoEscsafeMove, NoTeleportMove, nodrug : boolean;
                     minemap : integer; nopositionmove: Boolean;
                     backmap: string;
                     npc: TObject; setnumber, setvalue, autoAttack, GuildAgit: integer;
                     nochat, nogroup, nothrowitem, nodropitem,Boolean겠覡: Boolean;
                     timemap:string; nousemagic: Boolean;
                                           nousemagicname: string;
                                           nouseitems: Boolean;
                                           nouseitemname: string; NODEARRECALL: Boolean): TEnvirnoment;
...
uses
   svMain, ObjBase, Event, ObjNpc, LocalDB, UsrEngn, StrUtils;
         ...
         for i:=pm.ObjList.Count-1 downto 0 do begin
            if (PTAThing (pm.ObjList[i]).Shape = OS_MOVINGOBJECT) then begin
               ...
               if cret <> nil then begin
                  ...
                  if (cret.RaceServer = RC_USERHUMAN) and (not cret.BoGhost) and (cret.HoldPlace) and (not aliveonly or not cret.Death) then begin
                     Result := cret;
                     bohum := True;
                     break;
                  end;
...
//norecon : 재접 가능 여부
function  TEnvirList.AddEnvir (mapname, newmapname, title: string; serverindex, needlevel: integer;
                    lawful, fightzone, fight2zone, fight3zone, fight4zone, dark, dawn, sunny, quiz, norecon,
                    needhole, norecall, norandommove, NoEscapeMove, NoEscsafeMove, NoTeleportMove, nodrug : boolean;
                    minemap : integer; nopositionmove: Boolean;
                    backmap: string;
...
begin
   ...
   envir.boStall :=Boolean겠覡;
   envir.MapName := mapname;
   envir.NewMapName := NewMapName;
   if newmapname <> '' then envir.UseNewMap := True;
   ...
   envir.NoEscsafeMove := NoEscsafeMove;
   ...
   envir.TimeMap := StringReplace(timemap,'+',' ',[rfReplaceAll]);

   envir.NoUseMagic := nousemagic;
   envir.NODEARRECALL := NODEARRECALL;
   if nousemagicname <> '' then begin
     if envir.NoUseMagicList = nil then envir.NoUseMagicList := TList.Create;
     sTmpList.Text := AnsiReplaceText(nousemagicname, '|', #13);
     for ii := 0 to sTmpList.Count - 1 do begin
       pm := UserEngine.GetDefMagic(sTmpList[ii]);
       if pm <> nil then
       envir.NoUseMagicList.Add(Pointer(pm.MagicId));
     end;
   end;

   envir.NoUseItem := nouseitems;
   if nouseitemname <> '' then begin
     if envir.NoUseItemList = nil then envir.NoUseItemList := TList.Create;
     sTmpList.Text := AnsiReplaceText(nouseitemname, '|', #13);
     for ii := 0 to sTmpList.Count - 1 do begin
       idx := UserEngine.GetStdItemIndex(sTmpList[ii]);
       if idx <> -1 then
       envir.NoUseItemList.Add(Pointer(idx));
     end;
   end;
