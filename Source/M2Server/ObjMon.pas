...
begin
   ...
   MonsterSayMsg(LastHiter, s_KillHuman);  //2022¸üÐÂ¹Ì¶¨¹ÖÎïËµ»°
...
//2022/9/9 BOBO¸üÐÂAI
 if not Death and not RunDone and not BoGhost then begin
      if GetTickCount - JumpTime > 30 * 1000 then begin
         JumpTime := GetTickCount;
         if (TargetCret <> nil) and (SiegeLockCount >= 4) then begin  //4¸í¿¡°Ô µÑ·¯ ½×ÀÓ
            //nn := Random(VisibleActors.Count-2) + 1;
            //ncret := TCreature (PTVisibleActor(VisibleActors[nn]).cret);
            //if ncret <> nil then SelectTarget (ncret);
            GetBackPosition (TargetCret, nx, ny);
...
begin
  if GetCurrentTime - m_SearchTick > 100 then       // ÉñÊÞÉ¨Ãè¹ÖÎïÊ±¼ä
   begin
     m_SearchTick:=GetCurrentTime;
     MonsterNormalAttack;
   end;
         ...
         if TwinGenDelay <= 0 then begin
            ...
            if cret <> nil then begin
               if not DontAttack then begin
                  {$IFDEF KOREA}
                  UserEngine.CryCry (RM_CRY, PEnvir, CX, CY, 10000, ' -' + __GoldenImugi + ' ÖÜÎ§ÓÐ¸´»îµÄÆøÏ¢Âð£¿');
                  {$ELSE}
...
procedure TGoldenImugi2.RangeAttack (targ: TCreature); //¹Ýµå½Ã target <> nil
var
   i, pwr, dam: integer;
   sx, sy, tx, ty : integer;
   list: TList;
...
procedure TGoldenImugi2.RangeAttack2 (targ: TCreature); //¹Ýµå½Ã target <> nil
var
   i, ix, iy, ixf, ixt, iyf, iyt, pwr, dam: integer;
   sx, sy, tx, ty : integer;
   list: TList;
      ...
      if imugicount = 1 then begin
         if TwinGenDelay <= 0 then begin
            ...
            if cret <> nil then begin
               if not DontAttack then begin
                  {$IFDEF KOREA}
                  UserEngine.CryCry (RM_CRY, PEnvir, CX, CY, 10000, ' -' + __VomaAncestor + ' ´òÆÆÁËÊ±¿ÕµÄÆÁÕÏ£¬ÕÙ»½³öÒì¿Õ¼äµÄÁíÒ»Î»ÎÖÂêÊ¼×æ²¢¼ç×÷Õ½.');
                  {$ELSE}
