          ...
          case AutoRunInfo.nMoethod of
            nRODAY: begin
                if GetTickCount - AutoRunInfo.dwRunTick > 24 * 60 * 60 * 1000 * LongWord(AutoRunInfo.nParam1) then begin
                  ...
                  g_RobotNPC.NpcSayTitle(self, AutoRunInfo.sParam2);
                end;
              end;
            nROHOUR: begin
...
var
  nSEC, nMIN, nHOUR, nWeek        : Integer;
  wWeek, wHour, wMin, wSec, wMSec: Word;
  sSEC, sMIN, sHOUR, sWeek        : string;
...
begin
  ...
  sLineText := GetValidStr3(sLineText, sSEC, [':']);
  nHOUR := Str_ToInt(sHOUR, -1);
  nMIN := Str_ToInt(sMIN, -1);
  nSEC := Str_ToInt(sSEC, -1);
  sLabel := AutoRunInfo.sParam2;
  DecodeTime(Time, wHour, wMin, wSec, wMSec);
  if (nHOUR in [0..24]) and (nMIN in [0..60]) and (nSEC in [0..60])then begin
    if (wHour = nHOUR) then begin
      if (wMin = nMIN) and (wSec = nSEC ) then begin
...
begin
  ...
  if (nWeek in [1..7]) and (nHOUR in [0..24]) and (nMIN in [0..60]) and (nSEC in [0..60])then begin
...
begin
  sFileName := EnvirDir + 'Robot_def\' + m_sScriptFileName + '.txt';
    ...
    for i := 0 to LoadList.Count - 1 do begin
      ...
      if (sLineText <> '') and (sLineText[1] <> ';') then begin
        ...
        if (sRobotName <> '') and (sScriptFileName <> '') then begin
          ...
          RobotHuman.UserName := sRobotName;
          RobotHuman.m_sScriptFileName := sScriptFileName;
          RobotHuman.LoadScript;
          RobotHumanList.AddObject(RobotHuman.UserName, RobotHuman);
        end;
      end;
    end;
    LoadList.Free;
