...
uses
  Forms, Windows, Messages, SysUtils, Classes, Controls, ExtCtrls, StdCtrls, HUtil32, ObjBase, Grobal2,
  mmsystem, ScktComp, EDCode, Dialogs, mudutil, Envir, MfdbDef;


//DB ¼­¹ö¿Í Åë½ÅÇÏ´Â ºÎºÐÀº FrnEngn¿¡¼­¸¸ »ç¿ëÇØ¾ß¸¸ ÇÑ´Ù.
...
procedure SendNonBlockDatas   ( data : AnsiString );
procedure SendRDBSocket (certify: integer; data: AnsiString);
...
begin
   with prcd.Block.DBHuman do begin
      ...
      hum.Abil.FameCur   := Abil_FameCur;    //ÇöÀç ¸í¼ºÄ¡(2004/10/22)
      hum.Abil.FameBase  := Abil_FameBase;   //´©Àû ¸í¼ºÄ¡(2004/10/22)
      hum.WAbil.Level:= Abil_LEVEL;
      hum.WAbil.HP   := Abil_HP;
      hum.WAbil.MP   := Abil_MP;
      hum.WAbil.EXP  := Abil_EXP;

      //Áßº¹ ÄÚµå (sonmg 2004/10/22)
//      hum.wAbil.Level := Abil_LEVEL;
//      hum.wAbil.HP    := Abil_HP;
//      hum.wAbil.MP    := Abil_MP;
//      hum.wAbil.EXP   := Abil_EXP;
...
{$ENDIF}
      ...
      hum.NotReadTag := HorseRace ; // 2003-08-21 HorseRace ¸¦  NotReadTag ·Î»ç¿ë

      hum.WAbil.FameCur   := Abil_FameCur;    //ÇöÀç ¸í¼ºÄ¡(2004/10/22)
      hum.WAbil.FameBase  := Abil_FameBase;   //´©Àû ¸í¼ºÄ¡(2004/10/22)

      hum.SecondsCard  := Seconds;
...
//UserEngine½º·¡µå¿¡¼­¸¸ È£ÃâÇØ¾ß ÇÔ. ´Ù¸¥ ½º·¡µå È£Ãâ ºÒ°¡
procedure FDBMakeHumRcd (hum: TUserHuman; prcd: PFDBRecord);
var
   i: integer;
   umi: TUseMagicInfo;
...
{$ENDIF}
      ...
      HorseRace := hum.NotReadTag; // 2003-08-21 : PDS NotReadTag Ãß°¡

      //¸í¼ºÄ¡ Ãß°¡
      Abil_FameCur   := hum.Abil.FameCur;    //ÇöÀç ¸í¼ºÄ¡(2004/10/22)
      Abil_FameBase  := hum.Abil.FameBase;   //´©Àû ¸í¼ºÄ¡(2004/10/22)

      Seconds := hum.SecondsCard;
