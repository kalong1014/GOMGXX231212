...
{------------------------------------------------------------------------------}
  ...
  csShare: TCriticalSection;  //悼扁拳 矫埃捞 篮 傍蜡函荐俊 荤侩秦具 窃.
  csDelShare: TCriticalSection;  //悼扁拳 矫埃捞 篮 傍蜡函荐俊 荤侩秦具 窃.
...
begin
 ...
 //-摆摊
         ...
         if g_boDiableHumanRun then bostr := 'TRUE'
         else bostr := 'FALSE';
         ini.WriteString ('Server', 'DiableHumanRun', bostr);

         if g_boRUNHUMAN then bostr := 'TRUE'
         else bostr := 'FALSE';
         ini.WriteString ('Server', 'RUNHUMAN', bostr);

         if g_boRUNMON then bostr := 'TRUE'
         else bostr := 'FALSE';
         ini.WriteString ('Server', 'RUNMON', bostr);

         if g_boRunNpc then bostr := 'TRUE'
         else bostr := 'FALSE';
         ini.WriteString ('Server', 'RunNpc', bostr);

         if g_boRunGuard then bostr := 'TRUE'
         else bostr := 'FALSE';
         ini.WriteString ('Server', 'RunGuard', bostr);

         if g_boWarDisHumRun then bostr := 'TRUE'
         else bostr := 'FALSE';
         ini.WriteString ('Server', 'WarDisHumRun', bostr);

         if g_boGMRunAll then bostr := 'TRUE'
         else bostr := 'FALSE';
         ini.WriteString ('Server', 'GMRunAll', bostr);

         if g_boSafeAreaLimited then bostr := 'TRUE'
         else bostr := 'FALSE';
         ini.WriteString ('Server', 'SafeAreaLimited', bostr);
