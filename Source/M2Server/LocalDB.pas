...
type
  ...
  public
    ...
    function  LoadMonSayMsg: integer; //rainee ¹ÖÎïËµ»°
...
begin
   ...
   Result := -1;
   flname := EnvirDir + MAPDEFFILE;  //¼ÓÔØµØÍ¼
        ...
        // »ñÈ¡ÖØ¸´ÀûÓÃµØÍ¼  Ö§³Ö| Óë¡«
            ...
            if map <> '' then begin
               ...
               while TRUE do begin
                  ...
                  if data <> '' then begin
                  //STALL
                     if CompareText(data, 'STALL') = 0 then Boolean°ÚÌ¯:= TRUE;
...
begin
   ...
   scopestr := '10'; //¾ÈÀüÁö´ë ±âº» ¹üÀ§ //°²È«…^Ä¬ÕJ¹ ‡úÊÇ10¸ñ
   if FileExists (flname) then begin
      strlist := TStringList.Create;
      strlist.LoadFromFile (flname);
      for i:=0 to strlist.Count-1 do begin
...
procedure TFrmDB.LoadCityInfo; //Ë½ÓÐ¹ý³Ì
var
  i: integer;
...
begin
  ...
  if FileExists(flname) then begin
    strlist := TStringList.Create;
    strlist.LoadFromFile(flname);
    setlength(SafeZoneForClient, strlist.Count); //ƒžÏÈ¶¨Áx”µ½MéL¶È
    FillChar(SafeZoneForClient[0], length(SafeZoneForClient), #0); //³õÊ¼»¯
    for I := 0 to strlist.Count - 1 do begin
      str := Trim(strlist[i]);
      if str = '' then Continue;
      if str[1] = ';' then Continue;
      if str <> '' then begin
        str := GetValidStr3(str, smap, [' ', #9]); //Ö÷µØÍ¼
        str := GetValidStr3(str, smap2, [' ', #9]); //³Çæ‚Ãû×Ö
        str := GetValidStr3(str, xstr, [' ', #9]); //×ù˜ËX
        str := GetValidStr3(str, ystr, [' ', #9]); //×ù˜ËY
        str := GetValidStr3(str, scopestr, [' ', #9]); //¹ ‡ú
    ...
    setlength(SafeZoneForClient, TempInt); //²Ã¼ô”µ½MéL¶È
    MainOutMessage('ÒÑ³É¹¦¼ÓÝd ' + inttostr(length(SafeZoneForClient)) + ' ‚€³Çæ‚ÐÅÏ¢...');
    strlist.free;
  end;
end;
...
begin
  ...
  if not FileExists(sFileName) then begin
    LoadList := TStringList.Create();
    LoadList.Add(';ÒýÇæ²å¼þÉÌÆÌÅäÖÃÎÄ¼þ');
    LoadList.Add(';ÎïÆ·Àà±ð'#9'ÎïÆ·Ãû³Æ'#9'Item.wilÐòºÅ'#9'³öÊÛ¼Û¸ñ'#9'Effect.wilÐòºÅ'#9'Í¼Æ¬ÊýÁ¿'#9'ÃèÊö');
...
begin
   ...
   if FileExists (EnvirDir + MARKETDEFDIR + QFUNCTIONFILE + '.txt') then begin
      ...
      npc.CX := 0;
      npc.CY := 0;
      npc.UserName := QFUNCTIONFILE;  //ÀÇ¹Ì ¾øÀ½
      npc.NpcFace := 0;
      npc.Appearance := 0;
      npc.DefineDirectory := MARKETDEFDIR;
      npc.BoInvisible := TRUE;
...
begin
  try
    ...
    if not FileExists(sScriptFile) then begin
      ...
      tSaveList.Add(';´Ë½ÅÎª»úÆ÷ÈË×¨ÓÃ½Å±¾£¬ÓÃÓÚ»úÆ÷ÈË´¦Àí¹¦ÄÜÓÃµÄ½Å±¾');
...
var
  ...
  //    if UpperCase(cmdstr) = 'RIGHTHAND' then ident := QI_RIGHTHAND;      //¼ì²éÀ¯Öò£¬Ñ«ÕÂÎ»ÖÃÊÇ·ñÓÐÎïÆ· 2022/10/6
      ...
      if UpperCase(cmdstr) = 'CHECKGAMEPOINT' then ident := QI_CHECKGAMEPOINT;   //»ý·Ö
      if UpperCase(cmdstr) = 'HAVEGUILD' then ident := QI_HAVEGUILD; //¼ì ²âÊÇ·ñ¼ÓÈëÐÐ»á
      if UpperCase(cmdstr) = 'ISCASTLEMASTER' then ident := QI_ISCASTLEMASTER;  //¼ì²éÍæ¼ÒÊÇ·ñÎªÉ³³ÇÀÏ´ó
      if UpperCase(cmdstr) = 'ISCASTLEGUILD' then ident := QI_ISCASTLEGUILD;    //¼ì²éÍæ¼ÒÊÇ·ñÎªÉ³³Ç³ÉÔ±
      if UpperCase(cmdstr) = 'INSAFEZONE' then ident := QI_INSAFEZONE;    //¼ì²éÊÇ·ñ°²È«Çø
      if UpperCase(cmdstr) = 'CHECKITEMADDVALUE' then ident := QI_CHECKITEMADDVALUE;  //¼ì²â×°±¸ÊôÐÔ
      if UpperCase(cmdstr) = 'CHECKIDLIST' then ident := QI_CHECKIDLIST;         //¼ì²âIDÁÐ±í
      if UpperCase(cmdstr) = 'ISUSEITEM' then ident := QI_ISUSEITEM;  //µ±Ç°Ê¹ÓÃÎïÆ·
      if UpperCase(cmdstr) = 'CHECKINFO' then ident := QI_CHECKINFO;  //¼ì²âÊÇ·ñÔÚµ±Ç°µØÍ¼
      ...
      if UpperCase(cmdstr) = 'ISADMIN' then  ident := QI_ISADMIN;        //¼ì²âÊÇ²»ÊÇÏµÍ³¹ÜÀíÔ±2022.7.29
...
//      end;
         ...
         pqc.IfExtra2 := extrastr2;      //¼ì²âÊôÐÔ
   ...
   begin
      ...
      if UpperCase(cmdstr) = 'CLEARMAPMON' then ident := QA_CLEARMAPMON;  //ÐÂÇåÀí¹ÖÎï
      ...
      if UpperCase(cmdstr) = 'GAMEGOLD' then ident := QA_GAMEGOLD;//Ôª±¦½Å±¾Ö´ÐÐ
      if UpperCase(cmdstr) = 'GAMEPOINT' then ident := QA_GAMEPOINT;     //»ý·Ö
      ...
      if UpperCase(cmdstr) = 'SETRANKLEVELNAME' then ident := QA_SETRANKLEVELNAME; //·âºÅ
      if UpperCase(cmdstr) = 'ADDSKILL' then ident := QA_ADDSKILL; //Ôö¼Ó¼¼ÄÜ
      if UpperCase(cmdstr) = 'DELSKILL' then ident := QA_DELSKILL; //É¾³ý¼¼ÄÜ
      if UpperCase(cmdstr) = 'DELNOJOBSKILL' then ident := QA_DELNOJOBSKILL; //É¾³ý·Ç±¾Ö°ÒµµÄÆäËû¼¼ÄÜ
      if UpperCase(cmdstr) = 'CLEARSKILL' then ident := QA_CLEARSKILL;  //É¾³ýËùÓÐ¼¼ÄÜ
      if UpperCase(cmdstr) = 'SKILLLEVEL' then ident := QA_SKILLLEVEL;  //µ÷Õû¼¼ÄÜµÈ¼¶
      if UpperCase(cmdstr) = 'MESSAGEBOX' then ident := QA_MESSAGEBOX; //´°¿ÚÌáÊ¾
      if UpperCase(cmdstr) = 'SETITEMEVENT' then ident := QA_SETITEMEVENT;     //×Ô¶¨Òå×Ö¶Î´¥·¢Ë«»÷ÎïÆ·
      if UpperCase(cmdstr) = 'CHANGEHUMABILITY' then ident := QA_CHANGEHUMABILITY;  //changeHumAbilityÊôÐÔ¼Óµã
      if UpperCase(cmdstr) = 'ADDGUILDMEMBER' then ident := QA_ADDGUILDMEMBER; //¼ÓÈëÖ¸¶¨ÐÐ»á
      if UpperCase(cmdstr) = 'CLEARNAMELIST' then ident := QA_CLEARNAMELIST; //ÇåÀíÁÐ±í
      if UpperCase(cmdstr) = 'NOTONLINE' then ident := QA_NOTONLINE;//ÀëÏß¹Ò»ú
