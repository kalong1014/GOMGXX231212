...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Dialogs, ScktComp, syncobjs, MudUtil, HUtil32,
  Grobal2, Envir, EdCode, ObjBase, M2Share, Event;
...
type
   ...
   public
      ...
      function MagMakeYinYangMatrix(user: TCreature; dam, htime, x, y: integer): integer;
      function MagMakePoisonCloud(user: TCreature; dam, htime, x, y: integer): integer;

      function  IceBlaze (user: TCreature; skilllevel: integer): Boolean;
   end;
   ...
   if (target.RaceServer <> RC_USERHUMAN) and (Random(4-shocklevel) = 0) then begin
      ...
      if (Random(2) = 0) then begin
         if target.Abil.Level <= user.Abil.Level + 2 then begin
            if Random(3) = 0 then begin        //ÓÕ»ó³É¹¦ÂÊ   Ä¬ÈÏÊÇRandom(3)
               if ((youhuojilv) + target.Abil.Level) < Random(20 + user.Abil.Level + shocklevel * 5) then begin     //µÚÒ»¸ö5Ä¬ÈÏÊÇ10  ÕâÀïÊÇÓÕ»ó¼¸ÂÊ
...
//                     (user.SlaveList.Count < (2 + shocklevel) and
                  if user.EnableRecallMob( target , shockLevel ) then begin
                     ...
                     ran := target.WAbil.MaxHP div (youhuoxueliang);       //ÓÕ»ó³É¹¦ÂÊ£¬Ä¬ÈÏÊÇ div 100
                     ...
                     if (target.Master <> user) and (Random(ran) = 0) then begin
                      ...
                      //target.MasterRoyaltyTime := GetTickCount + longword(20 + shocklevel * 20 + Random(user.Abil.Level * 2))* 60 * 1000; //¹Ù·½Ô­°æÄ¬ÈÏ·¨Ê¦±¦±¦ÅÑ±äµÄ¹«Ê½
                      target.MasterRoyaltyTime := GetTickCount +longword((Bbpb) * 60) * 60 * 1000;  //±¦±¦ÅÑ±äÊ±¼ä¹Ì¶¨ÎªËÄÐ¡Ê± £¨4*60£©=4x60·ÖÖÓ
                      //target.MasterRoyaltyTime := GetTickCount +longword(Bbpb) * 1000;  //²âÊÔÓÃ
                  ...
                  if (target.LifeAttrib <> LA_UNDEAD) and (Random(3) = 0) then begin //Çé¿öÊ§¿Ø×´Ì¬
                     target.MakeCrazyMode (10 + Random(20));
                  end;
               end;
            end else begin
               if target.LifeAttrib <> LA_UNDEAD then begin
                  target.MakeCrazyMode (10 + Random(20));     //Çé¿öÊ§¿Ø×´Ì¬£¬±äºì
               end;
            end;
         end;
      end else begin
         target.MakeHolySeize ((10 + shocklevel * 5) * 1000);  //¾«ÉñµÄÉ¥Ê§, Ê§È¥ÁÖ
      end;
      Result := TRUE;
   end else
      if Random(2) = 0 then
...
begin
   ...
   then begin
      ...
      if (target.Abil.Level < (_MIN(user.Abil.Level, 51) - 1 + Random(4))) and
         //2003/02/11 ÃÖ´ë·¡¹ë º¯°æ
         (target.Abil.Level < g_MONLEVELTIAOJIE)   //50 -> 61    //   (target.Abil.Level < MAXKINGLEVEL-1)
      then begin
   ...
   if user.PEnvir.CanWalk (x, y, TRUE) then begin
      ...
      for i:=0 to rlist.Count-1 do begin
         ...
         if (cret.RaceServer >= RC_ANIMAL) and
            ...
            //2003/02/11 ÃÖ´ë·¡¹ë º¯°æ
            (cret.Abil.Level < g_MONLEVELTIAOJIE) and    //50 -> 61 //      (cret.Abil.Level < MAXKINGLEVEL-1) and
            (cret.Master = nil)
         then begin  //°á°è¿¡ °É¸°´Ù.
...
function  TMagicManager.IceBlaze (user: TCreature; skilllevel: integer): Boolean;
var
   i    : integer;
   rlist: TList;
   cret : TCreature;
...
begin
   ...
   for i:=0 to rlist.Count-1 do begin
      ...
      begin
         ...
         if CriticalDamage then begin
            DcRandom := HIBYTE(user.Wabil.MC);
         end else begin
            DcRandom := Random( HIBYTE(user.Wabil.MC) );
         end;

         if isnear then begin
            pwr := ( (12 + 3 * ( SkillLevel + user.Abil.Level div 20 )) ) * DcRandom div 30 + LOBYTE(user.Wabil.MC);
         end else begin
            pwr := ( (8 + 2 * ( SkillLevel + user.Abil.Level div 20 )) ) * DcRandom div 30 + LOBYTE(user.Wabil.MC);
         end;
         ...
         if pwr > 0 then begin
            if (Random(10) = 0) then cret.MakePoison( POISON_SLOW , 2 ,1 );
            windcuthit( user, cret, pwr, 0 );
            Result := TRUE;
         end;
      end;
...
begin
   User.UseBlizzard := True;
    ...
    event := TBlizzardEvent.Create(user, x + 2, y, ET_BLIZZARD, htime, dam);
    EventMan.AddEvent(event);
    event := TBlizzardEvent.Create(user, x, y - 2, ET_BLIZZARD, htime, dam);
    EventMan.AddEvent(event);
    event := TBlizzardEvent.Create(user, x, y + 2, ET_BLIZZARD, htime, dam);
    EventMan.AddEvent(event);
...
begin
 User.UseBlizzard := True;
    ...
    event := TMeteorEvent.Create(user, x + 2, y, ET_BLIZZARD, htime, dam);
    EventMan.AddEvent(event);
    event := TMeteorEvent.Create(user, x, y - 2, ET_BLIZZARD, htime, dam);
    EventMan.AddEvent(event);
    event := TMeteorEvent.Create(user, x, y + 2, ET_BLIZZARD, htime, dam);
    EventMan.AddEvent(event);
Result := 1;
...
function TMagicManager.MagMakeYinYangMatrix (user: TCreature; dam, htime, x, y: integer): integer;
var
  event: TEvent;
  i:     integer;
  rlist: TList;
  cret:  TCreature;
begin
    event := TYinYangEvent.Create(user, x, y, ET_YINYANGFIVE, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x + 1, y, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x + 2, y, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x - 1, y, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x - 2, y, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x, y + 1, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x, y + 2, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x, y - 1, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x, y - 2, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x + 1, y + 1, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x + 2, y + 1, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x + 1, y + 2, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x + 2, y + 2, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x - 1, y + 2, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TYinYangEvent.Create(user, x - 2, y + 2, ET_BLIZZARD, htime * 1000, dam);
...
function TMagicManager.MagMakePoisonCloud(user: TCreature;
  dam, htime, x, y: integer): integer;
var
  event: TEvent;
  i:     integer;
  ...
  cret:  TCreature;
begin
    event := TPoisonCloudEvent.Create(user, x, y, ET_POISONCLOUD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TPoisonCloudEvent.Create(user, x - 1, y - 1, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TPoisonCloudEvent.Create(user, x, y - 1, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TPoisonCloudEvent.Create(user, x + 1, y - 1, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TPoisonCloudEvent.Create(user, x - 1, y, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TPoisonCloudEvent.Create(user, x + 1, y, ET_BLIZZARD, htime * 1000, dam);
    EventMan.AddEvent(event);
    event := TPoisonCloudEvent.Create(user, x - 1, y + 1, ET_BLIZZARD, htime * 1000, dam);
...
function  TMagicManager.SpellNow (user: TCreature; pum: PTUserMagic; xx, yy: integer; target: TCreature ; spell:integer): Boolean;
      ...
      if user.UseItems[U_BUJUK].Index > 0 then begin
         if user.UseItems[U_BUJUK].Dura < 100 then begin    // U_ARMRINGL->U_BUJUK
            ...
            if user.RaceServer = RC_USERHUMAN then begin
               ...
               {$IFDEF KOREA} hum.SysMsg('ÄãµÄ»¤Éí·ûÒÑ¾­ÓÃ¾¡', 0);
  ...
  function IsNotAllowUseMag(PlayObject: TUserHuman; cMag: PTDefMagic): Boolean;
  var
    i                       : Integer;
  resourcestring
  ...
  begin
    Result := False;
    if (PlayObject.PEnvir <> nil) and (PlayObject.PEnvir.NoUseMagic) then begin
      if PlayObject.PEnvir.NoUseMagicList <> nil then begin
        for i := 0 to PlayObject.PEnvir.NoUseMagicList.Count - 1 do begin
          if cMag.MagicId = Integer(PlayObject.PEnvir.NoUseMagicList[i]) then begin
            PlayObject.SysMsg(Format(sNotAllowUseMag, [cMag.MagicName]), 0);
            Result := True;
            Break;
          end;
        end;
...
begin
  ...
  if (user.RaceServer = RC_USERHUMAN) and IsNotAllowUseMag(TUserHuman(user), pum.pDef) then
    Exit;
      ...
      11: //°­°Ý
         if user.IsProperTarget (target) then begin
            if target.AntiMagic <= Random(50) then begin
               ...
               user.SendDelayMsg (user, RM_DELAYMAGIC, pwr, MakeLong(xx, yy), 2, integer(target), '', 600);
               if pum.pDef.MagicId = 35 then begin
                 if target.RaceServer = RC_USERHUMAN then begin
                   if target.Abil.MP > 0 then begin
                     target.DamageSpell(Abs(Round(pwr * 0.35)));  //jingdian add ÃðÌì»ð¼õMP
                   end;
                 end;
               end;
               if (target.RaceServer >= RC_ANIMAL) then train := TRUE;
...
//       end;
      ...
      33: //ºù¼³Ç³
         begin
            if MagBigExplosion (user,
                                ...
                                xx,
                                yy,
                                (bingpaoxiao)) then begin //1¸ÄÎª2¾ÍÊÇ±ùÅØÏø·¶Î§*2
               train := TRUE;
            end;
         end;
      24: //·Ú¼³È­
    ...
    57: //±ùÑæÊõ
      begin
//        if IceBlaze(user, pum.Level) then
        if IceBlaze(user, user.GetAttackPower (GetPower (MPow(pum)) +
               Lobyte(user.WAbil.MC), Integer(Hibyte(user.WAbil.MC) -
               Lobyte(user.WAbil.MC)) + 1)) then
        begin
          train := true;
        end;
      end;
...
//               if target <> target then begin
               if target.CelestialUp (Gap, pum.Level, GetPower (15 + pwr)) then
                  train := TRUE;
//               end;
            end;
         end;
      ...
      begin
        ...
        if user.MagBloodDragonSword(pum.Level, sec) then
        begin
          if user.MagHitSpeedUp(sec{Ãë}, pwr) then
            train := TRUE;
      ...
      29: //´ëÈ¸º¹¼ú
        ...
        50: //Rage
                 begin
            sec := 18 + (pum.Level * 6);
            if user.ExtraAbil[EABIL_MAGICDCUP] > 0 then pwr := user.ExtraAbil[EABIL_MAGICDCUP]
            else pwr := Round(Hibyte(user.WAbil.DC) * (g_Bloodbreakingcrazykilling / 100 + (pum.Level * 0.03)));

            if user.MagDcUpEx(sec{ÃÊ}, pwr) then begin
               train := True;
            end;
         end;
...
//          end;
            begin
            sec := 45 + (pum.Level * 15);
            if user.ExtraAbil[EABIL_MAGICACUP] > 0 then pwr := user.ExtraAbil[EABIL_MAGICACUP]
            else pwr := Round(Hibyte(user.WAbil.AC) * (g_seTIEBUSHAN / 100 + (pum.Level * 0.03)));

            if user.MagAcUpEx(sec{ÃÊ}, pwr) then begin
               train := True;
            end;
         end;
  ...
  begin
          {  if MagBlizzard(user, user.GetAttackPower(GetPower(MPow(pum)) +
            ...
            if MagBlizzard (user, user.GetAttackPower (GetPower (MPow(pum)) +
               Lobyte(user.WAbil.MC), Integer(Hibyte(user.WAbil.MC) -
               Lobyte(user.WAbil.MC)) + 1), 5000, xx, yy) > 0 then begin
               train := TRUE;
            end;
     ...
     56: //ÈÕÉÁ
         begin
            ndir := GetNextDirection (user.CX, user.CY, xx, yy);
            if GetNextPosition (user.PEnvir, user.CX, user.CY, ndir, 1, sx, sy) then begin
               GetNextPosition (user.PEnvir, user.CX, user.CY, ndir, 1, xx, yy);
               user.SendRefMsg (RM_NORMALEFFECTEX, 0, user.CX, user.CY, ndir, '');
               with user do begin
                  pwr := GetAttackPower (
                           GetPower (MPow(pum)) + Lobyte(WAbil.DC),
                           Integer(Hibyte(WAbil.DC)-Lobyte(WAbil.DC)) + 1
                         );
               end;

               if user.SlashingBust (sx, sy, xx, yy, ndir, pwr, FALSE) > 0 then begin
                  GetNextPosition (user.PEnvir, user.CX, user.CY, ndir, 2, xx, yy);
                  if user.MagSpaceMoveInRange(xx, yy, 1) then
                  train := TRUE;
               end else begin
                  GetNextPosition (user.PEnvir, user.CX, user.CY, ndir, 2, xx, yy);
                  if user.MagSpaceMoveInRange(xx, yy, 0) then
                  train := TRUE;
               end;
            end;
         end;
      ...
      6: //¾Ï¿¬¼ú
         begin
            ...
            if user.IsProperTarget (target) then begin
               ...
               begin
               if (not g_bochksigedu) and (user.UseItems[U_BUJUK].Index > 0) then begin        // U_ARMRINGL->U_BUJUK
                  pstd := UserEngine.GetStdItem (user.UseItems[U_BUJUK].Index);
                  if pstd <> nil then begin
              ...
              // end;
               ...
               if (user.UseItems[U_CHARM].Index > 0) then begin        // U_ARMRINGL->U_BUJUK
                  pstd := UserEngine.GetStdItem (user.UseItems[U_CHARM].Index);
                  if pstd <> nil then begin
                     if (pstd.StdMode = 25) and (pstd.Shape <= 2) then begin  //25:µ¶ÁÖ¸Ó´Ï
                        if user.UseItems[U_CHARM].Dura >= 100 then begin
                           user.UseItems[U_CHARM].Dura := user.UseItems[U_CHARM].Dura - 100;
                           //³»±¸¼º º¯°æÀº ¾Ë¸²
                           user.SendMsg (user, RM_DURACHANGE, U_CHARM, user.UseItems[U_CHARM].Dura, user.UseItems[U_CHARM].DuraMax, 0, '');
                           bhasItem := 1;
                        end;
                     end;
                  end;
                     ...
                     if 6 >= Random(7 + target.AntiPoison) then begin
                        case pstd.Shape of
                           1: //È¸»öµ¶°¡·ç: Áßµ¶
                              begin
                                 //sec = Áßµ¶½Ã°£  60ÃÊ + ¾ËÆÄ
                                 sec := GetPower13 (20) + 2 * GetRPow(user.WAbil.SC);
                                 //pwr := pum.Level;
                                 pwr := Round(pum.Level / 3 * (sec / (lvduweili)));             //ÂÌ¶¾µôÑª¹«Ê½
                           ...
                           2: //È²»öµ¶°¡·ç: ¹æ¾î·Â°¨¼Ò
                              begin
                                 ...
                                 //sec = Áßµ¶½Ã°£ 40ÃÊ + ¾ËÆÄ
                                 sec := GetPower13 (30) + 2 * GetRPow(user.WAbil.SC); //(Lobyte(user.WAbil.SC) + Random(ShortInt(Hibyte(user.WAbil.SC)-Lobyte(user.WAbil.SC)) + 1));
                                 //pwr := 2{pum.Level};
                                 pwr := _MAX(2, _MIN(5, HIBYTE(user.WAbil.SC) div (hongduweili)));
               ...
               if bhasitem = 1 then begin
                  if (not g_bochksigedu) and (user.UseItems[U_BUJUK].Dura < 100) then begin
                     user.UseItems[U_BUJUK].Dura := 0;
                     //ÏûÊ§ÁËµÄÒ©
                     if user.RaceServer = RC_USERHUMAN then begin
                        hum := TUserHuman(user);
                        hum.SendDelItem (user.UseItems[U_BUJUK]); //¿Í»§¶Ë·¢ËÍµÄ
                        //Ä¥ËðÊ±µÄÐÅÏ¢(2004/11/15)
                        {$IFDEF KOREA} hum.SysMsg('ÄãµÄ¶¾ÒÑÓÃ¾¡', 0);
                ...
                if bhasitem = 1 then begin
               if (g_bochksigedu) and (user.UseItems[U_CHARM].Dura < 100) then begin
                     user.UseItems[U_CHARM].Dura := 0;
                     //´Ù ¾´¾àÀº »ç¶óÁø´Ù.
                     if user.RaceServer = RC_USERHUMAN then begin
                        hum := TUserHuman(user);
                        hum.SendDelItem (user.UseItems[U_CHARM]); //Å¬¶óÀÌ¾ðÆ®¿¡ ¾ø¾îÁø°Å º¸³¿
                        //µ¶°¡·ç ´Ù ´â¾ÒÀ» ¶§ ¸Þ½ÃÁö(2004/11/15)
                        {$IFDEF KOREA} hum.SysMsg('ÄãµÄ¶¾ÒÑÓÃ¾¡', 0);
                        {$ELSE}        hum.SysMsg('The Poison item has been exhausted.', 0);
                        {$ENDIF}
                     end;
                     user.UseItems[U_CHARM].Index := 0;
                  end;
              end;
               if(user.UseItems[U_ARMRINGL].Index > 0) and (bhasitem = 2) and (not g_bochksigedu)  then begin
                  pstd := UserEngine.GetStdItem (user.UseItems[U_ARMRINGL].Index);
                  if pstd <> nil then begin
                     if (pstd.StdMode = 25) then begin  //25:¶¾ÄÒ
                        if user.UseItems[U_ARMRINGL].Dura < 100 then begin
                           user.UseItems[U_ARMRINGL].Dura := 0;
                           //ÏûÊ§ÁËµÄÒ©
                           if user.RaceServer = RC_USERHUMAN then begin
                              hum := TUserHuman(user);
                              hum.SendDelItem (user.UseItems[U_ARMRINGL]); //¿Í»§¶Ë·¢ËÍµÄ
                              //Ä¥ËðÊ±µÄÐÅÏ¢(2004/11/15)
                              {$IFDEF KOREA} hum.SysMsg('ÄãµÄ¶¾ÒÑÓÃ¾¡', 0);
                              {$ELSE}        hum.SysMsg('The Poison item has been exhausted.', 0);
                              {$ENDIF}
    ...
    54: //ËÕÉúÊõ
      begin
        ...
        bhasitem := 0;
        pstd := nil;
        if not user.PEnvir.NoRevive then
        begin
          Reslist := TList.Create;
          user.GetMapCreatures(user.PEnvir, xx, yy, 1, Reslist);
          if Reslist.Count = 0 then
          begin
            user.SendRefMsg(RM_MAGICFIRE, 0, MakeWord(pum.pDef.EffectType, pum.pDef.Effect), MakeLong(xx, yy), 0, '');
          ...
          begin
            ...
            begin
              ...
              begin
                if user.RevivalMode = False then
                begin
                  user.SendDelayMsg(user, RM_LOOPNORMALEFFECT, integer(user), 0, 0, NE_REVIVALCHARGE, '', 0); // 1000);
                 // UserEngine.CryCry(RM_CRY, user.PEnvir, xx, yy, 10000, user.UserName + 'ÕýÔÚ³¢ÊÔ¸´»î' + target.UserName);
                  user.SysMsg(target.UserName + 'ÕýÔÚÇëÇóÄã¸´»îËû£¨Ëý£©.', 10);
                ...
                begin
                  if target <> user.RevivalTarget then
                  begin
                    user.RevivalMode := False;
                    user.SendRefMsg(RM_MAGICFIRE, 0, MakeWord(pum.pDef.EffectType, pum.pDef.Effect), MakeLong(xx, yy), 0, '');
                  end
                  ...
                  begin
                    user.RevivalMode := False;
                    user.SendRefMsg(RM_MAGICFIRE, 0, MakeWord(pum.pDef.EffectType, pum.pDef.Effect), MakeLong(xx, yy), integer(target), '');
                    if (user.UseItems[U_BUJUK].Index > 0) then
                    begin
                                  // U_ARMRINGL->U_BUJUK
                      pstd := UserEngine.GetStdItem(user.UseItems[U_BUJUK].Index);
                      if pstd <> nil then
                      begin
                        if (pstd.StdMode = 25) and (pstd.Shape = 6) then
                        begin
                          if user.UseItems[U_BUJUK].Dura >= 100 then
                          begin
                            user.UseItems[U_BUJUK].Dura := user.UseItems[U_BUJUK].Dura - 100;
                            user.SendMsg(user, RM_DURACHANGE, U_BUJUK, user.UseItems[U_BUJUK].Dura, user.UseItems[U_BUJUK].DuraMax, 0, '');
                            bhasitem := 1;
                        ...
                        begin
                          ...
                          hum.SysMsg('Äã±ØÐëÅä±¸¸´»î·û.', 1);
                          exit;
                        end;
                      end;
                    end;
                    if (user.UseItems[U_ARMRINGL].Index > 0) and (bhasitem = 0) then
                    begin
                      pstd := UserEngine.GetStdItem(user.UseItems[U_ARMRINGL].Index);
                      if pstd <> nil then
                      begin
                    ...
                    case pum.Level of
                      0:
                        begin
                          ...
                          target.SendMsg(target,RM_ALIVEREQ,0,Integer(Self),0,0,user.UserName+' ¶ÔÄãÊ¹ÓÃ¸´»îÊõ\ÄãÈ·¶¨Òª¸´»îÂð£¿');
          ...
          begin
            ...
            begin
              user.UseItems[U_BUJUK].Dura := 0;
              if user.RaceServer = RC_USERHUMAN then
              begin
                hum := TUserHuman(user);
                hum.SendDelItem(user.UseItems[U_BUJUK]);
                hum.SysMsg('ÄãµÄ»¤Éí·ûÒÑ¾­ÓÃÍêÁË.', 0);
              end;
              user.UseItems[U_BUJUK].Index := 0;
            end;
          end;
          if (user.UseItems[U_ARMRINGL].Index > 0) and (bhasitem = 2) then
          begin
            pstd := UserEngine.GetStdItem(user.UseItems[U_ARMRINGL].Index);
            if pstd <> nil then
            begin
...
//               if user.IsProperTarget(target) or (target = nil) then begin
        ...
        if GetTickCount - hum.LatestHitTime > sec * 1000 then
        begin
          if (user.UseItems[U_BUJUK].Index > 0) then
          begin
                  // U_ARMRINGL->U_BUJUK
            ...
            if pstd <> nil then
            begin
              if (pstd.StdMode = 25) and (pstd.Shape = 1) then
              begin  //25:µ¶ÁÖ¸Ó´Ï
                if user.UseItems[U_BUJUK].Dura >= 500 then
                begin
...
//                                 sec := GetPower13 (40) + 2 * GetRPow(user.WAbil.SC);
                    sec := (pum.Level + 1) * 5 + 10;
                    pwr := _MAX(0, _MIN(HIBYTE(user.WAbil.SC), _MAX(0, (LOBYTE(user.WAbil.SC) + HIBYTE(user.WAbil.SC)) div 2 + (pum.Level + 1))));
                               //  target.SendDelayMsg(user, RM_MAKEPOISON,
                              //   POISON_DECHEALTH{wparam}, sec, integer(user), pwr, '', 1000);
                    if MagMakePoisonCloud(user, pwr, sec, xx, yy) > 0 then
                    begin
                      train := TRUE;
                      hum.LatestHitTime := GetTickCount;
                    end;
                  end;
                2: //È²»öµ¶°¡·ç: ¹æ¾î·Â°¨¼Ò
                  begin
...
//            end;
        ...
        begin
          waittime := (GetTickCount - hum.LatestHitTime) div 1000;
          user.SysMsg(IntToStr(sec - waittime) + 'ÃëÒÔºóÔÙ³¢ÊÔÊ¹ÓÃ¶¾Îí', 1);
          exit;
        end;
      end;
      //2003/03/15 ½Å±Ô¹«°ø Ãß°¡
         ...
         begin
            ...
            if bhasItem > 0 then begin
               ...
               case pum.pDef.MagicId of
                     ...
                     if user.MagCanHitTarget (user.CX, user.CY, target) then begin
                        if user.IsProperTarget(target) then begin
                           if (target.AntiMagic <= Random(50)) and (abs(target.CX-xx) <= 1) and (abs(target.CY-yy) <= 1) then begin
                              ...
                              //user.SelectTarget (target);
                              user.SendDelayMsg (user, RM_DELAYMAGIC, pwr, MakeLong(xx, yy), 2, integer(target), '', HFJL);
                              if (target.RaceServer >= RC_ANIMAL) then train := TRUE;
                           end;
                        end;
                     end else
                  ...
                  //2003/03/15 ½Å±Ô¹«°ø Ãß°¡
                  36: //ÎÞ¼«ÕæÆø
                  if not g_LIMITLESSQiswitch then

                     begin
                        sec := (pum.Level + 1) * 3;
                        //pwr := Round(((Hibyte(user.WAbil.SC) - 2) / 100) * 100);
                        pwr := Round(Hibyte(user.WAbil.SC)*(pum.Level * 0.2 + 0.4));
                        if user.MagScUp (sec, pwr) then
                           train := TRUE;
                           end else
//                     end;
//                  36: //¹«±ØÁø±â
...
{$ENDIF}
                  ...
                  83: //ÒõÑôÎåÐÐÕó
                     begin
                        pwr := user.GetAttackPower (GetPower (MPow(pum)) + Lobyte(user.WAbil.SC), Integer(Hibyte(user.WAbil.SC)-Lobyte(user.WAbil.SC)) + 1);
                        if MagMakeYinYangMatrix (user, pwr,
                                 GetPower(10) + GetRPow(user.WAbil.SC) div 2,
                                 xx,
                                 yy) > 0 then begin
                          train := TRUE;
                        end;
