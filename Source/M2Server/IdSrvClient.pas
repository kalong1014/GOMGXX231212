...
uses
  Windows, Messages, SysUtils, Classes, Graphics, Controls, Forms, Dialogs, ScktComp, ExtCtrls, IniFiles,
  MudUtil, HUtil32, Grobal2, EDCode;
...
type
  ...
  private
    ...
    procedure GetRecvUserRechargeInfo (body: string);
  public
    ...
    //2017-07-23 념令斤口繫祇
    procedure SendRechargeInfo (uname, uid: string; cert: integer);
    procedure SendRechargeDayAdd (uname, uid: string; cert: integer);
    procedure SendRechargeHoursAdd (uname, uid: string; cert: integer);
  end;
...
procedure TFrmIDSoc.SendRechargeInfo (uname, uid: string; cert: integer);
begin
   if IDSocket.Socket.Connected then
      IDSocket.Socket.SendText ('(' + IntToStr(ISM_RECHARGEINFO) + '/' + uid + '/' + IntToStr(cert) + ')');
end;

procedure TFrmIDSoc.SendRechargeDayAdd (uname, uid: string; cert: integer);
begin
   if IDSocket.Socket.Connected then
      IDSocket.Socket.SendText ('(' + IntToStr(ISM_RECHARGEDAYADD) + '/' + uid + '/' + IntToStr(cert) + ')');
end;

procedure TFrmIDSoc.SendRechargeHoursAdd (uname, uid: string; cert: integer);
begin
   if IDSocket.Socket.Connected then
      IDSocket.Socket.SendText ('(' + IntToStr(ISM_RECHARGEHOURSADD) + '/' + uid + '/' + IntToStr(cert) + ')');
end;
...
procedure TFrmIDSoc.GetRecvUserRechargeInfo (body: string);
var
   sUserId, sDay, sHours, sMiaoka: string;
begin
   try
      body := GetValidStr3 (body, sUserId, [char($a)]);
      body := GetValidStr3 (body, sDay, [char($a)]);
      body := GetValidStr3 (body, sHours, [char($a)]);
      body := GetValidStr3 (body, sMiaoka, [char($a)]);
   //   FrmMain.Memo1.Lines.Add ('GetRecvUserPotCash:'+sUserId+'/'+sPotCash+'/');
      UserEngine.ApplyUserRechargeInfo (sUserId, Str_ToInt(sDay, 0), Str_ToInt(sHours, 0), Str_ToInt(sMiaoka, 0));
   except
