...
uses
  ...
  HUtil32, ObjBase, Grobal2, EdCode, Envir, ObjMon, ObjMon2, objMon3, ObjAxeMon, ObjNpc, ObjGuard,
  M2Share, RunDB, Guild, Mission, MFdbDef, InterServerMsg, InterMsgClient, Event, UserMgr , CmdMgr,
  StallSystem;
...
type
   ...
   TUserEngine = class
   private
      ReadyList: TStringList; //µ¿±âÈ­ ÇÊ¿ä
      RunUserList: TStringList;
      OtherUserNameList: TStringList;  //´Ù¸¥ ¼­¹ö¿¡ ÀÖ´Â »ç¿ëÀÚ ¸®½ºÆ®
      ClosePlayers: TList;
      ...
      missiontime: longword;  //¹Ì¼ÇÀº 1ÃÊ¿¡ ÇÑ¹ø Æ½ÀÌ µÈ´Ù.
      onezentime: longword;  //Á¨À» Á¶±Ý¾¿ ÇÑ´Ù.
      runonetime: longword;
      hum200time: longword;
      usermgrcheck: longword;

      eventitemtime: longword;  //À¯´ÏÅ© ¾ÆÀÌÅÛ ÀÌº¥Æ®ÀÇ º¯¼ö
   ...
   public
      ...
      MerchantList: TList;
      NpcList: TList;
      MissionList: TList;  //¹Ì¼Ç...
      WaitServerList: TList;
      HolySeizeList: TList; //°á°èÀÇ ¸®½ºÆ®
      ...
      //User
      procedure AddNewUser (ui: PTUserOpenInfo); //½ÌÅ©¸Â¿ö¾ßÇÔ
      ...
      function  FindOtherServerUser (who: string; var svindex: integer): Boolean;  //´Ù¸¥¼­¹ö¿¡ Á¢¼ÓÇÏ°í ÀÖ´ÂÁö
      ...
      procedure SysMsgAllEx(saying: string; saytype, fc, bc: integer; fixde: Byte = 0);
      procedure SysMsgAllScroll(saying: string;fc, bc: integer);
      procedure SendQuestMsg(sQuestName: string);
      procedure UserMsgAll (saying: string);
      procedure UserMsgAllHorn (saying: string);
      procedure RefAllUserConfig(saying: string);
      ...
      function  ApplyUserRechargeInfo( uid: string; iDay, iHours, iMiaoka: integer):Boolean;
      //Monster, NPC
      function  GetMonRace (monname: string): integer;
      function  GetMonLevel (monname: string): integer;
      function ApplyMonsterAbility (cret: TCreature; monname: string): TList;
      ...
      function MonGetRandomItems(mon: TCreature; ItemList: TList): Integer;
      ...
      function  GetGenCount (mapname: string): integer; //¸Ê¿¡ ¸î¸¶¸®°¡ Á¨µÇ¾ú´ÂÁö
      ...
      //Merchant
      function  GetMerchant (npcid: integer): TCreature;  //npcid´Â TCreatureÀÓ.
      ...
      function  GetQFunctionNpc (npcid: integer): TCreature;
      ...
      procedure InitializeQFunctionNpc;
      //sys
      function  OpenDoor (envir: TEnvirnoment; dx, dy: integer): Boolean;
      function  CloseDoor(envir: TEnvirnoment; pd: PTDoorInfo): Boolean;
      //¹Ì¼Ç
      function  LoadMission (flname: string): Boolean;  //¹Ì¼Ç ÆÄÀÏÀ» ÀÐ¾î¼­ ¹Ì¼ÇÀ» È®¼ºÈ­ ½ÃÅ´
      function  StopMission (missionname: string): Boolean; //¹Ì¼ÇÀ» Á¾·áÇÑ´Ù. ÀÚµ¿ Á¾·áµÈ´Ù.
      //½ÃÀÛÀ§Ä¡
      ...
      procedure SendWeightConfig;

      procedure ApplyUserLevel( lv :integer );
      procedure ApplyUserGameGold( gold :integer );

      procedure SendBroadCastMsg(sMsg: string; MsgType: Integer = 2);
      function GetPlayObjectExOfAutoGetExp(sAccount: string): TUserHuman;
      function  GetMapHumCount(sMapStr: string): integer;
   end;
...
begin
   ...
   ReadyList := TStringList.Create; //µ¿±âÈ­  ÇÊ¿ä
   StdItemList := TList.Create;  //Index°¡ TUserItem¿¡¼­ ¸®ÆÛ·±½º ÇÏ¹Ç·Î ¼ø¼­°¡ º¯°æµÇ¾î¼­´Â ¾ÈµÈ´Ù.
   CharExpList := TList.Create;
...
begin
   ...
   for i:=0 to CharExpList.Count-1 do
      Dispose (PTCharExpInfo(CharExpList[i]));
   CharExpList.Free;
...
begin
   itemindex := itemindex - 1; //TUserItemÀÇ Index´Â +1µÈ °ÍÀÓ.  0Àº ºó°ÍÀ¸·Î °£ÁÖÇÔ.
...
begin
   ...
   if (index >= 0) and (index < StdItemList.Count) then begin
      Result := PTStdItem(StdItemList[index]); //ÀÌ¸§ÀÌ ¾ø´Â ¾ÆÀÌÅÛÀº »ç¶óÁø ¾ÆÀÌÅÛ
...
begin
   ...
   if (itmindex >= 0) and (itmindex < StdItemList.Count) then begin
      uitem.Index := itmindex + 1;  //Index=0Àº ºó°ÍÀ¸·Î ÀÎ½Ä
...
begin
   ...
   for i:=0 to StdItemList.Count-1 do begin
      if CompareText (PTStdItem(StdItemList[i]).Name, itmname) = 0 then begin
         ...
         uitem.Index := i + 1;  //Index=0Àº ºó°ÍÀ¸·Î ÀÎ½Ä
         uitem.MakeIndex := GetItemServerIndex;  //»õ MakeIndex ¹ß±Þ
...
function TUserEngine.GetShopItemByName(sItemName: string): pTShopItem;
var
  i: Integer;
...
procedure TUserEngine.SendWeightConfig;
var
   i, n: integer;
   hum: TUserhuman;
begin
   ...
   begin
      ...
      if (not hum.BoGhost)  then
      begin
         hum.SendDefMessage(SM_Server_Config,0,0,0,g_WeightSet,'');
      end;
   end;
end;
...
begin
   ...
   if pd <> nil then begin
      if (not pd.pCore.DoorOpenState) and (not pd.pCore.Lock) then begin //ÀÌ¹Ì ¿­·Á ÀÖ°Å³ª, Àá°ÜÀÖÁö¾ÊÀ¸¸é.
...
function  TUserEngine.GetQFunctionNpc (npcid: integer): TCreature;
begin
   Result := nil;
   if Integer(QFunctionNpc) = npcid then begin
      Result := TCreature(QFunctionNpc);
   end;
end;
...
procedure TUserEngine.InitializeQFunctionNpc;
var
  npc: TNormNpc;
begin
  npc := TNormNpc(QFunctionNpc);
  npc.Penvir := GrobalEnvir.GetEnvir (npc.MapName);
  if npc.Penvir <> nil then begin
    npc.Initialize;
    if npc.ErrorOnInit and not npc.BoInvisible then begin
      MainOutMessage ('QFunctionNpc Initalize fail... ' + npc.UserName);
      npc.Free;
    end else begin
      npc.LoadNpcInfos;
    end;
  end else begin
    MainOutMessage ('QFunctionNpc Initalize fail... [Mapinfo or Map] (npc.PEnvir=nil) ' + npc.UserName);
    npc.Free;
  end;
end;
...
function TUserEngine.ApplyMonsterAbility(cret: TCreature; monname: string): TList;
var
  i: integer;
  pm: PTMonsterInfo;
begin
  Result := nil;
  for i := 0 to MonDefList.Count - 1 do
  begin
    if CompareText(PTMonsterInfo(MonDefList[i]).Name, monname) = 0 then
    begin
...
//  for i := 0 to MonDefList.Count - 1 do
//  begin
//    if CompareText(PTMonsterInfo(MonDefList[i]).Name, mon.UserName) = 0 then
//    begin
...
////            RealMaxPoint := Random(_MAX(1, pmi.MaxPoint div 5))  //²âÊÔÄ£Ê½£¬±¬ÂÊÌá¸ß5±¶
////         else
//      RealMaxPoint := Random(pmi.MaxPoint);
//      if pmi.SelPoint >= RealMaxPoint then
//      begin
//        if CompareText(pmi.ItemName, NAME_OF_MONEY) = 0 then
//        begin
...
//            iname := pmi.ItemName;
//          new(pu);
//          if CopyToUserItemFromName(iname, pu^) then
//          begin
//            pu.Dura := Round((pu.DuraMax / 100) * (20 + Random(80)));
//            pstd := GetStdItem(pu.Index);
//            if Random((jpzonggailv)) = 1 then    //¹ÖÎï¼«Æ·±¬ÂÊ£¬ÊýÖµÔ½Ð¡¸ÅÂÊÔ½¸ß Ä¬ÈÏ20
//              RandomUpgradeItem(pu);
//            if pstd <> nil then
//            begin
//              if pstd.StdMode in [15, 19, 20, 21, 22, 23, 24, 26, 52, 53, 54] then
//              begin
//                if (pstd.Shape = RING_OF_UNKNOWN) or (pstd.Shape = BRACELET_OF_UNKNOWN) or (pstd.Shape = HELMET_OF_UNKNOWN) then
//                begin
...
function TUserEngine.MonGetRandomItems(mon: TCreature; ItemList: TList): Integer;
  ...
  begin
    ...
    begin
     if g_WemadeGoldEvent then begin
       mon.Gold := mon.Gold + (((MonItem.Count div 2) + Random(MonItem.Count))*(1 + g_ExtraMoneyPer div 100));
     end
       else begin
      mon.Gold := mon.Gold + (MonItem.Count div 2) + Random(MonItem.Count);
     end;
    end
    else
    begin
      New(UserItem);
      if CopyToUserItemFromName(MonItem.MonItemName, UserItem^) then
      begin
        StdItem := GetStdItem(UserItem.Index);
        UserItem.Dura := Round((UserItem.DuraMax / 100) * (20 + Random(80)));
        if Random((jpzonggailv)) = 0 then    //¹ÖÎï¼«Æ·±¬ÂÊ£¬ÊýÖµÔ½Ð¡¸ÅÂÊÔ½¸ß Ä¬ÈÏ20
          RandomUpgradeItem(UserItem);
        if StdItem <> nil then
        begin
          if StdItem.StdMode in [15, 19, 20, 21, 22, 23, 24, 26, 52, 53, 54] then
          begin
            if (StdItem.Shape = RING_OF_UNKNOWN) or (StdItem.Shape = BRACELET_OF_UNKNOWN) or (StdItem.Shape = HELMET_OF_UNKNOWN) then
            begin
              UserEngine.RandomSetUnknownItem(UserItem);
            end;
          end;
          if StdItem.OverlapItem >= 1 then
          begin
            UserItem.Dura := 1;
          end;
        end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          GetItemsList(MonItem.List, MonItem.boItemDropRandom);
        end
        else
          RefItems(MonItem);
      end;
    ...
    begin
      ...
      begin
        ...
        //nMaxPoint:= MonItem.MaxPoint;
       // nMaxPoint := Round(MonItem.MaxPoint / 100 * g_ExtraItemPer);
        if g_WemadeItemEvent then
        begin
          nMaxPoint := Round(MonItem.MaxPoint / ((100 + g_ExtraItemPer) / 100));
        end
        else
        begin
          nMaxPoint := MonItem.MaxPoint;
        ...
        if Random(nMaxPoint) <= MonItem.SelPoint then
        begin
          if MonItem.List <> nil then
          begin
            GetItemsList(MonItem.List, MonItem.boItemDropRandom);
...
begin
   ...
   case race of
      ...
      RC_ANIMAL+1:  //´ß
         begin
            ...
            cret.BoAnimal := TRUE;
            cret.MeatQuality := 3000 + Random(3500); //±âº»°ª.
            cret.BodyLeathery := 50; //±âº»°ª
         end;
      RC_RUNAWAYHEN: //´Þ¾Æ³ª´Â ´ß(sonmg 2004/12/27)
         begin
            cret := TChickenDeer.Create; //´Þ¾Æ³²
            cret.BoAnimal := TRUE;
            cret.MeatQuality := 3000 + Random(3500); //±âº»°ª.
            cret.BodyLeathery := 50; //±âº»°ª
         end;
      RC_DEER:  //»ç½¿
         begin
            if Random(30) = 0 then begin
               cret := TChickenDeer.Create; //°ÌÀïÀÌ »ç½¿, ´Þ¾Æ³²
               cret.BoAnimal := TRUE;
               cret.MeatQuality := 10000 + Random(20000);
               cret.BodyLeathery := 150; //±âº»°ª
            end else begin
               cret := TMonster.Create;
               cret.BoAnimal := TRUE;
               cret.MeatQuality := 8000 + Random(8000); //±âº»°ª.
               cret.BodyLeathery := 150; //±âº»°ª
            end;
         end;
      RC_WOLF:
      ...
      RC_DUALAXESKELETON: //½Öµµ³¢ÇØ°ñ
         begin
            cret := TDualAxeMonster.Create;
         end;
      RC_HEAVYAXESKELETON: //Å«µµ³¢ÇØ°ñ
         begin
            cret := TATMonster.Create;
         end;
      RC_KNIGHTSKELETON: //ÇØ°ñÀü»ç
         begin
            cret := TATMonster.Create;
         end;
      RC_BIGKUDEKI: //´ëÇü±¸µ¥±â
         begin
      ...
      RC_WHITESKELETON:
         begin
            cret := TWhiteSkeleton.Create; //¼ÒÈ¯¹é°ñ
         end;

      RC_ANGEL:
         begin
            cret := TAngelmon.Create; // Ãµ³à(¿ù·É)
         end;

      RC_CLONE:
         begin
            cret := TClonemon.Create; //ºÐ½Å
         end;

      RC_FIREDRAGON:
         begin
            cret := TDragon.Create; //È­·æ
         end;

      RC_DRAGONBODY:
         begin
            cret := TDragonBody.Create; //È­·æ¸ö
         end;
      RC_DRAGONSTATUE:
         begin
            cret := TDragonStatue.Create; //¿ë¼®»ó
         end;
      ...
      RC_SCULKING_2:
         begin
            cret := TScultureKingMonster_2.Create;
            TScultureKingMonster_2(cret).BoCallFollower := FALSE;
         end;

      RC_BEEQUEEN:
         begin
            cret := TBeeQueen.Create;   //¹úÅë
         end;

      RC_ARCHERMON://Ó¥ÎÀ
         begin
            cret := TArcherMonster.Create; //¸¶±Ã»ç
         end;

      RC_GASMOTH:  //°¡½º½î´Â ½û±â³ª¹æ
         begin
      ...
      RC_BIGHEARTMON:
         begin
            cret := TBigHeartMonster.Create;  //Ç÷°Å¿Õ, ½ÉÀå
         end;
      ...
      RC_SPIDERHOUSEMON:
         begin
            cret := TSpiderHouseMonster.Create;  //°Å¹ÌÁý,  Æø¾È°Å¹Ì
         end;

      RC_EXPLOSIONSPIDER:
         begin
            cret := TExplosionSpider.Create;  //ÆøÁÖ
         end;
      ...
      RC_BLACKSNAKEKING:   //Èæ»ç¿Õ, ´õºí °ø°Ý
         begin
            cret := TDoubleCriticalMonster.Create;
         end;

      RC_NOBLEPIGKING:     //±Íµ·¿Õ, °­·Â °ø°Ý(´õºí ¾Æ´Ô)
         begin
            cret := TATMonster.Create;
         end;

      RC_FEATHERKINGOFKING:  //ÈæÃµ¸¶¿Õ
         begin
            cret := TDoubleCriticalMonster.Create;
         end;

      // 2003/02/11 ÇØ°ñ ¹Ý¿Õ, ºÎ½Ä±Í, ÇØ°ñº´Á¹
      RC_SKELETONKING:  //ÇØ°ñ¹Ý¿Õ
         begin
            cret := TSkeletonKingMonster.Create;
         end;
      RC_TOXICGHOST:  //ºÎ½Ä±Í
         begin
            cret := TGasAttackMonster.Create;
         end;
      RC_SKELETONSOLDIER:  //ÇØ°ñº´Á¹
         begin
            cret := TSkeletonSoldier.Create;
         end;
      // 2003/03/04 ¹Ý¾ßÁÂ»ç, ¿ì»ç, »ç¿ìÃµ¿Õ
      RC_BANYAGUARD:  //¹Ý¾ßÁÂ/¿ì»ç
         begin
            cret := TBanyaGuardMonster.Create;
         end;
      RC_DEADCOWKING:  //»ç¿ìÃµ¿Õ
         begin
      ...
      RC_PBKING: //°ú°ÅºñÃµ º¸½º
         begin
            cret := TPBKingMonster.Create;
         end;
      RC_GOLDENIMUGI: //È²±ÝÀÌ¹«±â(ºÎ·æ±Ý»ç)
         begin
            cret := TGoldenImugi.Create;
         end;
      RC_GOLDENIMUGI2: //È²±ÝÀÌ¹«±â(ºÎ·æ±Ý»ç)
         begin
            cret := TGoldenImugi2.Create;
         end;
      ...
      RC_ARCHERGUARD:  //±Ã¼ö°æºñ
         begin
            cret := TArcherGuard.Create;
         end;
      RC_ARCHERMASTER:  //±Ã¼öÈ£À§º´
         begin
            cret := TArcherMaster.Create;
         end;

      RC_ARCHERPOLICE:  //±Ã¼ö°æÂû
         begin
      ...
      RC_ELFMON:
         begin
            cret := TElfMonster.Create;  //½Å¼ö º¯½ÅÀü
         end;

      RC_ELFWARRIORMON:
         begin
            cret := TElfWarriorMonster.Create;  //½Å¼ö º¯½ÅÈÄ
         end;
      ...
      RC_EYE_PROG:      //»ç¾ÈÃæ -> ¼³ÀÎ´ëÃæ
         begin
            cret := TEyeProg.Create;
         end;
      RC_STON_SPIDER:   //È¯¸¶¼®°Å¹Ì -> ½Å¼®µ¶¸¶ÁÖ
         begin
            cret := TStoneSpider.Create;
         end;
      RC_GHOST_TIGER:   //È¯¿µÇÑÈ£
         begin
            cret := TGhostTiger.Create;
         end;
      RC_JUMA_THUNDER:  //ÁÖ¸¶·Ú°ÝÀå -> ÁÖ¸¶°Ý·ÚÀå
         begin
      ...
      RC_FOXWARRIOR:  //ºñ¿ù¿©¿ì(Àü»ç) ºñ¿ùÈæÈ£
         begin
            cret := TFoxWarrior.Create;
         end;
      RC_FOXWIZARD:  //ºñ¿ù¿©¿ì(¼ú»ç) ºñ¿ùÀûÈ£
         begin
            cret := TFoxWizard.Create;
         end;
      RC_FOXTAOIST:  //ºñ¿ù¿©¿ì(µµ»ç) ºñ¿ù¼ÒÈ£
         begin
   ...
   if cret <> nil then begin
      cret.m_CanDropItemList := ApplyMonsterAbility (cret, monname);
      cret.Penvir := env;
      cret.MapName := map;
      cret.CX := x;
      cret.CY := y;
...
//      MonGetRandomItems (cret);
      ...
      cret.Initialize;
      if cret.ErrorOnInit then begin //Á¨ÀÚ¸®°¡ ¸ø¿òÁ÷ÀÌ´Â ÀÚ¸®
         outofrange := nil;
...
begin
   ...
   try
      ...
      if pz.MonRace > 0 then begin
         if Random(100) < pz.SmallZenRate then begin //Á¨ÀÌ ¸ô·Á¼­ µÈ´Ù.
            ...
            for i:=0 to n-1 do begin
               ...
               if cret <> nil then begin
                  ...
                  //cret = nilÀÌ¸é...
//¿Õ¸÷Á¨ ½ºÅµ ¸ð´ÏÅÍ¸µ(sonmg)
if (pz.MonRace = RC_SKELETONKING) then begin
   MainOutMessage('RegenMon Nil : ÇØ°ñ¹Ý¿Õ-NIL');
...
if (pz.MonRace = RC_DEADCOWKING) then begin
   MainOutMessage('RegenMon Nil : »ç¿ìÃµ¿Õ-NIL');
   MainOutMessage(pz.MapName + ' ' +
                  IntToStr(zzx) + ',' +
                  IntToStr(zzy) + ' ' +
                  IntToStr(pz.MonRace) + ' ' +
...
if (pz.MonRace = RC_FEATHERKINGOFKING) then begin
   MainOutMessage('RegenMon Nil : ÈæÃµ¸¶¿Õ-NIL');
   MainOutMessage(pz.MapName + ' ' +
                  IntToStr(zzx) + ',' +
                  IntToStr(zzy) + ' ' +
                  IntToStr(pz.MonRace) + ' ' +
...
if (pz.MonRace = RC_PBKING) then begin
   MainOutMessage('RegenMon Nil : ÆÄÈ²¸¶½Å-NIL');
   MainOutMessage(pz.MapName + ' ' +
                  IntToStr(zzx) + ',' +
                  IntToStr(zzy) + ' ' +
                  IntToStr(pz.MonRace) + ' ' +
   ...
   if Result = TRUE then begin
      if (GetMonLevel( pz.MonName ) = 70) or
            (pz.MonName = '¿ì¸é±Í¿Õ') or (pz.MonName = 'ÃË·æ½Å') or (pz.MonName = 'ÇØ°ñ¹Ý¿Õ') or
            (pz.MonName = '»ç¿ìÃµ¿Õ') or (pz.MonName = 'ÁÖ¸¶¿Õ') or (pz.MonName = 'Àû¿ù¸¶') or
            (pz.MonName = 'Èæ»ç¿Õ') or (pz.MonName = '±Íµ·¿Õ') or (pz.MonName = '±¯ÔÂÌìÖé') or
            (pz.MonName = 'ÆÄÈ²¸¶½Å') then begin
         MainOutMessage('Ë¢ÐÂ: ' + pz.MonName + ' (' + pz.MapName + ') ' + TimeToStr(Now));
      end;
   end;
end;
...
procedure TUserEngine.SysMsgAllEx(saying: string; saytype, fc, bc: integer; fixde: Byte);
var
   i: integer;
   hum: TUserhuman;
begin
   for i:=0 to RunUserList.Count-1 do begin
      hum := TUserHuman(RunUserList.Objects[i]);
      if( not hum.BoGhost) then begin
         hum.SysMsg (saying, saytype, fc, bc, fixde);
      end;
   end;
end;

procedure TUserEngine.SysMsgAllScroll(saying: string; fc, bc: integer);
var
   i: integer;
   hum: TUserhuman;
begin
...
begin
  for i := 0 to RunUserList.Count - 1 do begin
    hum := TUserHuman(RunUserList.Objects[i]);
    if not hum.Death and not hum.BoGhost then
      TMerchant(DefaultNpc).NpcSayTitle(hum, sQuestName);
  end;
end;
...
procedure TUserEngine.UserMsgAllHorn(saying: string);
var
   i: integer;
   hum: TUserhuman;
begin
...
procedure TUserEngine.RefAllUserConfig(saying: string);
var
   i, ii: integer;
   hum: TUserhuman;
...
begin
  ...
  begin
    ...
    begin
      ...
      MirConfig.g_boDiableHumanRun := g_boDiableHumanRun;
      MirConfig.g_boRUNHUMAN := g_boRUNHUMAN;
      MirConfig.g_boRUNMON := g_boRUNMON;
      MirConfig.g_boRunNpc := g_boRunNpc;
      MirConfig.g_boRunGuard := g_boRunGuard;
      MirConfig.g_boWarDisHumRun := g_boWarDisHumRun;
      MirConfig.g_boGMRunAll := g_boGMRunAll;
      MirConfig.g_boSafeAreaLimited := g_boSafeAreaLimited;
      ...
      hum.SendDefMessage(SM_MIRCONFIG, 0, 0, 0, hum.Bright, TempStr);
      hum.SendRefMsg(RM_CLOSEHEALTH, 0, 0, 0, 0, '');
      if g_boÈËÎïËÄ¸ñ then
      begin
        if hum.UseItems[U_ARMRINGL].Index > 0 then
        begin
          ps := GetStdItem(hum.UseItems[U_ARMRINGL].Index);
          if ps <> nil then
          begin
            if ps.StdMode = 25 then
            begin
              New(PUserItem);
              PUserItem^ := hum.UseItems[U_ARMRINGL];
              if not hum.AddItem(PUserItem) then
                hum.ItemList.Insert(0, PUserItem)
              else hum.SendAddItem (PUserItem^);
              hum.UseItems[U_ARMRINGL].Index := 0;
            end;
          end;
        end;
//        hum.SendBagItems;
        hum.SendUseItems;
        hum.SendRefMsg (RM_ABILITY, 0, 0, 0, 0, '');
        hum.SendRefMsg (RM_SUBABILITY, 0, 0, 0, 0, '');
        hum.RecalcAbilitys;
//        if hum.m_StallMgr.OnSale then
//          hum.UserRequestClose := TRUE;
      end
      else
      begin
        for ii := U_BUJUK to U_CHARM do
        begin
          if hum.UseItems[ii].Index > 0 then
          begin
            New(PUserItem);
            PUserItem^ := hum.UseItems[ii];
            if not hum.AddItem(PUserItem) then
              hum.ItemList.Insert(0, PUserItem)
            else hum.SendAddItem (PUserItem^);
            hum.UseItems[ii].Index := 0;
...
//          hum.UserRequestClose := TRUE;
    ...
    if not bo°ÚÌ¯ then begin
      if  hum.m_StallMgr.OnSale then begin
        hum.m_StallMgr.OnSale := False;
        StallInfo.open := False;

        m_DefMsg := MakeDefaultMsg(SM_OPENSTALL, Integer(hum), hum.cX, hum.cY, hum.dir);
        hum.SendSocket(@m_DefMsg, EncodeBuffer(@StallInfo, SizeOf(TStallInfo)));
        hum.SendRefMsg(RM_STALLSTATUS, 0,hum.CX,hum.cY, hum.dir, '');
      end;
    end;
...
procedure TUserEngine.CheckHolySeizeValid;  //°á°è°¡ ±úÁ³´ÂÁö °Ë»çÇÑ´Ù.
var
   i, k: integer;
   phs: PTHolySeizeInfo;
   cret: TCreature;
begin
   for i:=HolySeizeList.Count-1 downto 0 do begin
      ...
      if phs <> nil then begin
         for k:=phs.seizelist.Count-1 downto 0 do begin  //°á°è¿¡ °É¸° ¸ó½ºÅÍ°¡ Á×¾ú°Å³ª, Ç®·È´ÂÁö °Ë»ç
...
begin
   ...
   for i:=0 to hum.SlaveList.Count-1 do begin
      ...
      if i <= 4 then begin
         ...
         sui.Slaves[i].RemainRoyalty := (cret.MasterRoyaltyTime - GetTickCount) div 1000;  //ÃÊ´ÜÀ§
   ...
   for i:=0 to EXTRAABIL_SIZE - 1 do begin
      ...
      sui.ExtraAbilTimes[i] :=  hum.ExtraAbilTimes[i]- GetTickCount//³²Àº ½Ã°£¸¸ ÀúÀåÇÔ
...
begin
   ...
   for i:=0 to 4 do
      if psui.Slaves[i].SlaveName <> '' then begin
         ...
         pslave^ := psui.Slaves[i];
            // 2003/06/12 ½½·¹ÀÌºê ÆÐÄ¡
            hum.PrevServerSlaves.Add (pslave);  //½º·¹µå¿¡ ¾ÈÀüÇÏÁö ¾ÊÀ½
   ...
   for i:=0 to EXTRAABIL_SIZE-1 do begin
      ...
      hum.ExtraAbilTimes[i] := psui.ExtraAbilTimes[i] + GetTickCount  //ÀúÀåµÈ ½Ã°£Àº ³²Àº ½Ã°£ÀÓ
      else
...
begin
   usIMLock.Enter;
   try
       if ServerIndex = 0 then begin  //¸¶½ºÅÍ ¼­¹öÀÎ°æ¿ì
          FrmSrvMsg.SendServerSocket (msgstr);
       end else begin  //½½·¡ÀÌºê ¼­¹öÀÎ°æ¿ì
...
begin
   ...
   if flname <> '' then begin
      ...
      hum.TempStr := flname;  //³ªÁß¿¡ ÀÌµ¿ÇÏ·Á´Â ¼­¹ö¿¡¼­ Àß ¹Þ¾Ò´ÂÁö È®ÀÎÇÏ´Âµ¥ ¾²ÀÓ
...
begin
   ...
   // 2003/03/18 Å×½ºÆ® ¼­¹ö ÀÎ¿ø Á¦ÇÑ
   if BoTestServer then begin                  // gadget:Å×½ºÆ®¼­¹ö¿ë
...
begin
   ...
   for i:=0 to RunUserList.Count-1 do begin
      if CompareText (TUserHuman(RunUserList.Objects[i]).UserId, uid) = 0 then begin
         Result := TUserHuman(RunUserList.Objects[i]).SetExpiredTime(60); // ÃÊ ÀÔ·Â
         break;
      end;
   end;
end;
...
function TUserEngine.ApplyUserRechargeInfo( uid: string; iDay, iHours, iMiaoka: integer):Boolean;
var
   i: integer;
   hum: TUserHuman;
begin
   ...
   for i:=0 to RunUserList.Count-1 do begin
      hum := TUserHuman(RunUserList.Objects[i]);
      if CompareText (hum.UserId, uid) = 0 then begin
         if iDay > 0 then  hum.iDay := iDay
         else hum.iDay := 0;
         if iHours > 0 then  hum.iHours := iHours
         else hum.iHours := 0;
         if iMiaoka > 0 then hum.SecondsCard := iMiaoka
         else hum.SecondsCard := 0;
         hum.RecalcAbilitys;
         Result := TRUE;
         break;
      end;
...
procedure TUserEngine.ApplyUserLevel( lv :integer );
var
   i: integer;
   hum: TUserhuman;
begin
   for i:=0 to RunUserList.Count-1 do begin
      hum := TUserHuman(RunUserList.Objects[i]);
      if( not hum.BoGhost) then begin
         hum.CmdManLevelChange (hum.UserName, lv);
      end;
   end;
end;

procedure TUserEngine.ApplyUserGameGold( gold :integer );
var
   i: integer;
   hum: TUserhuman;
begin
...
procedure TUserEngine.ProcessUserHumans;
   ...
   begin
      Result := FALSE;
      if FrontEngine.IsDoingSave (uname) then begin  //¾ÆÁ÷ ÀúÀåÀÌ Ã¤ µÇÁö ¾Ê¾ÒÀ½
         Result := TRUE;
         exit;
      end;
      for k:=0 to RunUserList.Count-1 do begin
         if CompareText (RunUserList[k], uname) = 0 then begin  //ÇöÀç Á¢¼ÓÁß
            Result := TRUE;
            break;
         end;
      end;
   ...
   begin
      ...
      try
         ...
         if pshift = nil then begin //¼­¹ö ÀÌµ¿ÀÌ ¾Æ´Ô
            FDBLoadHuman (@pui.Rcd, hum);
            hum.RaceServer := RC_USERHUMAN;
            if hum.HomeMap = '' then begin //¾Æ¹«°Íµµ ¼³Á¤µÇ¾î ÀÖÁö ¾ÊÀ½...
               ERROR_MAP:
               GetRandomDefStart (hmap, hum.HomeX, hum.HomeY);
               hum.HomeMap := hmap;

               hum.MapName := hum.HomeMap;    //HomeMapÀ» ±âÁØÀ¸·Î
               hum.CX := hum.GetStartX;
               hum.CY := hum.GetStartY;

               if hum.Abil.Level = 0 then begin  //¾ÆÀÌµð¸¦ Ã³À½ ¸¸µç °æ¿ì
            ...
            if mapenvir <> nil then begin
               ...
               if mapenvir.Fight3Zone  then begin
                  //Á×Àº °æ¿ì
            ...
            //2001-03-21ÀÏ »¡°»ÀÌ ¼öÄ¡, °æÄ¡ Á¶Á¤À¸·Î DBÀÏ°ý ¼öÁ¤µÊ
...
//4/16ÀÏ ºÎÅÍ Àû¿ë
            ...
            if hum.BonusApply <= 3 then begin
               ...
               hum.MapName := hum.HomeMap;  //¸¶À»¿¡¼­ ½ÃÀÛÇÏ°Ô ÇÑ´Ù. (Ã¼·ÂÀÌ ¶³¾îÁ® ÀÖ±â ¶§¹®¿¡)
               hum.CX := hum.HomeX - 2 + Random(5);
               hum.CY := hum.HomeY - 2 + Random(5);
            end;
{$ENDIF}
            ...
            if hum.Abil.HP <= 0  then begin
               ...
               if hum.PKLevel < 2 then begin
                  ...
                  //ÇÇÄÉÀÌ´Â ¼º¹Û¿¡¼­ ½ÃÀÛÇÑ´Ù.
                  hum.MapName := BADMANHOMEMAP; //hum.HomeMap;
                  hum.CX := BADMANSTARTX - 6 + Random(13);   //À¯¹èÁö
            ...
            mapenvir := GrobalEnvir.ServerGetEnvir (ServerIndex, hum.MapName);
            if mapenvir = nil then begin // ..
               //ÇØ´ç¸ÊÀÌ ´Ù¸¥ ¼­¹ö¿¡ ÀÖ´Â°æ¿ì (¼­¹ö ÀÌµ¿ÇØ¾ß ÇÔ)
               ...
               hum.WAbil := hum.Abil; //±âº» ÃÊ±âÈ­
               hum.ChangeToServerNumber := GrobalEnvir.GetServer (hum.MapName);

               //Å×½ºÆ®
               if hum.Abil.HP <> 14 then  //Á×¾î¼­ µé¾î¿Â°Ô ¾Æ´Ô
               ...
               if not mapenvir.CanWalk (hum.CX, hum.CY, TRUE) then begin
                  ...
                  //°ÉÀ» ¼ö ¾ø´Â ¸ÊÀÎ °æ¿ì(Àß¸øµÈ ÁÂÇ¥)
                  hum.MapName := DefHomeMap;    //ÀÌ ¼­¹ö¿¡ ²À ÀÖ´Â ¸ÊÀÌ¾î¾ß ÇÔ
                  mapenvir := GrobalEnvir.GetEnvir (DefHomeMap);  //²À ÀÖÀ½.
                  hum.CX := DefHomeX;
                  hum.CY := DefHomeY;
               end;
            end;
            ...
            hum.ReadyRun := FALSE; //ÃÊ±âÈ­°¡ µÇ¾î¾ß ÇÑ´Ù´Â Ç¥½Ã

         end else begin
            //pui : DB ¼­¹ö¿¡¼­ ÀÐÀº µ¥ÀÌÅ¸
            //pshift : ¼­¹ö ÀÌµ¿À¸·Î ÀÐÀº µ¥ÀÌÅ¸
//          FDBLoadHuman (@pui.Rcd, hum);
            FDBLoadHuman (@pshift.Rcd, hum);

            //map, hp µîÀº ¼­¹öÀÌµ¿ÀÇ µ¥ÀÌÅ¸¸¦ »ç¿ëÇÑ´Ù. ¼­¹öÀÌµ¿ÀÇ µ¥ÀÌÅ¸¸¦ ÀÐÁö ¸øÇßÀ» °æ¿ì
            //Àß ¸øµÉ °¡´É¼ºÀÌ ÀÖÀ½.
            ...
            hum.Abil.FameCur   := pshift.rcd.Block.DBHuman.Abil_FameCur;   //ÇöÀç ¸í¼ºÄ¡(2004/10/22)
            hum.Abil.FameBase  := pshift.rcd.Block.DBHuman.Abil_FameBase;  //´©Àû ¸í¼ºÄ¡(2004/10/22)

            //¼­¹ö ÀÌµ¿ÀüÀÇ µ¥ÀÌÅ¸¸¦ ÀÐÀ½
            ...
            hum.ReadyRun := FALSE; //ÃÊ±âÈ­°¡ µÇ¾î¾ß ÇÑ´Ù´Â Ç¥½Ã
            hum.LoginSign := TRUE; //¼­¹öÀÌµ¿Àº °øÁö»çÇ× ¾Èº¸ÀÌ°Ô
...
begin
   ...
   bugcount := 6;
                  ...
                  cuglist.Add (pointer(TUserHuman(ReadyList.Objects[i]).GateIndex)); //thread lockdownÀ» ÇÇÇÏ±â À§ÇØ¼­
   ...
   try
      ...
      for i:=0 to ClosePlayers.Count-1 do begin
         ...
         if GetTickCount - hum.GhostTime > 5 * 60 * 1000 then begin
            try
               TUserHuman(ClosePlayers[i]).Free;  //ÀÜ»óÀÌ ³²´Â´Ù¸é ¿¡·¯°¡³¯ ¼ö ÀÖ´Ù.
            except
            ...
            if hum.BoChangeServer then begin
               if hum.BoSaveOk then begin   //ÀúÀåÀ» ÇÏ°í ³­ ÈÄ¿¡ ¼­¹ö ÀÌµ¿À» ½ÃÅ²´Ù.
            ...
            if hum.BoChangeServerNeedDelay then begin
               if (hum.BoChangeServerOK) or (GetTickCount - hum.ChangeServerDelayTime > 10 * 1000) then begin
                  hum.ClearAllSlaves;  //ºÎÇÏµéÀ» ¸ðµÎ ¾ø¾Ø´Ù.
               ...
               if not hum.LoginSign then begin
                  ...
                  try
                     ...
                     if not hum.ReadyRun then begin
                        hum.Initialize;  //Ä³¸¯ ¼³Á¤À» Á¡°ËÇÏ°í ·Î±×ÀÎ
                        hum.ReadyRun := TRUE;
                     end else begin
                     ...
                     down := 7;
                        ...
                        if ( not FrontEngine.HasServerHeavyLoad ) and  // ÀúÀå°£°Ý 10ºÐ¿¡¼­ 15ºÐ º¯°æ ->30ºÐ º¯°æ
                           ( GetTickCount > ( 30 * 60 * 1000 + hum.LastSaveTime ) ) then // À½¼ö°¡  ³ª¿Ã¼ö ÀÖÀ¸¹Ç·Î º¯°æ
                        begin
                           hum.LastSaveTime := GetTickCount + LongWord(random( 10 * 60 * 1000 )); // 1 ºÐ ·£´ý->10ºÐ ¹ø°æ
                     down := 8;
                           hum.ReadySave;
                     down := 9;
                           SavePlayer (hum);
   ...
   Inc (HumRotCount);
   if HumCur = 0 then begin  //ÇÑ¹ÙÄû µµ´Âµ¥ °É¸®´Â ½Ã°£
      HumRotCount := 0;
...
procedure TUserEngine.ProcessMonsters;
   function GetZenTime (ztime: longword): longword;  //»ç¿ëÀÚÀÇ ¼ö¿¡ µû¶ó¼­ Á¨ÀÇ ºü¸£±â°¡ ¹Ù²ñ.
   var
      r: Real;
   begin
      if ztime < 30 * 60 * 1000 then begin
         r := (GetUserCount - UserFullCount) / ZenFastStep;  //¸Å 200¸íÀÌ ´Ã¶§¸¶´Ù 10%¾¿ ¸÷À» ´õ Á¨ ½ÃÅ´
         if r > 0 then begin
            if r > 6 then r := 6;
...
begin
   ...
   try
      ...
      pz := nil;
      if GetTickCount - onezentime > 200 then begin   //Ë¢¹ÖÊ±¼äÄ¬ÈÏÊÇ200
         onezentime := GetTickCount;
         ...
         if pz <> nil then  begin
            if (pz.MonName <> '') and (not BoVentureServer) then begin //¸ðÇè¼­¹ö¿¡¼­´Â ÀéÀÌ ¾ÈµÈ´Ù.
         ...
         while TRUE do begin
            ...
            if not cret.BoGhost then begin
               if tcount - cret.RunTime > cret.RunNextTick then begin
                  ...
                  try   // 2003-09-09  PDS ¿¡·¯¹ß»ý½Ã ¸ó½ºÅÍ ¸®½ºÆ®¿¡¼­ »èÁ¦
                      cret.Run;
                      Inc (MonCurRunCount);
                  except
                      pz.Mons.Delete (k);
...
procedure TUserEngine.ProcessQFunctionNpc;
var
  tcount: integer;
  cret: TCreature;
begin
...
begin
   if StartPoints.Count > 0 then begin
      ...
      if PHILIPPINEVERSION then n := 0;   //ÇÊ¸®ÇÉ ¹öÁ¯ÀÌ¸é ½ÃÀÛ Æ÷ÀÎÆ® °íÁ¤(sonmg 2006/03/20)
...
var
   ...
   pz: PTZenInfo;
begin
   LoadRefillCretInfos; //¸ó½ºÅÍ ¸®Á¨ Á¤º¸¸¦ ÀÐ´Â´Ù.
   InitializeMerchants;
   InitializeNpcs;
   InitializeDefaultNpcs;
   InitializeQFunctionNpc;
...
begin
   ...
   down := 8;
      if GetTickCount - timer10min > 10 * 60 * 1000 then begin //10ºÐ¿¡ ÇÑ ¹ø
         timer10min := GetTickCount;
         NoticeMan.RefreshNoticeList;
         MainOutMessage (DateTimeToStr(Now) + ' ÔÚÏßÈËÊý = ' + IntToStr(GetUserCount));
//         UserEngine.SendBroadCastMsg('µ±Ç°·þÎñÆ÷ÕæÊµÔÚÏßÈËÊý£º' + intTostr(UserEngine.GetUserCount * 2 )+'ÈË.');
         UserCastle.SaveAll;
         //Àå¿ø²Ù¹Ì±â ¾ÆÀÌÅÛ ³»±¸ ¾÷µ¥ÀÌÆ®
         Inc(gaDecoItemCount);
         if gaDecoItemCount >= 6 then gaDecoItemCount := 0; //6*10ºÐ = 1½Ã°£¿¡ ÇÑ ¹ø
        ...
        if g_WemadeGoldEvent or g_WemadeItemEvent then  begin
           UserEngine.SendBroadCastMsg('(*)ÏµÍ³»î¶¯½ð±Ò±©ÂÊÌáÉý£º' + IntToStr(g_ExtraMoneyPer) +'%,'+ 'ÎïÆ·±©ÂÊÌáÉý£º'+ IntToStr(g_ExtraItemPer) +'%', 11);
           end;
{$IFDEF DEBUG} //sonmg
...
{$ENDIF}
   ...
   down := 9;
      if GetTickCount - timer10sec > 10 * 1000 then begin   //10ÃÊ¿¡ ÇÑ ¹ø
         timer10sec := GetTickCount;
   ...
   down := 91;
         if GetTickCount - timer1min > 60 * 1000 then begin   //1ºÐ¿¡ ÇÑ ¹ø
            timer1min := GetTickCount;
            Inc(gaCount);
...
//         exit;
      ...
      case pmsg.Ident of
         ...
         CM_SITDOWN:
            begin
               hum.SendMsg (hum, pmsg.Ident, pmsg.Tag, LoWord(pmsg.Recog), HiWord(pmsg.Recog), pmsg.Series, body);
               // ¼­¹ö ÀÌµ¿ Áß°£¿¡´Â ¼ö ÃÊ µ¿¾È ÆÐÅ¶ °Ë»ç º¸·ù
               if GetTickCount - hum.ServerShiftTime > 5000 then begin
                  if pmsg.Etc2 <> ((LOWORD(Integer(hum)) and $EC) or $28) xor $A9 then begin
                     {$IFDEF KOREA}
                     hum.SysMsg ('¼ÇÂ¼ÎªºÚ¿Í³ÌÐòµÄÓÃ»§', 0);
                     {$ELSE}
                  ...
                  if BYTE(((pmsg.Recog and $57CD) + (pmsg.Ident or $48) + (pmsg.Param or $30) + (pmsg.Tag and $2D) + pmsg.Series) xor (GetPublicKey xor RandKey)) <> HIBYTE(pmsg.Etc) then begin
                     {$IFDEF KOREA}
                     hum.SysMsg ('¼ÇÂ¼ÎªºÚ¿Í³ÌÐòµÄÓÃ»§(2).', 0);
                     {$ELSE}
         ...
         CM_OPENSTALL:
            begin                               //È±ÉÙwParam
               hum.SendMsg (hum, CM_OPENSTALL, pmsg.Series{0}, 0, 0, 0,body);
            end;
         CM_SAY:
            begin
         ...
         CM_CREATEGROUP,
         CM_CREATEGROUPREQ_OK,   //±×·ì °á¼º È®ÀÎ
         CM_CREATEGROUPREQ_FAIL,   //±×·ì °á¼º È®ÀÎ
         CM_CHAOSHI1_FAIL,
         CM_ADDGROUPMEMBER,
         CM_ADDGROUPMEMBERREQ_OK,   //±×·ì °á¼º È®ÀÎ
         CM_ADDGROUPMEMBERREQ_FAIL,   //±×·ì °á¼º È®ÀÎ
         CM_DELGROUPMEMBER,
         CM_DEALTRY,
         CM_DEALADDITEM,
         CM_DEALDELITEM,
         ...
         CM_MARKET_CLOSE,
         CM_GUILDAGITLIST,
         CM_GUILDAGIT_TAG_ADD, // Àå¿ø ÂÊÁö
         CM_GABOARD_LIST,   //Àå¿ø°Ô½ÃÆÇ¸ñ·Ï
         CM_GABOARD_READ,
         CM_GABOARD_ADD,
         CM_GABOARD_EDIT,
         CM_GABOARD_DEL,
         ...
         CM_FRIEND_ADD      ,// Ä£±¸Ãß°¡
         CM_FRIEND_DELETE   ,// Ä£±¸»èÁ¦
         CM_FRIEND_EDIT     ,// Ä£±¸¼³¸í º¯°æ
         CM_FRIEND_LIST     ,// Ä£±¸ ¸®½ºÆ® ¿äÃ»
         CM_TAG_ADD         ,// ÂÊÁö Ãß°¡
         CM_TAG_DELETE      ,// ÂÊÁö »èÁ¦
         CM_TAG_SETINFO     ,// ÂÊÁö »óÅÂ º¯°æ
         CM_TAG_LIST        ,// ÂÊÁö ¸®½ºÆ® ¿äÃ»
         CM_TAG_NOTREADCOUNT,// ÀÐÁö¾ÊÀº ÂÊÁö °³¼ö ¿äÃ»
         CM_TAG_REJECT_LIST ,// °ÅºÎÀÚ ¸®½ºÆ®
         CM_TAG_REJECT_ADD  ,// °ÅºÎÀÚ Ãß°¡
         CM_TAG_REJECT_DELETE// °ÅºÎÀÚ »èÁ¦
   ...
   try
      ...
      if ps <> nil then begin
         ...
         if CopyToUserItemFromName(ps.Name, newpu^) then begin
            ...
            if ItemName = NAME_OF_GOLD{'±ÝÀü'} then begin
               pmi.Name := NAME_OF_GOLD{'±ÝÀü'};
               pmi.Count := Amount;
            ...
            if pr = pmi then begin
               ...
               // ¼­¹ö·Î±×
               MainOutMessage('[»ðÁúÉñÏîÄ¿ÎïÆ·] ' + pmi.Name + '(' + IntToStr(dx) + ',' + IntToStr(dy) +')');
            end else begin
               //½ÇÆÐÀÎ°æ¿ì
               Dispose (pmi);
            end;
...
procedure TUserEngine.SendBroadCastMsg(sMsg: string; MsgType: Integer);
var
  i                         : Integer;
  PlayObject                : TUserHuman;
begin
  for i := 0 to RunUserList.Count - 1 do begin
    PlayObject := TUserHuman(RunUserList.Objects[i]);
    if not PlayObject.BoGhost and not PlayObject.Death then begin
      PlayObject.SysMsg(sMsg, MsgType);
    end;
  end;
end;
function TUserEngine.GetPlayObjectExOfAutoGetExp(sAccount: string): TUserHuman; //»ñÈ¡ÀëÏß¹ÒÈËÎï
var
  i: Integer;
  PlayObject: TUserHuman;
begin
  Result := nil;
    for i := 0 to RunUserList.Count - 1 do begin
      ...
      if PlayObject <> nil then begin
        if (CompareText(PlayObject.UserId, sAccount) = 0) and PlayObject.m_boNotOnlineAddExp then begin
          Result := PlayObject;
          break;
        end;
      end;
...
function  TUserEngine.GetMapHumCount(sMapStr: string): integer;
var
  i: Integer;
  hum: TUserHuman;
...
begin
  ...
  if sMapStr <> '' then
  begin
    Envirnoment := GrobalEnvir.GetEnvir(sMapStr);
    if Envirnoment <> nil then
    begin
      for i:=0 to RunUserList.Count-1 do
      begin
        hum := TUserHuman(RunUserList.Objects[i]);
        if (not hum.BoGhost) and (hum.PEnvir = Envirnoment) then
        begin
          Inc (Result);
        end;
      end;
