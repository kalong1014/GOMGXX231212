...
uses
  ...
  HUtil32, Grobal2, Envir, EdCode, M2Share, SQLEngn;
...
type
   ...
   public
      ...
      function  GetMemgerFull: Boolean;
   end;
...
begin
   ...
   timeunit := 60 * 60 * 1000; //濫轢珂쇌 데貫1000角봉취
{$IFDEF DEBUG}
   timeunit := 60 * 1000; //DEBUG친駕苟濫轢珂쇌 데貫봉취
{$ENDIF}
{
         ...
         for i:=0 to KillGuilds.Count-1 do begin
            if PTGuildWarInfo(KillGuilds.Objects[i]).WarGuild = aguild then begin
               ...
               if (pgw.WarStartTime + pgw.WarRemain - currenttime) div timeunit <= 1 then begin
                  ...
                  GuildMsg ('콱돨契삔宅' + aguild.GuildName + '돨契삔濫轢綠儺낀逞 '+IntToStr(timevalue)+'鬼珂');
                  backresult := 2;  //연장
                  //서버 로그
//                  MainOutMessage('[문파전 연장]' + GuildName + '<->' + aguild.GuildName + '과(와)의 문파전이 3시간 연장되었습니다.(남은 시간 : 약 ' + IntToStr(pgw.WarRemain div timeunit) + '시간)');
               end else begin
...
function TGuild.GetMemgerFull : Boolean;
var
   i: integer;
begin
   Result := False;
   if MemberList.Count >= MAXGUILDMEMBER then begin
      Result := True;
   end;
end;
...
begin
   if ServerIndex = 0 then begin  //마스터 서버만 저장을 한다.
      ...
      //항목 설명
      strlist.Add (';鏤蹈긍뵀|쳔탰츰|쳔寮츰|理줏휑|假岱莖鑒|얕騰운|饋簡櫓|饋簡쏜띨|뭔찜儺퍅퍅掘|뭔찜쳔탰츰|뭔찜쳔寮츰|뭔찜폡獨휑');
...
{$IFNDEF DEBUG}   //sonmg
         RemainDay := FloatToStr( RemainDateTime ) + '莖';
{$ELSE}
         RemainDay := FloatToStr( RemainDateTime ) + '롸爐';
{$ENDIF}
      end;
      ...
      if ps <> nil then begin
         ...
         if UserEngine.CopyToUserItemFromName(ps.Name, newpu^) then begin
            ...
            if pr = pmi then begin
               ...
               // 서버로그
               MainOutMessage( '[陋干膠] ' + pmi.Name + '(' + IntToStr(dx) + ',' + IntToStr(dy) + ')' );
            end else begin
               //실패인경우
               Dispose (pmi);
            end;
