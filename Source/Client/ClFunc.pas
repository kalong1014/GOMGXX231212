...
uses
  ...
  Grobal2, ScktComp, ExtCtrls, HUtil32, EdCode;
const
   DR_0      = 0;
   DR_1      = 1;
   DR_2      = 2;
...
function  fmStrEX (str: string; len: integer): string;
...
//   if (cu.S.StdMode = 25) then begin //µ¶°¡·ç, ºÎÀû
//      for i:=4 to 5 do
...
//   for i:=6 to MAXBAGITEMCL-1 do begin
   for i:=MAXBAGITEMCL-1 Downto 6 do begin
      if (ItemArr[i].S.StdMode = cu.S.StdMode) and (ItemArr[i].S.Shape = cu.S.Shape) and
         (ItemArr[i].S.Name <> '') and (ItemArr[i].S.Looks = cu.S.Looks) and
         (ItemArr[i].S.Price = cu.S.Price) then begin
         Result := i;
         Exit;
      end;
   end;
...
begin
   ...
   if MsgNum = 1 then DScreen.AddSysMsg (iname + '±»·¢ÏÖ.');
...
begin
   ...
   case smode of //StdMode
      ...
      24, 26:
         Result := U_ARMRINGR;
      // 2003/03/15 ¾ÆÀÌÅÛ ÀÎº¥Åä¸® È®Àå 25->26
//    26:
//       Result := U_ARMRINGL;
      30:
         Result := U_RIGHTHAND;
      // 2003/03/15 ¾ÆÀÌÅÛ ÀÎº¥Åä¸® È®Àå   // ËÄ¸ñ´úÂë
      25:
         Result := U_BUJUK;//»¤Éí·û
      54:
         Result := U_BELT;//Ñü´ø
