...
const
   ...
   bmp_die              = 'wav\log-in-long2.wav';
   bmp_levelup          = 'wav\levelup.wav';
   ...
   s_killer_man_struck     = 164;
   s_killer_wom_struck     = 165;
   s_killer_man_die        = 166;
   s_killer_wom_die        = 167;
   s_killer_man_att3       = 168;
   s_killer_wom_att3       = 169;
   ...
   s_ride_walk_r = 177;
   s_ride_run = 178;
   s_tiger_struck_1 = 179;
   s_tiger_struck_2 = 180;
   s_tiger_attack_1 = 181;
   s_tiger_attack_2 = 182;
   s_tiger_attack_3 = 183;
   ...
   s_baldo_man_hit   = 10625;
   s_baldo_wom_hit   = 10626;
var
  g_SndMgr                  : TSoundMgr;
...
//  LoadSoundList('.\wav\mirsound.lst');
...
begin
  if (X <> -1) and (MySelf <> nil) then begin
    Mon.X := X;
    Mon.Y := Y;
    Chr.X := MySelf.XX;
    Chr.Y := MySelf.YY;
...
procedure TSoundMgr.ItemClickSound(Std: TStdItem);
begin
   case Std.StdMode of
      ...
      24, 26: begin
            if (pos ('ÊÖÌ×', std.Name) > 0) or (pos ('¼×ÆÓ', std.Name) > 0) then
...
begin
  g_SndMgr.PlaySound(idx);
end;

procedure PlaySound (wavname: string);
begin
  g_SndMgr.PlaySound(wavname);
end;

procedure PlayBGM (wavname: string);
begin
  g_SndMgr.PlayBKGSound(wavname);
end;
...
procedure ItemClickSound (std: TStdItem);
begin
  g_SndMgr.ItemClickSound(std);
end;

procedure ItemUseSound (stdmode: integer);
begin
 g_SndMgr.ItemUseSound (stdmode);
end;
