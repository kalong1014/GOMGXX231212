...
label
  labFailExit;
begin
//  g_pLogMgr.Add(Format('数据包: (%s),数据包长度(%d)',[PChar(addr), Len]));
...
//          end;
  ...
  if m_fHandleLogin = 0 then begin
    ArrestStringEx(string(PChar(addr)), '#', '!', sData);
...
//          nEnCodeLen := EncodeBuf(Integer(nABuf), nDeCodeLen, Integer(nBBuf));
        ...
        CM_SELECTSERVER: begin
          m_dwClientTimeOutTick := GetTickCount;
          pszBuf[0] := '%';
          StrFmt(@pszBuf[1], 'A%d/#%d%s!$', [m_pUserOBJ._SendObj.Socket,nPacketIdx, sDefMsg + sDataMsg]);
          m_tLastGameSvr.SendBuffer(@pszBuf[0], StrLen(pszBuf));
        end;
        CM_ADDNEWUSER: begin
          m_dwClientTimeOutTick := GetTickCount;
...
//  g_pLogMgr.Add(Format('服务端数据包:%s 长度: %d', [PChar(SendAddr),Len]));
