...
type
  ...
  private
    ...
    FBlockSize: Integer;
    FBlockCount: Integer;
    FInUseBlock: Integer;    //连接数
    FMaxInUseBlock: Integer; //最大连接数
...
begin
  ...
  BaseMemory.MemBuffer := PAnsiChar(Integer(BaseMemory) + SizeOf(TXMemItem));

  PTRMemory := PAnsiChar(BaseMemory);
  ...
  for i := 0 to BaseBlockCount - 3 do begin
    ...
    PXMemItem(PTRMemory).MemBuffer := PAnsiChar(Integer(PTRMemory) + SizeOf(TXMemItem));
