...
uses
  ...
  AsphyreFactory, AbstractTextures, ZLib, IOUtils, uEDCode, uGameEngine, Types,
  pngimage, Dialogs, uMiniResFileInfo;
...
type
  ...
  protected
    ...
    function GetImageSurface(index: integer): TAsphyreLockableTexture;
  ...
  public
    ...
    function GetCachedImage(index: integer; var m_wPx, m_wPy: integer): TAsphyreLockableTexture;
    ...
    property Images[index: integer]: TAsphyreLockableTexture read GetImageSurface;
...
const
  WMImageInfoSize: Integer = 12;//TWMImageInfo;
...
function CreateImageFile(const AFileName: String; BitCount: Integer): TWMImages;
...
var
   ClientFileHash : TDictionary<string,Boolean>;
...
//
//  if FileExists(MIRPath + S) then
//  begin
//    ANewFileName := MIRPath + S;
//    Result := True;
//    Exit;
//  end;
//  if FileExists(MIRPath + 'Data\' + S) then
//  begin
//    ANewFileName := MIRPath + 'Data\' + S;
//    Result := True;
//    Exit;
//  end;
//  if FileExists(MIRPath + ResDir + S) then
//  begin
//    ANewFileName := MIRPath + ResDir + S;
//    Result := True;
//    Exit;
//  end;
//  if FileExists(MIRPath + ResDir + 'Data\' + S) then
//  begin
...
begin
  ...
  if SameText(AExt, '.WIL') or SameText(AExt, '.WIS') then
    Result  :=  TWILImages.Create
...
begin
  ...
  begin
    m_FileStream.Seek(position, 0);
    if btVersion <> 0 then
      m_FileStream.Read(AImgInfo, SizeOf(TWMImageInfo) - 4)
    else
      m_FileStream.Read(AImgInfo, SizeOf(TWMImageInfo));
...
function TWMImages.GetCachedImage(index: integer; var m_wPx, m_wPy: integer): TAsphyreLockableTexture;
var
  position: integer;
...
function TWMImages.GetImageSurface(index: integer): TAsphyreLockableTexture;
var
  nX,nY:Integer;
begin
