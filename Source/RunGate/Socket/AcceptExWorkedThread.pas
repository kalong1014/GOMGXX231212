...
type
  ...
  _tagUserObj = record
    ...
    nIPLen: Integer;
    nIPAddr: Integer;
    pszIPAddr: array[0..24 - 1] of AnsiChar;
    Port: Integer;
  end;
  ...
  public
    ...
    function AddUser(const SendObj: PIOCPCommObj; const CIP: PAnsiChar; const NIP: Integer; const CPort: Integer): PUserOBJ;
  ...
  TOnReadEvent = procedure(UserData: TObject; const Buffer: PAnsiChar; var BufLen: DWORD; var Succeed: BOOL) of object;
  ...
  public
    ...
    function SendData(const WriterOBJ: PIOCPCommObj; const Buffer: PAnsiChar; const size: UINT): Boolean;
  ...
  private
    ...
    FABuffer: PAnsiChar;
    FBBuffer: PAnsiChar;
    FThread_A_Buffer: array[0..TEMP_ENCODE_LEN + 127] of AnsiChar; //进行CPU缓寸间隔
...
//    szRemoteIP[StrLen(FClientIP)] := #0;
    ...
    if Filter.IsBlockIP(pRemoteSockaddr.sin_addr.S_addr) then begin
      ...
      FClientIP := inet_ntoa(pRemoteSockaddr.sin_addr);
      StrFmt(@szRemoteIP[0], AnsiString('%s'), [FClientIP]);
...
{$IFDEF SHDEBUG}
  SendMessage(hDebug, LB_ADDSTRING, 0, Integer(PAnsiChar(AnsiString(PTRMsgError))));
{$ENDIF}
end;
...
procedure TIOCPWriter.PostAllUserSend(const Buffer: PAnsiChar; const BufLen: DWORD);
var
...
function TIOCPWriter.SendData(const WriterOBJ: PIOCPCommObj; const Buffer: PAnsiChar; const size: UINT): Boolean;
var
  iRc               : Integer;
begin
...
function TUserManager.AddUser(const SendObj: PIOCPCommObj; const CIP: PAnsiChar; const NIP: Integer; const CPort: Integer): PUserOBJ;
var
  dwUserID          : DWORD;
begin
