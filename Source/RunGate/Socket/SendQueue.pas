...
type
  ...
  public
    ...
    function AddBuffer(ComOBJ: PIOCPCommObj; const pInBuffer: PAnsiChar; const nSize: Integer): Integer;
    function AddBuffer2(ComOBJ: PIOCPCommObj; const pInBuffer: PAnsiChar; const nSize: Integer): Boolean;
...
function TSendQueue.AddBuffer(ComOBJ: PIOCPCommObj; const pInBuffer: PAnsiChar; const nSize: Integer): Integer;
var
  PTRSendObj                : PSendQueueNode;
  iSendLen                  : Integer;
    ...
    try
      ...
      if (nSize > 0) and (nSize <= SEND_BUFFER_LEN) then begin
        ...
        if BufLPos = 0 then begin
          if RightLen >= Size then begin
            ...
            DPBuffer := PAnsiChar(Integer(BPBuffer) + Size);
            BufLPos := Size;
            AddBufferOK := True;
          end else begin
            //缓冲区满，暂时加到动态列表
      ...
      if not InSend then begin
        ...
        if iSendLen > 0 then begin
          ...
          CommObj.WBuf.Buf := PAnsiChar(Integer(BPBuffer) + LeftLen);
...
function TSendQueue.AddBuffer2(ComOBJ: PIOCPCommObj; const pInBuffer: PAnsiChar; const nSize: Integer): Boolean;
var
  PTRSendObj                : PSendQueueNode;
  iSendLen                  : Integer;
  QuerySend                 : Boolean;
      ...
      if (nSize > 0) and (nSize <= SEND_BUFFER_LEN) then begin
        if BufLPos = 0 then begin
          if RightLen >= nSize then begin
            ...
            DPBuffer := PAnsiChar(Integer(BPBuffer) + nSize);
            BufLPos := nSize;
          end;
        end else begin
...
begin
  ...
  for i := 0 to FSendPool.RealBlockCount - 1 do begin
    ...
    PTRSendObj.DynSendList.Free; // 释放列表对象和其中的元素
    DeleteCriticalSection(PTRSendObj.QueueLock); // 释放临界区内存
    Inc(PTRMemId);
  end;
...
begin
      ...
      if InSend then begin
        ...
        if iSendLen > 0 then begin
          ...
          if BufLPos > 0 then begin
            ...
            CommObj.WBuf.Buf := BPBuffer;
            SPBuffer := BPBuffer;
            DPBuffer := PAnsiChar(Integer(BPBuffer) + BufRPos);
            BufLPos := 0;
            bSendData := True;
          end else begin
            Result := True;
  ...
  if bSendData then begin
    Result := PostIOCPSend(ComOBJ);
  end else if bAddData then begin
    //这时候缓冲区为空
