...
//  PByte(Addr + Len)^ := 0;
  ...
  g_pLogMgr.Add('m_fHandleLogin: ' + IntToStr(m_fHandleLogin));
...
begin
  ...
  if sCheckData <> '' then begin
    ...
    iSendLen := Length(AnsiString(sCheckData)) + 1; //发送长度
    GetMem(Buffer, Length(AnsiString(sCheckData)) + 1);
    Move(PAnsiChar(AnsiString(sCheckData))^, Buffer^, Length(AnsiString(sCheckData)) + 1);
