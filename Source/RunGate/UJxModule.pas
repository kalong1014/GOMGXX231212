...
uses
  ...
  Dialogs, Contnrs, StdCtrls, IdHashSHA, IdHashCRC, DCPcrypt, Mars, EDCode;
...
type
  PFNENDECODE = function(szbuf: PAnsiChar; len: Integer; szOut: PAnsiChar): Integer; stdcall;
  PFNGETFUNC = function(index: Integer): Integer; stdcall;
  PFNGETVERSION = function(): Integer; stdcall;
  PFNGETSTRCHECK = function(szbuf:PAnsiChar;len:Integer): Integer; stdcall;
...
begin
  ...
  begin
    ...
    dllh := LoadLibraryA(PAnsiChar(AnsiString(dllname)));
    ...
    begin
      ...
      begin
        if (cardinal(thunksread^) and $80000000 <> 0) then
          thunkswrite^ := GetProcAddress(dllh, PAnsiChar(cardinal(thunksread^) and $FFFF)) else
          thunkswrite^ := GetProcAddress(dllh, PAnsiChar(integer(dllbasep) + integer(thunksread^) + 2));
...
var rlen: Integer;
  packet: PJX_PACKET;
  encryptbuf: array[0..1024 * 4 - 1] of AnsiChar;
  ptmp: PAnsiChar;
  start: Integer;
begin
  ...
  ptmp := PAnsiChar(@encryptbuf[0]);
  CopyMemory(PAnsiChar(ptmp) + sizeof(TJX_PACKET), PAnsiChar(mClient.Memory) + start, rlen);
  ...
  packet.curLen := rlen;
//  packet.Crc := CalcBufferCRC(PAnsiChar(Integer(mClient.Memory) + start), rlen);

  rlen := rlen + SizeOf(TJX_PACKET);

  //数据都得加密
  Result := EncodeBuffer(Integer(@encryptbuf[0]), rlen, Integer(p));
...
var
  ...
  MyHash: TIdHashSHA1; // 或 TIdHashSHA256
begin
  MemSteam := TMemoryStream.Create;
  MyHash := TIdHashSHA1.Create; // 或 TIdHashSHA256.Create
  try
    MemSteam.LoadFromFile(iFileName);
    Result := MyHash.HashStreamAsHex(MemSteam);
  finally
    MyHash.Free;
    MemSteam.Free;
  end;
