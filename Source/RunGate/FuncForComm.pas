...
uses
  ...
  AbusiveFilter, ConfigManager, Grobal2, HUtil32, Misc, LogManager, Filter{, UJxModule};
...
var
  ...
  PSI               : PServerInfo;
  pszBuf            : array[0..255] of AnsiChar;
begin
  if not g_fServiceStarted then
    Exit;
  ...
  for i := 0 to IOCPAccepter.ServerCount - 1 do begin
    if IOCPAccepter.Active then begin
      ...
      if PSI^.pClient.m_dwSendBytes > (1024 * 1000) then
        StrFmt(@pszBuf[0], AnsiString('%fM'), [PSI^.pClient.m_dwSendBytes / (1024 * 1000)])
      else if PSI^.pClient.m_dwSendBytes > 1024 then
        StrFmt(@pszBuf[0], AnsiString('%fK'), [PSI^.pClient.m_dwSendBytes / 1024])
      else
        StrFmt(@pszBuf[0], AnsiString('%dB'), [PSI^.pClient.m_dwSendBytes]);
      PSI^.pClient.m_dwSendBytes := 0;
      FormMain.GridSocketInfo.Cells[3, nRow] := pszBuf;

      if PSI^.pClient.m_dwRecvBytes > (1024 * 1000) then
        StrFmt(@pszBuf[0], AnsiString('%fM'), [PSI^.pClient.m_dwRecvBytes / (1024 * 1000)])
      else if PSI^.pClient.m_dwRecvBytes > 1024 then
        StrFmt(@pszBuf[0], AnsiString('%fK'), [PSI^.pClient.m_dwRecvBytes / 1024])
      else
        StrFmt(@pszBuf[0], AnsiString('%dB'), [PSI^.pClient.m_dwRecvBytes]);
...
procedure BroadCastUserItemSpeed();  //2018.4.21 暂时不会解决
//var
//  i, nLen           : Integer;
//  tCmd              : TCmdPack;
//  pszSendBuf        : array[0..48 - 1] of AnsiChar;
...
procedure BroadCastClientActionSpeed();  //2018.4.21 暂时不会解决
//var
//  i, nLen           : Integer;
//  tCmd              : TCmdPack;
//  pszSendBuf        : array[0..48 - 1] of AnsiChar;
