...
type
  TfrmPacketRule = class(TForm)
    ...
    cbCheckEnableIDService: TCheckBox;
    cbCheckNoThisSqlServer: TCheckBox;
    GroupBox3: TGroupBox;
    Label1: TLabel;
    Label2: TLabel;
    Label3: TLabel;
    ...
    EditSqlAccount: TEdit;
    EditSqlPassword: TEdit;
    Label6: TLabel;
    ListBoxBlockIDList: TListBox;
    cbCheckRegisterUser: TCheckBox;
    cbCheckModifyPassword: TCheckBox;
    BlockIDListPopupMenu: TPopupMenu;
    IPOPMENU_REFLIST: TMenuItem;
    IPOPMENU_SORT: TMenuItem;
    MenuItem3: TMenuItem;
    IPOPMENU_ADD: TMenuItem;
    IPOPMENU_DELETE: TMenuItem;
    IPOPMENU_DELETE_ALL: TMenuItem;
    ...
    procedure EditSqlServerChange(Sender: TObject);
    procedure IPOPMENU_REFLISTClick(Sender: TObject);
    procedure IPOPMENU_SORTClick(Sender: TObject);
    procedure IPOPMENU_ADDClick(Sender: TObject);
    procedure IPOPMENU_DELETEClick(Sender: TObject);
    procedure IPOPMENU_DELETE_ALLClick(Sender: TObject);
  public
    m_ShowOpen: Boolean;
  end;
...
uses
  ...
  AppMain, Protocol, FuncForComm, IDFilter,
...
begin
  with g_pConfig, (Sender as TCheckBox) do begin
    case Tag of
      ...
      130: InterlockedExchange(Integer(m_fEnableIDService), Integer(Checked));
      131: InterlockedExchange(Integer(m_fNoThisSqlServer), Integer(Checked));
      132: InterlockedExchange(Integer(m_fRegisterUser), Integer(Checked));
      133: InterlockedExchange(Integer(m_fModifyPassword), Integer(Checked));
    end;
  end;
...
procedure TfrmPacketRule.EditSqlServerChange(Sender: TObject);
begin
  if not m_ShowOpen then
    Exit;
  with g_pConfig, (Sender as TEdit) do begin
    case Tag of
      0: m_szSqlServer := Text;
      1: m_szSqlDatabase := Text;
      2: m_szSqlAccount := Text;
      3: m_szSqlPassword := Text;
    end;
  end;
  btnSave.Enabled := True;
end;
...
begin
  ...
  dwIPLow := Misc.ReverseIP(inet_addr(PAnsiChar(AnsiString(szIPLow))));
  dwIPHigh := Misc.ReverseIP(inet_addr(PAnsiChar(AnsiString(szIPHigh))));
...
procedure TfrmPacketRule.IPOPMENU_ADDClick(Sender: TObject);
var
  szID          : string;
begin
  szID := '';
  if not InputQuery('注册ID过滤', '请输入一个新的ID: ', szID) then
    Exit;
  if szID <> '' then begin
    ListBoxBlockIDList.Items.Add(szID);
    g_BlockIDList.Add(szID);
  end else
    Application.MessageBox('输入ID错误！', '错误信息', MB_OK + MB_ICONERROR);
end;

procedure TfrmPacketRule.IPOPMENU_DELETEClick(Sender: TObject);
var
  szID          : string;
  i             : Integer;
begin
  if (ListBoxBlockIDList.ItemIndex >= 0) and (ListBoxBlockIDList.ItemIndex < ListBoxBlockIDList.Items.Count) then begin
    szID := ListBoxBlockIDList.Items.Strings[ListBoxBlockIDList.ItemIndex];
    if (ListBoxBlockIDList.ItemIndex < g_BlockIDList.Count) and (g_BlockIDList[ListBoxBlockIDList.ItemIndex] = szID) then begin
      g_BlockIDList.Delete(ListBoxBlockIDList.ItemIndex);
      ListBoxBlockIDList.Items.Delete(ListBoxBlockIDList.ItemIndex);
    end;
    for i := 0 to g_BlockIDList.Count - 1 do begin
      if g_BlockIDList.Strings[i] = szID then begin
        g_BlockIDList.Delete(i);
        ListBoxBlockIDList.Items.Delete(ListBoxBlockIDList.ItemIndex);
        Break;
      end;
    end;
...
procedure TfrmPacketRule.IPOPMENU_DELETE_ALLClick(Sender: TObject);
begin
  ListBoxBlockIDList.Items.Clear;
  g_BlockIDList.Clear;
end;

procedure TfrmPacketRule.IPOPMENU_REFLISTClick(Sender: TObject);
var
  i                 : Integer;
begin
  ListBoxBlockIDList.Clear;
  for i := 0 to g_BlockIDList.Count - 1 do
    ListBoxBlockIDList.Items.AddObject(g_BlockIDList[i], g_BlockIDList.Objects[i]);
end;

procedure TfrmPacketRule.IPOPMENU_SORTClick(Sender: TObject);
begin
  ListBoxBlockIDList.Sorted := True;
end;
...
begin
  if (ListBoxActiveList.ItemIndex >= 0) and (ListBoxActiveList.ItemIndex < ListBoxActiveList.Items.Count) then begin
    ...
    if Application.MessageBox(PChar('是否确认将此IP: ' + szIPaddr + ' 加入动态过滤列表中？'#13#10'加入过滤列表后，此IP建立的所有连接将被强行中断。'), PChar('确认信息'),
      MB_OKCANCEL + MB_ICONQUESTION) <> IDOK then
      Exit;
    nIPaddr := inet_addr(PAnsiChar(AnsiString(szIPaddr)));
