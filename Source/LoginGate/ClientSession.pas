...
//  Move(Cmd, TempBuf[1], SizeOf(TDefaultMessage)); //把Cmd的内容，复制到TempBuf中
  ...
  if sMsg <> '' then
    sSendMsg := '#' + EncodeMessage(Cmd) + string(PAnsiChar(AnsiString(EncodeString(sMsg)))) + '!'
...
//  szSenfBuf := '%' + Format('L%d/%s/%s$', [m_pUserOBJ._SendObj.Socket, m_pUserOBJ.pszIPAddr, m_pUserOBJ.pszLocalIPAddr]);
  ...
  m_tLastGameSvr.SendBuffer(PAnsiChar(AnsiString(szSenfBuf)), Length(AnsiString(szSenfBuf)));
end;
