...
uses
  AppMain, IOCPManager, AcceptExWorkedThread, SHSocket, EDcode,
  ConfigManager, Grobal2, HUtil32, Misc, LogManager, IPAddrFilter, IDFilter;
...
var
  ...
  PSI                       : PServerInfo;
  pszBuf                    : array[0..512 - 1] of AnsiChar;
begin
  if not g_fServiceStarted then
    Exit;
  ...
  for i := 0 to IOCPAccepter.ServerCount - 1 do begin
    if IOCPAccepter.Active then begin
      ...
      if PSI^.pClient.m_dwRecvBytes > (1024 * 1000) then
        StrFmt(@pszBuf[nLen + 2], AnsiString('↓%fM'), [PSI^.pClient.m_dwRecvBytes / (1024 * 1000)])
      else if PSI^.pClient.m_dwRecvBytes > 1024 then
        StrFmt(@pszBuf[nLen + 2], AnsiString('↓%fK'), [PSI^.pClient.m_dwRecvBytes / 1024])
      else
        StrFmt(@pszBuf[nLen + 2], AnsiString('↓%dB'), [PSI^.pClient.m_dwRecvBytes]);
