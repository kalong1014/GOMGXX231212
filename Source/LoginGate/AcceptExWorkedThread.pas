...
type
  ...
  _tagLoginObj = record
    ...
    pszBuffer: array [0 .. LOGIN_BUFFER_LEN + 63] of AnsiChar;
  end;
  ...
  _tagRecvObj = record
    ...
    tUserData: TObject;
    dwRecvPos: DWORD;
    pszBuffer: array [0 .. RECV_BUFFER_LEN + 63] of AnsiChar;
  end;
  ...
  _tagUserObj = record
    ...
    pszIPAddr: array [0 .. 32 - 1] of AnsiChar;
    nIPAddr: Integer;
    Port: Integer;

    pszLocalIPAddr: array [0 .. 32 - 1] of AnsiChar;
  ...
  TOnUserEnter = procedure(Sender: TUserManager; const UserOBJ: PUserOBJ)
    of object;
  TOnUserLeave = procedure(Sender: TUserManager; const UserOBJ: PUserOBJ)
    of object;
  ...
  public
    ...
    function AddUser(const SendObj: PIOCPCommObj; const CIP: PAnsiChar;
      const CNIP: Integer; const CPort: Integer; const LIP: PAnsiChar;
      const LNIP: Integer; const LPort: Integer): PUserOBJ;
  ...
  public
    ...
    property OnInitCompPortFinished: TOnCompPortEvent
      read FOnInitCompPortFinished write FOnInitCompPortFinished;
    ...
    procedure FreeAcceptEx(const IOCPData: PIOCPCommObj;
      const ListenSocket: TSocket);
    // 轮询所有的SOCKET是否有效
  ...
  TOnReadEvent = procedure(UserData: TObject; const Buffer: PAnsiChar;
    var BufLen: DWORD; var Succeed: BOOL) of object;
  ...
  public
    ...
    function HandleReadData(const IOCPData: PIOCPCommObj;
      const IOLen: DWORD): BOOL;
    constructor Create(MaxUser: Integer; SendSocket: TIOCPWriter;
      UserManager: TUserManager);
    destructor Destroy; override;
  end;

  TOnBroadCastEvent = procedure(UserOBJ: PUserOBJ; wParam, lParam: DWORD)
    of object;
  ...
  protected
    ...
    procedure DeleteLoginUser(IOCPData: PIOCPCommObj);
    function AddLoginUser(IOCPData: PIOCPCommObj;
      var WriterOBJ: PIOCPCommObj): Boolean;
  public
    property SendQueue: TSendQueue read FSendQueue;
    property CurrentUser: Integer read GetCurrentUser;
    property OnBroadcastEvent: TOnBroadCastEvent read FOnBroadCastEvent
      write FOnBroadCastEvent;
    procedure PostAllUserSend(const Buffer: PAnsiChar; const BufLen: DWORD);
    procedure BroadUserMsg(wParam, lParam: Integer;
      OnBroadcastEvent: TOnBroadCastEvent);
    function SendData(const WriterOBJ: PIOCPCommObj; const Buffer: PAnsiChar;
      const size: UINT): Boolean;
  ...
  private
    ...
    FABuffer: PAnsiChar;
    FBBuffer: PAnsiChar;
    FThread_A_Buffer: array [0 .. TEMP_ENCODE_LEN + 127] of AnsiChar;
    // 进行CPU缓寸间隔
    FThread_B_Buffer: array [0 .. TEMP_ENCODE_LEN + 127] of AnsiChar;
    procedure HandleExcept(const IOCPData: PIOCPCommObj; const Str: string);
  protected
    procedure Run(); override;
    function HandleRecvEvent(const IOCPData: PIOCPCommObj;
      const IOLength: DWORD): Boolean;
    function HandleSendEvent(const IOCPData: PIOCPCommObj;
      const IOLength: DWORD): Boolean;
...
constructor TAcceptExWorkedThread.Create(SendSocket: TIOCPWriter;
  RecvSocket: TIOCPReader; AcceptExSocket: TIOCPAccepter; IOCPPort: THANDLE);
begin
...
begin
  case IOCPData.WorkType of
    ...
    SOCKET_READ:
      FSendSocket.FUserManager.DeleteUser(IOCPData.AddObject);
    SOCKET_SEND:
      FSendSocket.FUserManager.DeleteUser(IOCPData.AddObject);
  end;
end;

procedure TAcceptExWorkedThread.HandleExcept(const IOCPData: PIOCPCommObj;
  const Str: string);
var
  PTRUser: PUserOBJ;
  StrExcept: string;
...
begin
  ...
  // sLog.outDetail(StrExcept + #13#10 + Str);
            ...
            SOCKET_ACCEPT:
              begin
                try
                  ...
                  on E: Exception do
                  begin
                    // sLog.outError('HandleAcceptEvent Error: ' + E.Message);
...
begin
  ...
  begin
    PostQueuedCompletionStatus(FCompPort, 0, 0, nil);
  end;
...
procedure TIOCPAccepter.FreeAcceptEx(const IOCPData: PIOCPCommObj;
  const ListenSocket: TSocket);
var
  bClose: Boolean;
begin
...
begin
  ...
  try
    ...
    FGetAcceptExSockaddrsFunc(@PTRLoginObj.pszBuffer, 0, FAddrAddLen,
      FAddrAddLen, pLocalSockaddr, LocalSockaddrLen, pRemoteSockaddr,
      RemoteSockaddrLen);
    ...
    begin
      // sLog.outDetail('Block IP: %s', [szRemoteIP]);
      if g_pLogMgr.CheckLevel(1) then
    ...
    begin
      // sLog.outDetail('Block IP Area: %s', [szRemoteIP]);
      if g_pLogMgr.CheckLevel(1) then
    ...
    begin
      // sLog.outDetail('超过每IP连接[%d]: %s', [g_pConfig.m_nMaxConnectOfIP, szRemoteIP]);
      if g_pLogMgr.CheckLevel(1) then
    ...
    iRc := setsockopt(IOCPData.Socket, SOL_SOCKET, SO_UPDATE_ACCEPT_CONTEXT,
      @ServerOBJ.nSocket, SizeOf(TSocket));
    ...
    iRc := CreateIoCompletionPort(IOCPData.Socket, FCompPort,
      IOCPData.Socket, 0);
    ...
    begin
      UserOBJ := FUserManager.AddUser(WriterOBJ, @szRemoteIP,
        pRemoteSockaddr.sin_addr.S_addr, FClientPort, @szLocalIP,
        pLocalSockaddr.sin_addr.S_addr, FLocalPort);
      ...
      begin
        ...
        // 备注1 咸鱼
{$IFDEF CPU64BITS}
        InterlockedExchange64(int64(Socket), int64(IOCPData.Socket));
{$ELSE}
...
{$ENDIF}
      ...
      begin
        // sLog.outDebug('用户登陆: %s', [szRemoteIP]);
        if g_pLogMgr.CheckLevel(1) then
...
begin
  // sLog.outError(PTRMsgError);
  if g_pLogMgr.CheckLevel(2) then
    g_pLogMgr.Add(PTRMsgError);
{$IFDEF SHDEBUG}
  SendMessage(hDebug, LB_ADDSTRING, 0,
    Integer(PAnsiChar(AnsiString(PTRMsgError))));
{$ENDIF}
end;
...
var
  SI: TSockAddrIn;
  iRc: NativeInt;  //备注1 咸鱼
  Func: NativeInt; //备注1 咸鱼
  dwBytes: DWORD;
  i: NativeInt;    //备注1 咸鱼
  dwIOKey: DWORD;
...
begin
  ...
  iRc := CreateIoCompletionPort(FListenSocket, FCompPort, dwIOKey, 0);
  ...
  iRc := WSAIoctl(FListenSocket, SIO_GET_EXTENSION_FUNCTION_POINTER,
    @WSAID_ACCEPTEX, SizeOf(WSAID_ACCEPTEX), @Func, // @FAcceptExFunc,
    SizeOf(LPFN_ACCEPTEX), @dwBytes, nil, nil);
  ...
  iRc := WSAIoctl(FListenSocket, SIO_GET_EXTENSION_FUNCTION_POINTER,
    @WSAID_GETACCEPTEXSOCKADDRS, SizeOf(WSAID_GETACCEPTEXSOCKADDRS), @Func,
    SizeOf(LPFN_GETACCEPTEXSOCKADDRS), @dwBytes, nil, nil);
  ...
  begin
    ...
    CloseHandle(FCompPort);
    FCompPort := INVALID_HANDLE_VALUE;
    HandleException(Format('得到GetAcceptExSockaddrs地址错误，错误代码为：%d',
      [WSAGetLastError()]));
  end;
...
begin
  ...
  try
    ...
    begin
      ...
      // 得到连接建立时间
      iRc := getsockopt(PTRAcceptObj.tCommObj.Socket, SOL_SOCKET,
        SO_CONNECT_TIME, @dwConTime, IPLen);
      if iRc <> SOCKET_ERROR then
      begin
...
procedure TIOCPAccepter.PostAcceptEx(ListenSocket: TSocket;
  LoginObj: PLoginObj = nil);
var
  NewSocket: TSocket;
...
begin
  ...
  begin
    ...
    // 备注1 咸鱼
    bRc := FAcceptExFunc(ListenSocket, tCommObj.Socket, @pszBuffer, dwBytes,
      FAddrAddLen, FAddrAddLen, dwBytes, @tCommObj.PTRIOKey);
...
function TIOCPReader.AddLoginUser(const IOCPData: PLoginObj;
  var ReaderOBJ: PRecvObj): Boolean;
var
  MemID: UINT;
  PTRUserOBJ: PUserOBJ;
begin
...
begin
  ...
  begin
    // 备注1 咸鱼
{$IFDEF CPU64BITS}
    InterlockedExchange64(int64(Socket), int64(INVALID_SOCKET));
{$ELSE}
...
function TIOCPReader.HandleReadData(const IOCPData: PIOCPCommObj;
  const IOLen: DWORD): BOOL;
var
  IOWorkOut: DWORD;
begin
...
function TIOCPReader.ReadDone(const IOCPData: PIOCPCommObj;
  const IOLen: DWORD): BOOL;
var
  PTRRecvOBJ: PRecvObj;
  iRecvLen: Integer;
...
function TIOCPWriter.AddLoginUser(IOCPData: PIOCPCommObj;
  var WriterOBJ: PIOCPCommObj): Boolean;
begin
...
procedure TIOCPWriter.BroadUserMsg(wParam, lParam: Integer;
  OnBroadcastEvent: TOnBroadCastEvent);
begin
...
procedure TIOCPWriter.PostAllUserSend(const Buffer: PAnsiChar;
  const BufLen: DWORD);
var
...
function TIOCPWriter.SendData(const WriterOBJ: PIOCPCommObj;
  const Buffer: PAnsiChar; const size: UINT): Boolean;
var
  iRc: Integer;
begin
...
function TUserManager.AddUser(const SendObj: PIOCPCommObj; const CIP: PAnsiChar;
  const CNIP: Integer; const CPort: Integer; const LIP: PAnsiChar;
  const LNIP: Integer; const LPort: Integer): PUserOBJ;
var
  dwUserID: DWORD;
...
begin
  ...
  begin
    ...
    try
      ...
      on E: Exception do
      begin
        // sLog.outError('TUserManager::DeleteUser %d: %s', [nCode, E.Message]);
        if g_pLogMgr.CheckLevel(2) then
          g_pLogMgr.Add(Format('TUserManager::DeleteUser %d: %s',
            [nCode, E.Message]));
      end;
    end;
  end;
