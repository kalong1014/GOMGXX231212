...
uses
  ...
  IOCPManager, ClientThread, AcceptExWorkedThread, Protocol, utest;
...
uses
  ...
  IPAddrFilter, GeneralConfig, PacketRuleConfig, IDFilter, EDcode, SHSocket;
...
//  EDHeader, EDHeader2: TEDHeader;
  pszBufPtr: PAnsiChar;
  pszBuffer: array[0..MAX_CLIENT_FUNC_SIZE - 1] of AnsiChar;
...
(*
  if fHandle <> 0 then begin
    try
      ...
      g_DCP_mars.InitStr('RAMM2 LoginGate - edHeader 20160201');
...
{$IFDEF  FREE}
          g_DCP_mars.InitStr(('ram gate free'));
{$ELSE}
          //g_DCP_mars.InitStr(VMProtectDecryptStringA('blue gate'));
          g_DCP_mars.InitStr(('ram gate'));
{$ENDIF}
...
begin
  ...
  if LoadPublicKey(g_pConfig.m_szEncKey) then
    g_pLogMgr.Add('秘钥文件加载成功...');
...
begin
  with frmGeneralConfig, g_pConfig do begin
    ...
    EditEncKeyFile.Text := m_szEncKey;
...
begin
  with frmPacketRule, g_pConfig do begin
    ...
    for I := 0 to g_BlockIDList.Count - 1 do
      ListBoxBlockIDList.Items.AddObject(g_BlockIDList.Strings[I], g_BlockIDList.Objects[I]);
    ...
    cbCheckEnableIDService.Checked := m_fEnableIDService;
    cbCheckRegisterUser.Checked := m_fRegisterUser;
    cbCheckModifyPassword.Checked := m_fModifyPassword;
    cbCheckNoThisSqlServer.Checked := m_fNoThisSqlServer;

    EditSqlServer.Text := m_szSqlServer;
    EditSqlAccount.Text := m_szSqlAccount;
    EditSqlPassword.Text := m_szSqlPassword;
    EditSqlDatabase.Text := m_szSqlDatabase;
...
begin
  ...
  g_pLogMgr.Add(('程序网站: http://www.mir2c.com'));
