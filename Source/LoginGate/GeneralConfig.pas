...
type
  TfrmGeneralConfig = class(TForm)
    ...
    procedure EditEncKeyFileChange(Sender: TObject);
  private
...
procedure TfrmGeneralConfig.EditEncKeyFileChange(Sender: TObject);
begin
  if not m_Showed then
    Exit;
  g_pConfig.m_szEncKey := EditEncKeyFile.Text;
end;
...
begin
  ...
  if inet_addr(PAnsiChar(AnsiString(szServerIPaddr))) <> INADDR_NONE then begin
