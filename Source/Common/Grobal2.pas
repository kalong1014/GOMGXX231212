...
uses
	Windows, SysUtils, Classes, Hutil32, MD5;

type
    TShopItem = packed record
      sItemName: string[20];
      btClass: Byte;
      wLooks: Word; //Items.wil
      wPrice: DWORD;
      wShape1: Word;
      wShape2: Word;
      sExplain: string[127];
   ...
   TMonStatus = (s_KillHuman, s_UnderFire, s_Die, s_MonGen); //rainee
  ...
  pTHornColor = ^THornColor;

   TCharExpInfo = record
      Level: Byte;
      Exp: Longword;
   end;
   PTCharExpInfo = ^TCharExpInfo;
  ...
  TMirConfig = record
    ...
    g_boHelp: boolean;
    g_boHostpot: boolean;
    g_boHostpotWeb: string[30];
    g_boQuestions: boolean;
    g_boQuestionsWeb: string[30];
    g_boChinese: boolean;
   ...
   TStdItem = record
      ...
      Weight       : byte;              // ¹«°Ô
      AniCount     : byte;              // 1º¸´Ù Å©¸é ¾Ö´Ï¸ÞÀÌ¼Ç µÇ´Â ¾ÆÀÌÅÛ (´Ù¸¥ ¿ëµµ·Î ¸¹ÀÌ ¾²ÀÓ)
      SpecialPwr   : shortint;          // +ÀÌ¸é »ý¹°°ø°Ý+´É·Â, -ÀÌ¸é ¾ðµ¥µå°ø°Ý+
                                        //1~10 °­µµ
                                        //-50~-1 ¾ðµ¥µå ´É·ÂÄ¡ Çâ»ó
                                        //-100~-51 ¾ðµ¥µå ´É·ÂÄ¡ °¨¼Ò
      ItemDesc     : byte;              //$01 IDC_UNIDENTIFIED  (¾ÆÀÌ´íÆ¼ÆÄÀÌ ¾È µÈ °Í, Å¬¶óÀÌ¾ðÆ®¿¡¼­¸¸ »ç¿ëµÊ)
      ...
      UniqueItem   : byte;              // À¯´ÏÅ©¼Ó¼º
                                        // À¯´ÏÅ© --- $01:Ï´Á·/Éý¼¶²»³É¹¦
                                        // À¯´ÏÅ© --- $02:²»¿ÉÐÞÀí
                                        // À¯´ÏÅ© --- $04:ÈÓµôÊ±ÏûÊ§(²»»á´Ó´ü×Ó´°¿ÚµôÂä)
                                        // À¯´ÏÅ© --- $08:½»»»ºÍ´æ´¢½»Ò×²»¿ÉÄÜ£¨12 = 4 + 8£º²»¿É½»Ò×£¬²»¿É¶ªÆú£©
                                        //             32:ÎÞ·¨¶ªÆú  40= 32 + 8 £º²»¿É½»Ò×£¬²»¿É³öÊÛ£¬²»¿ÉÒÔ¶ªÆú
      OverlapItem  : byte;              // Áßº¹Çã¿ë
      light        : byte;              // ºûÀ»³»´Â ¾ÆÀÌÅÛ
      {--------------------}
      ItemType     : byte;              // ¾ÆÀÌÅÛÀÇ ±¸ºÐ
      ItemSet      : Word;              // ¼ÂÆ® ¾ÆÀÌÅÛ ±¸ºÐ
   ...
   TAbility = record
      ...
      HandWeight    : byte;
      MaxHandWeight : byte;  //µé ¼ö ÀÖ´Â °ËÀÇ ¹«°Ô (¹«°Ô¸¦ ÃÊ°úÇÏ¸é ±²ÀåÈ÷ ´À¸®°Ô °ø°ÝÇÑ´Ù 2-3¹è´À¸²)
   ...
   TPricesInfo = record            //°¡°Ý Á¤º¸
      Index       : word;  //Ç¥ÁØ ¾ÆÀÌÅÛÀÇ ÀÎµ¦½º
      SellPrice   : integer;    //±âº» °¡°Ý, BuyPrice´Â SellPriceÀÇ Àý¹Ý
   end;
...
//      Hint      : string[40];
   ...
   TClientItem = record      //Å¬¶óÀÌ¾ðÆ®¿¡¼­ ÇÊ¿äÇÑ Æ÷¸ä
   ...
   TUserStateInfo = record
      ...
      UseItems : array[0..13] of TClientItem;    // 8->12
  ...
  TDropItem = record  //Å¬¶óÀÌ¾ðÆ®¿¡¼­ »ç¿ë
    Id: integer;
    X: word;
    Y: word;
    Looks: word;
    FlashTime: longword; //¸¶Áö¸·À¸·Î ¹ÝÂ¦°Å¸° ½Ã°£
    BoFlash: Boolean;
   ...
   TDefMagic = record
      MagicId: word;
      MagicName: string[14];       //Ä­ ´Ã¸±°Í 12->14(Å¬¶óÀÌ¾ðÆ®¿Í ÇÔ²² »ç¿ë)
      EffectType: byte;
      Effect: byte;
      Spell: word;
      MinPower: word;
      ...
      Job: byte;         //0: Àü»ç 1:¼ú»ç  2:µµ»ç   99:¸ðµÎ°¡´É
      DelayTime: integer; //ÇÑ¹æ ½ð´ÙÀ½¿¡ ´ÙÀ½ ¸¶¹ýÀ» ¾µ ¼ö ÀÖ´Âµ¥ °É¸®´Â ½Ã°£
      DefSpell: byte;
      DefMinPower: byte;
      MaxPower: word;
      DefMaxPower: byte;
   ...
   TUserMagic = record
      pDef        : PTDefMagic;  //¹Ýµå½Ã nilÀÌ ¾Æ´Ï¾î¾ß ÇÑ´Ù.
   ...
   TMapItem = record
      ...
      Reserved: byte;
      Count: integer;
      Ownership: TObject; //¹°°ÇÀ» ÁýÀ» ¼ö ÀÖ´Â »ç¶÷
   ...
   {¸Ê ¿¡¼­ ÀÏ¾î³ª´Â ÀÌº¥Æ®, activate½ÃÄÑ¾ß¸¸ ÀÌº¥Æ®°¡ ¹ß»ýÇÑ´Ù.}
   ...
   TMapInfo = record
      ...
      Area     : byte;    //Áö¿ª ±¸ºÐ (¸¶À»,¼ö·ÃÀå,µîµî)
      Reserved : byte;    //¹Ì»ç¿ë
   ...
   TUserCharacterInfo = record          // °¡»ó¼¼°è¿¡ µé¾î¿À±â Àü¿¡ »ç¿ëÀÚ¿¡°Ô Àü´ÞµÇ´Â
   ...
   TMonsterInfo = record
      ...
      RaceImg: byte;  //Å¬¶óÀÌ¾ðÆ® ÇÁ·¡ÀÓ ½Äº°
      Appr: word;   //ÀÌ¹ÌÁö ¹øÈ£
      Level: byte;
      LifeAttrib: byte;
...
//   PTMonItemInfo = ^TMonItemInfo;
  ...
  TMonItemInfo = record
    SelPoint: Integer;
    MaxPoint: Integer;
    MonItemName: string[14];
    boGold: Boolean;
    Count: Integer;
    boItemDropRandom: Boolean;
    List: TList;
  end;
...

{
    ...
    TTempAddAbil = record     //changeHumAbilityÊôÐÔ¼Óµã
     nAddWord: word;
     nAddTick: LongWord;
   end;
...
const
  SM_SendSafeZone = 7000; //°²È«Çø·¶Î§ 2019-02-19
...
{$endif}
   ...
   POISON_DECHEALTH     = 0;   //$80000000 ÂÌ¶¾
   POISON_DAMAGEARMOR   = 1;   //$40000000 ºì¶¾
...
//   STATE_TEMPORARY3     = 15;  //$00010000  //ÀÓ½Ã3(ÇÏÆ®»©»©·Î)
   STATE_CELESTIALUP    = 13;
   STATE_BLOODDRAGONSWORD = 14;
   ...
   IDC_NEVERTAKEOFF     = $04;   //¼Õ¿¡¼­ Àý´ë·Î ¶³¾îÁöÁö ¾ÊÀ½
   IDC_DIEANDBREAK      = $08;   //Á×À¸¸é ±úÁü
   IDC_NEVERLOSE        = $10;   //Á×¾îµµ ÀÒ¾î¹ö¸®Áö ¾ÊÀ½


   STATE_STONE_MODE     = $00000001;  //¼®»ó¸ó½ºÅÍÀÇ ¸ð½À(¼®»óÀ¸·Î ÀÖÀ½)
   STATE_OPENHEATH      = $00000002;  //Ã¼·Â °ø°³»óÅÂ


   HAM_ALL              = 0;  //¸ðµÎ °ø°Ý
   HAM_PEACE            = 1;  //ÆòÈ­¸ðµå, ¸ó½ºÅÍ¸¸ °ø°Ý
   HAM_GROUP            = 2;  //±×·ì¿ø ÀÌ¿Ü ¾Æ¹«³ª °ø°Ý
   HAM_GUILD            = 3; //±æµå¿ø ÀÌ¿Ü ¾Æ¹«³ª °ø°Ý
   HAM_PKATTACK         = 4; //»¡°»ÀÌ ´ë ÈòµÕÀÌ
   HAM_GUILDWAR         = 5; //Àû´ë¹®ÆÄ¸¸ °ø°Ý
   ...
   SM_TEST                 = 1;
   //Èå¸§Á¦¾î ¸í·É
   SM_STOPACTIONS          = 2;  //¸ðµç Ä³¸¯ÅÍ/¸¶¹ýÀÇ µ¿ÀÛÀ» ¸ØÃá´Ù.
                                 //´Ù¸¥ ¸Ê¿¡ µé¾î°£ °æ¿ì,
   ...
   SM_THROW                = 5;
   SM_RUSH                 = 6;  //¾ÕÀ¸·Î ÀüÁø
   SM_RUSHKUNG             = 7;  //¾ÕÀ¸·Î ÀüÁø½ÇÆÐ
   SM_FIREHIT              = 8;  //¿°È­°á
   SM_BACKSTEP             = 9;  //µÞ°ÉÀ½Áú,
   SM_TURN                 = 10;
   SM_WALK                 = 11;
   SM_SITDOWN              = 12;
   SM_RUN                  = 13;
   ...
   SM_FLYAXE               = 22;
   SM_LIGHTING             = 23;  //¸¶¹ý »ç¿ë
   SM_WIDEHIT              = 24;
   SM_ACTION_MAX           = 25;

   // 2003/03/15 ½Å±Ô¹«°ø
   SM_CROSSHIT             = 35;  //±¤Ç³Âü, ÁÖº¯8Å¸ÀÏ °ø°Ý
   SM_TWINHIT              = 36;  //½Ö·æÂü, ºü¸£°Ô 2¹ø °ø°Ý
   SM_STONEHIT             = 37;  //»çÀÚÈÄ, ÁÖº¯8Å¸ÀÏ µ¹·Î¸¸µë
   SM_WINDCUT              = 38;  //°øÆÄ¼¶,  ¾ÕÅ¸ÀÏ 9°³ °ø°Ý
   SM_DRAGONFIRE           = 39; // Ãµ·æ±â¿°(È­·æ±â¿°)  ÀÚ½ÅÁÖº¯ Å¸ÀÏ 25°³ °ø°Ý
   SM_CURSE                = 40; // ÀúÁÖ¼ú
   ...
   SM_LIGHTING_1           = 87; //¸¶¹ý_1:ÀÌ¹«±â ¸êÃµÈ­
   SM_LIGHTING_2           = 88;
   ...
   MAGIC_SIDESTONE_ATT1    = 100; //È£È¥±â¼® Àü±â °ø°Ý
   ...
   SM_ACTION2_MIN          = 1000;
   //SM_READYFIREHIT         = 1000;  //Å¬¶óÀÌ¾ðÆ®¿¡¼­¸¸ ¾²ÀÓ, ¿°È­°á ÁØºñ
   ...
   SM_Server_Config        = 11106;
   SM_SYSSCROLLMESSAGE     = 11107;

   //ITEM ?
   SM_ADDITEM              = 200;  //¾ÆÀÌÅÛÀ» »õ·Î ¾òÀ½  Series(¼ö·®)
   SM_BAGITEMS             = 201;  //°¡¹æÀÇ ¸ðµç ¾ÆÀÌÅÛ
   ...
   SM_PASSWD_FAIL          = 503;
   SM_NEWID_SUCCESS        = 504;  //»õ¾ÆÀÌµð Àß ¸¸µé¾î Á³À½
   SM_NEWID_FAIL           = 505;  //»õ¾ÆÀÌµð ¸¸µé±â ½ÇÆÐ
   ...
   SM_LIXIANGUAJI          = 538;  //ÀëÏß¹Ò»ú×¨ÓÃÐÅÏ¢
   ...
   SM_TAKEON_FAIL          = 616; //Âø¿ë ½ÇÆÐ
   SM_EXCHGTAKEON_OK       = 617; //Âø¿ë¾ÆÀÌÅÛ ±³È¯ ¼º°ø
   SM_EXCHGTAKEON_FAIL     = 618; //Âø¿ë¾ÆÀÌÅÛ ±³È¯ ½ÇÆÐ
   SM_TAKEOFF_OK           = 619; //¹þ±â ¼º°ø, + New Feature
   SM_TAKEOFF_FAIL         = 620; //
   SM_SENDUSEITEMS         = 621; //Âø¿ë ¾ÆÀÌÅÛ ¸ðµÎ º¸³¿
   ...
   SM_EAT_FAIL             = 636;
   SM_BUTCH                = 637;
   SM_MAGICFIRE            = 638; //¸¶¹ý ¹ß»çµÊ  CM_SPELL -> SM_SPELL + SM_MAGICFIRE
   ...
   SM_DONATE_OK            = 765;
   SM_MYSTATUS             = 766;
   SM_MENU_OK              = 767;  //descriptionÀ¸·Î ¸Þ¼¼Áö Àü´Þ
   ...
   SM_TIMECHECK_MSG        = 810;  //Å¬¶óÀÌ¾ðÆ®¿¡¼­ ½Ã°£
   ...
   SM_GABOARD_READ         = 833;  // Àå¿ø°Ô½ÃÆÇ ±ÛÀÐ±â
   ...
   SM_NORMALEFFECTEX       = 850;
   SM_SPACEMOVE_SHOW_MAGIC = 860;
   SM_SPACEMOVE_SHOW_MAGIC2= 861;
   ...
   SM_NEXTTIME_PASSWORD    = 1105;  //´ÙÀ½¹ø¿¡´Â ºñ¹Ð¹øÈ£ ÀÔ·Â ¸ðµåÀÌ´Ù.
   SM_CHECK_CLIENTVALID    = 1106;  //Å¬¶óÀÌ¾ðÆ®ÀÇ ¼öÁ¤ ¿©ºÎ È®ÀÎ
   ...
   SM_MIRCONFIG              = 1500;
   SM_Master              = 1501;
   SM_RufuseMaster         = 1502;  // °ü°è µî·Ï ¿äÃ»

   SM_PING                  = 1600;
   SM_GATA_FAIL            = 538;

   SM_SHELLEXECUTE          = 1601;

   SM_MAA_LIST              = 1602;
   SM_MAA_SONMAE            = 1603;

   SM_SPECOFFERITEM         = 1604;
   SM_OFFERITEM             = 1605;
   SM_BUGITEMFAIL           = 1606;
   SM_PRESENDITEMFAIL       = 1607;
   SM_UPGRADEITEM_RESULTT     = 1608;
   SM_ALIVEREQ              = 1609;
   SM_NOVICEHEIP            =1610;
   ...
   CM_THROW                = 3005;
   CM_TURN                 = 3010;    //CM_TURN - 3000 = SM_TURN ±ÔÄ¢À» ¹Ýµå½Ã ÁöÄÑ¾ß ÇÔ
   CM_WALK                 = 3011;
   CM_SITDOWN              = 3012;
   CM_RUN                  = 3013;
   CM_HIT                  = 3014;
   ...
   CM_TAKEOFFITEM          = 1004;  //º¹ÀåÀ» ¹þ´Â´Ù
   CM_EXCHGTAKEONITEM      = 1005;  //Âø¿ëÇÑ ¾ÆÀÌÅÛÀ» ÁÂ¿ì¸¦ ¹Ù²Û´Ù.(¹ÝÁö,ÆÈÂî)
   CM_EAT                  = 1006;  //¸Ô´Ù, ¸¶½Ã´Ù
   ...
   CM_TAG_NOTREADCOUNT     = 1054;  // ÀÐÁö¾ÊÀº ÂÊÁö °³¼ö ¿äÃ»
   ...
   CM_MARKET_GETPAY        = 1072;  // À§Å¹±ÝÈ¸¼ö NPC -> À¯Àú
   ...
   {-------------°ÚÌ¯---------------}
    CM_OPENSTALL = 1122; //¿Í»§¶Ë·¢ÎïÆ·ÐÅÏ¢
    CM_GAMECLOSE            = 1123;
    CM_DEATH_FALL = 1124;
    CM_RufuseMaster         = 1125;  // °ü°è µî·Ï ¿äÃ»
    ...
    SM_OPENSTALL = 219; //³É¹¦»òÕßÊ§°Ü ¿Í»§¶Ë½ÓÊÕ
    RM_STALLSTATUS = 9001; //°ÚÌ¯³É¹¦ºó²ÅÓÐ
    //-ÏÂÃæÁ¬¸öÎ´È·ÈÏ³åÍ»
    SM_USERSTALL  = 218;
    SM_BUYSTALLITEM =220;
   {-------------°ÚÌ¯---------------}
...
//   RM_HIDE                 = 10023;
   ...
   RM_GMWHISPER            = 10055;    //¿î¿µÀÚ ¸ðµåÀÏ ¶§ ±Ó¸»(2004/11/18)
   ...
   RM_SENDBUYPRICE         = 10130;  //»óÁ¡¿¡¼­ »ç¿ëÀÚÀÇ ¾ÆÀÌÅÛÀ» »ç´Â °¡°Ý
   ...
   RM_DELITEMS             = 10148;  //¾ÆÀÌÅÛ ÀÐ¾î ¹ö¸², Å¬¶óÀÌ¾ðÅ×¿¡ ¾Ë¸².
   ...
   RM_GAMEGOLDCHANGED      = 10160;
   RM_GAMEPOINTCHANGED     = 10161;
   RM_DIGUP                = 10200;
   RM_DIGDOWN              = 10201;
   RM_FLYAXE               = 10202;
   ...
   RM_DELAYATTACK          = 10338;  //Å¸°Ý ½ÃÁ¡À» ¸ÂÃß±â À§ÇØ¼­
   RM_SPACEMOVE_SHOW_NO    = 10339;  //ÀÌÆåÆ® ¾øÀÌ ³ªÅ¸³²
   RM_SPACEMOVE_SHOW_MAGIC = 10340;  //ÈÕÉÁÒÆ¶¯
   RM_SPACEMOVE_SHOW_MAGIC2= 10341;  //ÈÕÉÁÒÆ¶¯
   ...
   RM_BREAKWEAPON          = 10413;  //¹«±â°¡ ±úÁü, ¾Ö¹Ì¸ÞÀÌ¼Ç È¿°ú
   RM_INSTANCEHEALGUAGE    = 10414;
   RM_CHANGEFACE           = 10415;  //º¯½Å...
   RM_NEXTTIME_PASSWORD    = 10416;  //´ÙÀ½ ÇÑ¹øÀº ºñ¹Ð¹øÈ£ÀÔ·Â ¸ðµå
   ...
   RM_LM_DBEDIT2            = 10427;  // ¿¬ÀÎ»çÁ¦ ¸®½ºÆ®¿øÇÔ
   ...
   RM_SYSMSG_USE           = 11026;
   RM_SYSMSG_USE_HORN      = 11027;

   RM_POTCASHCHANGED       = 11060;

   RM_SYSMESSAGECOLOR      = 11061;

   RM_SYSSCROLLMESSAGE     = 11062;

   RM_MAA_DBWANTLIST       = 11063;

   RM_MAA_SONNAME          = 11064;

   RM_MAA_DBWANTLIST2       = 11065;

   RM_ALIVEREQ              = 11066;

   RM_SYSMSG_REMARK2        = 11067;

   RM_SYSMSG_REMARK3        = 11068;
   RM_NOVICEHEIP            = 11069;
   RM_SHOWBLUEHEALTH        = 11070; //»¤ÉíÏÔÀ¶
   {----------------------------}
   ...
   ISM_EVENTCHECK          = 120;
   //PCÔö¼ÓÍ¨Ñ¶
   ISM_RECHARGEINFO        = 121;
   ISM_RECHARGEDAYADD      = 122;
   ISM_RECHARGEHOURSADD    = 123;
   {----------------------------}
   ...
   ISM_RELOADGUILD         = 207;
   ISM_GUILDMSG            = 208;
   ISM_CHATPROHIBITION     = 209;    //Ã¤±Ý
   ISM_CHATPROHIBITIONCANCEL = 210;  //Ã¤±ÝÇØÁ¦
   ...
   ISM_FRIEND_CLOSE        = 217;    // Ä£±¸ ½Ã½ºÅÛ ´Ý±â
...
//   ISM_LM_LEVELINFO        = 225;   // °ü°è ·¹º§Á¤º¸ Àü¼Û
   ...
   ISM_LM_KILLED_MSG       = 237;   //¿¬ÀÎ »ìÇØ ¸Þ½ÃÁö
   ISM_REQUEST_LOVERRECALL = 238;   //¿¬ÀÎ ¼ÒÈ¯ ¿äÃ»

   ISM_GUILDWAR            = 239;   //¹®ÆÄÀü ½ÅÃ»¿¬Àå

   ISM_UPDATALOGINTIME    =  240; //¸üÐÂÉÏÏßÊ±¼ä
   ...
   DBR_TAG_NOTREADCOUNT     = 1208; // ÀÐÁö¾ÊÀº ÂÊÁö »õ¼ö Àü¼Û
   ...
   GM_CHECKCLIENT       = 4;     //Å¬¶óÀÌ¾ðÆ®¿¡¼­ Ã¤Å© ½ÅÈ£¸¦ º¸³¿
   GM_DATA              = 5;
   ...
   RC_BEEQUEEN       = 103;  //¹úÅë
   RC_ARCHERMON      = 104;  //¸¶±Ã»ç, ÇØ°ñ±Ã¼ö  Ó¥ÎÀ
   RC_GASMOTH        = 105;
   RC_DUNG           = 106;  //µÕ, °¡½º
   ...
   // 2003/02/11 Ãß°¡ ¸÷
   RC_SKELETONKING      = 126; //ÇØ°ñ¹Ý¿Õ
   RC_TOXICGHOST        = 127; //ºÎ½Ä±Í
   RC_SKELETONSOLDIER   = 128; //ÇØ°ñº´Á¹
   // 2003/03/04 Ãß°¡ ¸÷
   RC_BANYAGUARD        = 129; //¹Ý¾ßÁÂ»ç/¹Ý¾ß¿ì»ç
   RC_DEADCOWKING       = 130; //»ç¿ìÃµ¿Õ
   // 2003/07/15 Ãß°¡ ¸÷
   RC_PBOMA1         = 131; //³¯°³¿À¸¶
   RC_PBOMA2         = 132; //¼è¹¶Ä¡»ó±Þ¿À¸¶
   RC_PBOMA3         = 133; //¸ùµÕÀÌ»ó±Þ¿À¸¶
   RC_PBOMA4         = 134; //Ä®ÇÏ±Þ¿À¸¶
   RC_PBOMA5         = 135; //µµ³¢ÇÏ±Þ¿À¸¶
   RC_PBOMA6         = 136; //È°ÇÏ±Þ¿À¸¶
   ...
   RC_JUMA_THUNDER   = 150; //ÁÖ¸¶°Ý·ÚÀå

   RC_GOLDENIMUGI    = 151; //È²±ÝÀÌ¹«±â
   ...
   RC_CLONEMON       = 183; //ºÐ½Å¸ó½ºÅÍ
   RC_GOLDENIMUGI2    = 184; //È²±ÝÀÌ¹«±â
   ...
   UD_USER        = 0;
   UD_USER2       = 1;
   UD_OBSERVER    = 2;   // '2' µî±Þ
   UD_ASSISTANT   = 4;   // 'A' µî±Þ(observerµî±Þ°ú sysopµî±Þ »çÀÌ¿¡ Ãß°¡)
   UD_SYSOP       = 6;   // '1' µî±Þ
   UD_ADMIN       = 8;   // '*' µî±Þ
   UD_SUPERADMIN  = 10;  // '*' µî±Þ(Å×½ºÆ® ¼­¹ö ¶Ç´Â ÆÐ½º¿öµå ¼º°ø ÈÄ)
   ...
   ET_HEARTPALP      = 7;  //Ç÷°ÅÀÎ ¿Õ(½ÉÀå)¹æÀÇ ÃË¼ö °ø°Ý
   ET_MINE2          = 8;  //º¸¼®ÀÌ ¸ÅÀåµÇ¾î ÀÖÀ½
   ET_JUMAPEICE      = 9;  //ÁÖ¸¶°Ý·ÚÀå ƒÆÁø Á¶°¢
   ET_MINE3          = 10;  //ÀÌº¥Æ®¿ë ±¤¼® ¹× º¸¼®ÀÌ ¸ÅÀåµÇ¾î ÀÖÀ½(2004/11/03)
   ET_BLIZZARD       = 11;  //Blizzard
   ET_METEOR         = 12;  //MeteorStrike
   ET_YINYANGFIVE    = 14;  //ÒõÑôÎåÐÐÕó
 {6ÖÖÑÌ»¨}
   ET_FIREFLOWER_1   = 15; //Ò»ÐÄÒ»Òâ
   ET_FIREFLOWER_2   = 16; //ÐÄÐÄÏàÓ¡
   ET_FIREFLOWER_3   = 17;
   ET_FIREFLOWER_4   = 18;
   ET_FIREFLOWER_5   = 19;
   ET_FIREFLOWER_6   = 20;
   ...
   ET_FIREFLOWER_8   = 22;
   ET_FIREFLOWER_9   = 23;
   ET_POISONCLOUD    = 24;  //¶¾ÔÆ

   NE_HEARTPALP      = 1;  //±âº» È¿°ú ½Ã¸®Áî, 1¹ø ÃË¼ö°ø°Ý
   NE_CLONESHOW      = 2;  //ºÐ½Å³ªÅ¸³²
   NE_CLONEHIDE      = 3;  //ºÐ½Å»ç¶óÁü
   NE_THUNDER        = 4;  //¿ë´øÁ¯ ¹ø°³
   NE_FIRE           = 5;  //¿ë´øÁ¯ ¿ë¾Ï
   NE_DRAGONFIRE     = 6;  //¿ëºÒ°ø°Ý ÅÍÁü
   NE_FIREBURN       = 7;  //¿ë¼®»ó°ø°Ý ÅÍÁü Å¸¿À¸§
   ...
   NE_SOULSTONE_HIT  = 24; //È£È¥¼® °ø°Ý ÀÓÆåÆ®
   ...
   NE_HAPPYBIRTHDAY  = 28; //ÇÁ¸®¹Ì¾ö »ýÀÏ ÀÓÆåÆ®
   NE_KINGTURTLE_MOBSHOW  = 29; //Çö¹«Çö½Å ¼ÒÈ¯¸÷ ³ªÅ¸³ª´ÂÀÓÆåÆ®
   NE_USERHEALING    = 30; //ÃÊº¸ÀÚÁö¿ª NPCÈú ÀÌÆåÆ®
   NE_DEFENCEEFFECT  = 31; //¿µÁ¤°©ÁÖ ¹Ý»ç ÀÌÆåÆ®
   NE_KOREAFIGHTING  = 32; //¿ùµåÄÅÀÀ¿ø
   NE_REVIVALCHARGE  = 33; //ËÕÉúÊõ
   NE_SLASHINGBURST  = 36; //ÈÕÉÁ
   NE_YINYANGTARGET  = 40; //ÒõÑôÎåÐÐÕóÄ¿±ê
   NE_YINYANGFRIEND  = 41; //ÒõÑôÎåÐÐÕóÅóÓÑ


   SWD_LONGHIT       = 12; //¾î°Ë¼ú
   SWD_WIDEHIT       = 25; //¹Ý¿ù°Ë¹ý
   SWD_FIREHIT       = 26; //¿°È­°á
   SWD_RUSHRUSH      = 27; //¹«ÅÂº¸
   // 2003/03/15 ½Å±Ô¹«°ø
   ...
   //*dq
   QI_IFGETDAILYQUEST = 40;  //¿À´Ã Äù½ºÆ®¸¦ ¹Þ¾Ò´ÂÁö °Ë»ç, À¯È¿±â°£ °Ë»ç Æ÷ÇÔ
   QI_CHECKDAILYQUEST = 41;  //Æ¯Á¤ ¹øÈ£ÀÇ Äù½ºÆ®¸¦ ¼öÇàÁßÀÎÁö °Ë»ç, À¯È¿±â°£ °Ë»ç Æ÷ÇÔ
   QI_RANDOMEX        = 42;  //ÆÄ¶ó¸ÞÅ¸  5 100   5%ÀÓ...
   ...
   QI_CHECKIDLIST     = 159;
   QI_CHECKADDUSERIDTIME = 160;
   QI_CHECKSECONDCARD    = 161;
   QI_CHECKCREDITPOINT   = 162;
   QI_CHECKHAVEPRENTICE   = 163;
   QI_CHECKSLAVECOUNT     = 164; //¼ì²é±¦ÊýÁ¿
   QI_HAVEGUILD      = 165;  //¼ì²âÊÇ·ñÓÐÃÅÅÉ
   QI_ISCASTLEMASTER = 166; //¼ì²éÍæ¼ÒÊÇ·ñÎªÉ³³ÇÀÏ´ó
   QI_ISCASTLEGUILD = 167;  //¼ì²éÍæ¼ÒÊÇ·ñÎªÉ³³Ç³ÉÔ±
   QI_INSAFEZONE = 168;   //¼ì²âÊÇ·ñÔÚ°²È«Çø
   QI_CHECKITEMADDVALUE  = 169; //¼ì²é×°±¸ÊôÐÔ
   QI_ISUSEITEM = 170;   //µ±Ç°Ê¹ÓÃÎïÆ·
   QI_CHECKINFO       = 171;  //ÊÇ·ñÔÚµ±Ç°µØÍ¼
   QI_CHECKGAMEPOINT = 172;  //»ý·Ö
   QI_ISADMIN    = 173;    //¼ì²éÊÇ²»ÊÇÏµÍ³¹ÜÀíÔ±
   QI_CHECKMAPHUMANCOUNT = 174;
   QI_CHECKMAGICNAME = 175;
   QI_CHECKMAPMONCOUNT = 176;
 //  QI_RIGHTHAND     = 174; //¼ì²éÀ¯Öò£¬Ñ«ÕÂÎ»ÖÃÊÇ·ñÓÐÎïÆ· 2022/10/6
   //Action

   QA_SET            = 1;   //101ÀÌ»ó
   QA_TAKE           = 2;   //¾ÆÀÌÅÛÀ» ¹Þ´Ù
   QA_GIVE           = 3;
   QA_TAKEW          = 4;   //Âø¿ëÇÏ°í ÀÖ´Â ¾ÆÀÌÅÛÀ» ¹Þ´Ù
   QA_CLOSE          = 5;   //´ëÈ­Ã¢À» ´ÝÀ½
   QA_RESET          = 6;   //
   QA_OPENUNIT       = 7;
   ...
   QA_MAPMOVE        = 20;
   QA_MAPRANDOM      = 21;
   QA_TAKECHECKITEM  = 22;  //CHECKÇ×¸ñ¿¡¼­ °Ë»çµÈ ¾ÆÀÌÅÛÀ» ¹Þ´Â´Ù.
   QA_MONGEN         = 23;  //¸ó½ºÅÍ¸¦ Á¨½ÃÅ´
   QA_MONCLEAR       = 24;  //¸ó½ºÅÍ¸¦ ¸ðµÎ Á¦°Å ½ÃÅ²´Ù
   QA_MOV            = 25;
   QA_INC            = 26;
   QA_DEC            = 27;
   ...
   QA_CLOSENOINVEN    = 31;   //´ëÈ­Ã¢À» ´ÝÀ½(ÀÎº¥Ã¢Àº °Çµå¸®Áö ¾ÊÀ½)

   QA_MOVRANDOM      = 50;  //MOVR
   QA_EXCHANGEMAP    = 51;  //EXCHANGEMAP R001  //R001¿¡ ÀÖ´Â ÇÑ »ç¶÷°ú ÀÚ¸®¸¦ ¹Ù²Û´Ù.
   QA_RECALLMAP      = 52;  //RECALLMAP R001  //R001¿¡ ÀÖ´Â »ç¶÷µéÀ» ¸ðµÎ ¼ÒÈ¯ ÇÑ´Ù.
   QA_ADDBATCH       = 53;
   QA_BATCHDELAY     = 54;
   QA_BATCHMOVE      = 55;
   ...
   //*dq
   QA_RANDOMSETDAILYQUEST = 60;  //ÆÄ¶ó¸ÞÅÍ,  ÃÖ¼Ò, ÃÖ´ë  ¿¹) 401 450  401¿¡¼­ 450¹ø±îÁö ·£´ýÀ¸·Î ¼³Á¤
   QA_SETDAILYQUEST  = 61;
   ...
   QA_CHANGEGENDER   = 104;
   QA_KICK           = 105;
   QA_MOVEALLMAP     = 106;    // ÇöÀç ¸Ê À¯ÀúµéÀ» ¸ðµÎ Æ¯Á¤ ¸ÊÀ¸·Î ÀÌµ¿½ÃÅ´.
   ...
   QA_SETALLINMAP    = 110;    // ÇöÀç ¸Ê¿¡ ÀÖ´Â ¸ðµç À¯ÀúµéÀÇ ÇÃ·¡±×¸¦ SETÇÑ´Ù.
   ...
   //Àå¿ø±âºÎ±Ý
   QA_DECDONATION    = 118;    // ±âºÎ±Ý ÀÜ¾×À» °¨¼Ò ½ÃÅ²´Ù.
   ...
   QA_ADDUSERIDTIME       = 139;
   QA_ADDSECONDCARD       = 140;

   QA_SENDSCROLLMSG         = 141;
   QA_GMEXECUTE           = 142;
   ...
   QA_GAMEGOLD           = 149;
   QA_GIVEEX        = 150;  //µ÷Õû×°±¸ÊôÐÔ
   QA_CHANGEJOB        = 151; //½Å±¾ÐÞ¸ÄÖ°Òµ
   QA_CHANGELEVEL      = 152; //µÈ¼¶±ä¶¯
   QA_CHANGENAMECOLOR  = 153;  //¸Ä±äÃû×ÖÑÕÉ«
   QA_ADDSKILL = 154; //Ôö¼Ó¼¼ÄÜ
   QA_DELSKILL = 155; //É¾³ý¼¼ÄÜ
   QA_DELNOJOBSKILL = 156; //É¾³ý·Ç±¾Ö°ÒµËùÓÐ¼¼ÄÜ
   QA_CLEARSKILL = 157; //É¾³ýËùÓÐ¼¼ÄÜ
   QA_SKILLLEVEL = 158; //µ÷Õû¼¼ÄÜµÈ¼¶
   QA_MESSAGEBOX = 159; //´°¿ÚÌáÊ¾
   QA_SETRANKLEVELNAME  = 160; //³ÆºÅ
   QA_SETITEMEVENT = 161;  //×Ô¶¨Òå×Ö¶Î´¥·¢Ë«»÷ÎïÆ·
   QA_GAMEPOINT = 162; //µ÷Õû»ý·Ö
   QA_CHANGEHUMABILITY = 163; //changeHumAbilityÊôÐÔ¼Óµã
   QA_ADDGUILDMEMBER   = 164;  //¼ÓÈëÖ¸¶¨ÐÐ»á
   QA_CLEARNAMELIST   = 165; //ÇåÀíÁÐ±íÃû×Ö
   QA_NOTONLINE        = 166;
   QA_DELAYGOTO = 167; //ÑÓÊ±Ìø×ª
   QA_CLEARDELAYGOTO  = 168;// È¡ÏûÑÓÊ±Ìø×ª
   QA_npcsay           = 169;
   QA_CLEARMAPMON           = 170;   //ÐÂÇåÀíµØÍ¼¹ÖÎï
   QA_NOVICEHEIP    = 171;

   VERSION_NUMBER = 20050501;    //M2°æ±¾ºÅ£¿
   ...
   BUFFERSIZE = 10000;
    ...
    TAGSTATE_NOTREAD     = 0;       // ÀÐÁö¾ÊÀ½
    TAGSTATE_READ        = 1;       // ÀÐÀ½
    TAGSTATE_DONTDELETE  = 2;       // »èÁ¦±ÝÁö
...
// RelationShip Error Code...
    ...
    USERMARKET_MODE_BUY     = 1   ;     // »ç´Â¸ðµå
    USERMARKET_MODE_INQUIRY = 2   ;     // Á¶È¸¸ðµå
    USERMARKET_MODE_SELL    = 3   ;     // ÆÇ¸Å¸ðµå


    MARKET_CHECKTYPE_SELLOK     = 1;    //À§Å¹ Á¤»ó
    MARKET_CHECKTYPE_SELLFAIL   = 2;    //À§Å¹ ½ÇÆÐ
    MARKET_CHECKTYPE_BUYOK      = 3;    //±¸ÀÔ Á¤»ó
    MARKET_CHECKTYPE_BUYFAIL    = 4;    //±¸ÀÔ ½ÇÆÐ
    MARKET_CHECKTYPE_CANCELOK   = 5;    //Ãë¼Ò Á¤»ó
    MARKET_CHECKTYPE_CANCELFAIL = 6;    //Ãë¼Ò ½ÇÆÐ
    MARKET_CHECKTYPE_GETPAYOK   = 7;    //µ· È¸¼ö Á¤»ó
    MARKET_CHECKTYPE_GETPAYFAIL = 8;    //µ· È¸¼ö ½ÇÆÐ

    MARKET_DBSELLTYPE_SELL          = 1;//ÆÇ¸ÅÁß
    MARKET_DBSELLTYPE_BUY           = 2;//»òÀ½
    MARKET_DBSELLTYPE_CANCEL        = 3;//Ãë¼Ò
    MARKET_DBSELLTYPE_GETPAY        = 4;//±Ý¾×È¸¼ö
...
function  Effectfeature(cfeature: Integer): byte; //³á°òÔö¼Ó
