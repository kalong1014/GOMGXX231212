...
uses
   ...
   function GetSavedKey: WORD;
   procedure ChangeByDefaultKey;
   procedure ChangeBySavedKey;
...
//  g_EndeKey: WORD = ($6501);
  ...
  g_DefaultPubKey: WORD  = ($6501);
  g_SavedPubKey: WORD  = ($6501);
  g_EndeKey: WORD  = ($6501);
  // 치환 테이블 인코딩 값
  g_HideTable: BYTE = ($97);
  // 역치환 테이블 인코딩 값
...
procedure Decode6BitBuf (source: string; buf: PAnsiChar; buflen: integer);
const
...
procedure Encode6BitBuf_old (src, dest: PAnsiChar; srclen, destlen: integer);
var
   i, restcount, destpos: integer;
   made, ch, rest: byte;
begin
...
procedure Decode6BitBuf_old (source: string; buf: PAnsiChar; buflen: integer);
const
	Masks: array[2..6] of byte = ($FC, $F8, $F0, $E0, $C0);
   //($FE, $FC, $F8, $F0, $E0, $C0, $80, $00);
var
...
begin
   try
      ...
      Decode6BitBuf (PAnsiChar(AnsiString(str)), EncBuf, BUFFERSIZE);
...
begin
   try
      EnterCriticalSection (CSencode);
      Decode6BitBuf (PAnsiChar(AnsiString(str)), EncBuf, BUFFERSIZE);
      Result := string (EncBuf); //error, 1, 2, 3,...
   finally
      LeaveCriticalSection (CSencode);
   end;
end;
...
begin
   try
      EnterCriticalSection (CSencode);
      Decode6BitBuf_old (PAnsiChar(AnsiString(str)), EncBuf, BUFFERSIZE);
      Result := string (EncBuf); //error, 1, 2, 3,...
   finally
      LeaveCriticalSection (CSencode);
   end;
...
begin
   try
      EnterCriticalSection (CSencode);
      Decode6BitBuf (PAnsiChar(AnsiString(src)), EncBuf, BUFFERSIZE);
...
//      smsg.Etc := WORD(((smsg.Recog and $57CD) + (smsg.Ident or $48) + (smsg.Param or $30) + (smsg.Tag and $2D) + smsg.Series) xor GetPublicKey);
      ...
      Encode6BitBuf (TempBuf, EncBuf, sizeof(TDefaultMessage), BUFFERSIZE);
      Result := string (EncBuf);  //Error: 1, 2, 3, 4, 5, 6, 7, 8, 9
   finally
   	LeaveCriticalSection (CSencode);
   end;
end;
...
begin
   try
   	EnterCriticalSection (CSencode);
      Encode6BitBuf (PAnsiChar(AnsiString(str)), EncBuf, Length(AnsiString(str)), BUFFERSIZE);
