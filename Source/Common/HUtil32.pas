...
uses
	Classes, SysUtils, StrUtils, WinTypes, WinProcs, Graphics, Messages, Dialogs, DateUtils;
...
function CheckNumber(checkStr:String):Boolean; //ºÏ≤È◊÷∑˚¥Æ «∑ÒŒ™¥ø ˝◊÷
...
function AnsiCompareBackLStr (src, targ: AnsiString; compn: integer): Boolean;
...
begin
  ...
  if Str <> '' then
  begin
    if ((Word(Str[1]) >= Word('0')) and (Word(Str[1]) <= Word('9'))) or (Str[1] = '+') or (Str[1] = '-') then
...
begin
(*
  1°¢s := '“ª∂˛»˝{≤‚ ‘≤‚ ‘|251:249:1}ÀƒŒÂ¡˘';
  sMsg := ArrestStringEx(s, '{', '}', sMsg2);
  sMsg = ÀƒŒÂ¡˘
  sMsg2 = ≤‚ ‘≤‚ ‘|251:249:1

  2°¢s2 := '{≤‚ ‘≤‚ ‘|251:249:1}“ª∂˛»˝{≤‚ ‘≤‚ ‘|251:249:1}ÀƒŒÂ¡˘';
  sMsg := ArrestStringEx(s2, '{', '}', sMsg2);
  sMsg = “ª∂˛»˝{≤‚ ‘≤‚ ‘|251:249:1}ÀƒŒÂ¡˘
  sMsg2 = ≤‚ ‘≤‚ ‘|251:249:1

  3°¢s3 := '{≤‚ ‘≤‚ ‘|251:249:1}“ª∂˛»˝{≤‚ ‘≤‚ ‘|251:249:1}ÀƒŒÂ¡˘{≤‚ ‘≤‚ ‘|251:249:1}';
  sMsg := ArrestStringEx(s3, '{', '}', sMsg2);
  sMsg = “ª∂˛»˝{≤‚ ‘≤‚ ‘|251:249:1}ÀƒŒÂ¡˘{≤‚ ‘≤‚ ‘|251:249:1}
  sMsg2 = ≤‚ ‘≤‚ ‘|251:249:1

  4°¢s4 := '{“ª∂˛»˝{≤‚ ‘≤‚ ‘|251:249:1}ÀƒŒÂ¡˘';
  sMsg := ArrestStringEx(s4, '{', '}', sMsg2);
  sMsg = ÀƒŒÂ¡˘
  sMsg2 = “ª∂˛»˝{≤‚ ‘≤‚ ‘|251:249:1
*)
  ArrestStr := '';                      {result string}
  ...
  try
    ...
    if i > 0 then begin                 //”–«∞◊∫
      ...
      if n > i then begin               //”–∫Û◊∫
        ArrestStr := Copy(Source, i + 1, n - i - 1); //»°«∞∫Û◊∫÷Æº‰µƒ◊÷∑˚¥Æ
        Result := Copy(Source, n + 1, SrcLen - n); // £”‡ ˝æ›»°∫Û◊∫÷Æ∫Ûµƒ◊÷∑˚¥Æ
      end else begin                    //”–«∞◊∫√ª∫Û◊∫
        ArrestStr := Copy(Source, i + 1, SrcLen - i); //«∞◊∫÷Æ∫Ûµƒ◊÷∑˚¥Æ∂º «ArrestStr
        Result := '';                   // £”‡ ˝æ›Œ™ø’
      end;
    end else begin                      //√ª”–«∞◊∫
      n := Pos(ArrestBefore, Source);
      if n > 0 then begin               //√ª«∞◊∫”–∫Û◊∫
        ArrestStr := Copy(Source, 1, n - 1); //»°∫Û◊∫÷Æ«∞µƒ◊÷∑˚¥Æ
        Result := Copy(Source, n + 1, SrcLen - n) // £”‡ ˝æ›Œ™∫Û◊∫÷Æ∫Ûµƒ◊÷∑˚¥Æ
      end else begin                    //√ª«∞◊∫“≤√ª∫Û◊∫
        ArrestStr := Source;            //»°»´≤ø ˝æ›
        Result := '';                   // £”‡ ˝æ›Œ™ø’
      end;
    end;
    //---------------À„∑®”≈ªØΩ· ¯------------------------
...
begin
   ...
   for i:=1 to Length(AnsiStr) do begin
      if (byte(AnsiStr[i]) >= byte('0')) and (byte(AnsiStr[i]) <= byte('9')) or (AnsiStr[i] = '-') then begin
         Dest := String(Copy (AnsiStr, 1, i-1));
         Result := String(Copy (AnsiStr, i, Length(AnsiStr)));
         flag := TRUE;
         break;
      end;
   end;
...
{ GetValidStr3øÕ ¥ﬁ∏Æ Ωƒ∫∞¿⁄∞° ø¨º”¿∏∑Œ ≥™ø√∞ÊøÏ √≥∏Æ æ»µ  }
{ Ωƒ∫∞¿⁄∞° æ¯¿ª ∞ÊøÏ, nil ∏Æ≈œ.. }
function DivString (source: string; cap: char; var sel: string): string;
var
   n: integer;
...
// Ωÿ»°◊÷∑˚¥Æ
// ¿˝ AnsiArrestStringEx('[1234]','[',']',str)    str=1234
...
begin
  ...
  if (X > 0) and (Y > 0) then begin
    ...
    s2 := Copy(SourStr, Y + 1, Length(SourStr)); //∑≈÷√ £”‡≤ø∑÷
    Result := s1 + s2;
  end;
end;
