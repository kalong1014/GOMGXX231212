...
{$DEFINE SYMODIFY}
uses {$IFDEF SYMODIFY } Dialogs, {$ENDIF}Windows, AsphyreD3DTypes;  //随云增加一个Dialogs
...
type
  ...
  D3DCOLOR = AsphyreD3DTypes.D3DCOLOR;
  {$EXTERNALSYM D3DCOLOR}
  TD3DColor = AsphyreD3DTypes.TD3DColor;
...
type
  _D3DVECTOR = AsphyreD3DTypes._D3DVECTOR;
  {$EXTERNALSYM _D3DVECTOR}
  D3DVECTOR = AsphyreD3DTypes.D3DVECTOR;
  {$EXTERNALSYM D3DVECTOR}
  TD3DVector = AsphyreD3DTypes.TD3DVector;
  PD3DVector = AsphyreD3DTypes.PD3DVector;
...
const
  ...
  d3dx9MicrosoftDLL = 'Data\d3dx9_33.dll';
...
function D3DXAssembleShader(
  ...
  ppErrorMsgs: PID3DXBuffer): HResult; stdcall; external d3dx9MicrosoftDLL;
...
function D3DXSaveSurfaceToFile(
  ...
  pSrcRect: PRect): HResult; stdcall; external d3dx9MicrosoftDLL name 'D3DXSaveSurfaceToFileW';
...
{$IFDEF SYMODIFY}
var
  Direct3D9_33: THandle = 0;
procedure LoadDirect3D9_33DLL();
begin
  if Direct3D9_33 = 0 then
  begin
    Direct3D9_33:= LoadLibrary(d3dx9MicrosoftDLL);
    D3DXAssembleShader := GetProcAddress(Direct3D9_33, 'D3DXAssembleShader');
    D3DXSaveSurfaceToFile := GetProcAddress(Direct3D9_33, 'D3DXSaveSurfaceToFileW');
  end;

  if not Assigned(D3DXAssembleShader) then
  begin
     ShowMessage('异常:缺少' + d3dx9MicrosoftDLL + ',无法运行');
     Halt(0);
  end;

end;

procedure UnLoadDirect3D9_33DLL();
begin
  if Direct3D9_33 <> 0 then
  begin
    FreeLibrary(Direct3D9_33);
  end;
