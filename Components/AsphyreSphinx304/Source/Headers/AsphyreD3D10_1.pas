...
{$IFDEF DYNAMIC_LINK_ALL}
  {$DEFINE DIRECT3D10_1_DYNAMIC_LINK}
{$ENDIF}
...
uses
  Windows, AsphyreD3DTypes, AsphyreDXGI, AsphyreD3D10;
...
type
  ...
  PD3D10_SRV_Dimension1 = ^TD3D10_SRV_Dimension1;
  //todo: Check if it is a BYTE-sized enum!
  ...
  TD3D10_SRV_Dimension1 = D3D10_SRV_DIMENSION1;
...
{$IFDEF DIRECT3D10_1_DYNAMIC_LINK}
type
...
var
 D3D10CreateDevice1: TD3D10_CreateDevice1 = nil;
...
function D3D10CreateDevice1(
	...
	HardwareLevel: TD3D10_FeatureLevel1;
	SDKVersion: LongWord;
	out ppDevice: ID3D10Device1): HResult; stdcall; external D3D10_1_dll;
...
{$IFDEF DIRECT3D10_1_DYNAMIC_LINK}
var
 Direct3D10_1Lib: THandle = 0;

function Direct3D10_1Loaded(): Boolean;
begin
 Result:= Direct3D10_1Lib <> 0;
end;

function UnloadDirect3D10_1(): Boolean;
begin
 Result:= True;

 if (Direct3D10_1Loaded()) then
  begin
   Result:= FreeLibrary(Direct3D10_1Lib);
...
function LoadDirect3D10_1(): Boolean;
begin
 Result:= Direct3D10_1Loaded();

 if (not Result) then
  begin
   Direct3D10_1Lib:= LoadLibrary(D3D10_1_dll);

   if (Direct3D10_1Loaded()) then
    begin
      LinkDXGI10;
     D3D10CreateDevice1:= GetProcAddress(Direct3D10_1Lib, 'D3D10CreateDevice1');
     D3D10CreateDeviceAndSwapChain1:= GetProcAddress(Direct3D10_1Lib, 'D3D10CreateDeviceAndSwapChain1');

     Result:=
      (Assigned(D3D10CreateDevice1))and
      (Assigned(D3D10CreateDeviceAndSwapChain1));

     if (not Result) then UnloadDirect3D10_1();
    end;
  end;
end;
