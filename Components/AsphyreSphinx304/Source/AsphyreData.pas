unit AsphyreData;
//---------------------------------------------------------------------------
// AsphyreData.pas                                      Modified: 14-Sep-2012
// Data Compression and CRC32 routines.                          Version 1.05
//---------------------------------------------------------------------------
// Important Notice:
//
...
// under the License.
//
// The Original Code is AsphyreData.pas.
//
...
//---------------------------------------------------------------------------
{< Utility routines for data compression and checksum calculation. }
//---------------------------------------------------------------------------
interface
...
//---------------------------------------------------------------------------
uses
 Types, Classes, {$ifdef fpc}paszlib{$else}
 {$ifdef DelphiLegacy}AsphyreZLib{$else}ZLib{$endif}{$endif};
...
//---------------------------------------------------------------------------
{ Calculates CRC32 checksum from the specified data. This checkum can be
  used in integrity tests to verify that the data has not been modified. }
...
//---------------------------------------------------------------------------
// Note: this function was originally written by Earl F. Glynn, 1999 (?)
//---------------------------------------------------------------------------
function DataChecksum(Source: Pointer; Size: Integer): LongWord;
var
 pb   : PByte;
...
begin
 ...
 compress(Dest, OutSize, Source, SourceSize);
 ...
 try
  ZCompress(Source, SourceSize, OutBuf, OutSize, zLevel);
 except
  Exit;
 end;

 if (not Assigned(OutBuf)) then Exit;

 if (Int64(OutSize) <= MaxDestSize) then
  begin // The compressed data fits perfectly in the destination buffer.
   Move(OutBuf^, Dest^, OutSize);
   Result:= OutSize;
  end;
...
begin
 ...
 try
  ZDecompress(Source, SourceSize, OutBuf, OutSize);
 except
  Exit;
 end;
 ...
 if (OutSize > 0) then
  begin
   if (MaxDestSize > 0)and(Int64(OutSize) > MaxDestSize) then
    OutSize:= MaxDestSize;
