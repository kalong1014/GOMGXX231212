unit FileNameAliases;
//---------------------------------------------------------------------------
// FileNameAliases.pas                                  Modified: 14-Sep-2012
// Short aliases for long file names for media descriptions.     Version 1.02
//---------------------------------------------------------------------------
// Important Notice:
//
...
// under the License.
//
// The Original Code is FileNameAliases.pas.
//
...
begin
 ...
 while (Left <= Right) do
  begin
   while (Left <= Stop)and(SearchListCompare(Left, Pivot) < 0) do Inc(Left);
   while (Right > Start)and(SearchListCompare(Right, Pivot) >= 0) do Dec(Right);
...
begin
 if (Length(Items) > 1) then SearchListSort(0, Length(Items) - 1);
 SearchDirty:= False;
end;
