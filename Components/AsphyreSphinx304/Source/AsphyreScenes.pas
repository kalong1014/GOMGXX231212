unit AsphyreScenes;
//---------------------------------------------------------------------------
// AsphyreScenes.pas                                    Modified: 14-Sep-2012
// Cross-platform 3D renderer for Asphyre.                       Version 1.02
//---------------------------------------------------------------------------
// Important Notice:
//
...
//---------------------------------------------------------------------------
uses
 Vectors2px, Vectors2, Vectors3, Vectors4, Matrices4, HelperSets,
 AsphyreColors, AsphyreMatrices, AsphyreMeshes, AbstractTextures,
...
//---------------------------------------------------------------------------
 ...
 public
  ...
  procedure InsertPostTriangle(const v0, v1, v2: TVector4;
   const tx0, tx1, tx2: TPoint2; c0, c1, c2, s0, s1, s2: Cardinal;
...
begin
 ...
 if (Mesh.Vertices.Count < 1)or
  (Mesh.VertexIndices.Count < 1)or
  (Mesh.VertexIndices.Count mod 3 > 0)or
  (Mesh.TextureIndices.Count <> Mesh.VertexIndices.Count)or
  (Mesh.Vertices.Count <> Mesh.Normals.Count) then Exit;

 NoFaces:= Mesh.VertexIndices.Count div 3;

 if (NoFaces <> Mesh.FaceNormals.Count)or
  (NoFaces <> Mesh.FaceOrigins.Count)or
  (TriangleCount + NoFaces > MaxTriangles) then Exit;

 // Apply world transformation to mesh vertices and normals.
 VertexNo  := PreVertices.Count;
 TexCoordNo:= TextureCoords.Count;

 PreVertices.AddTransform(Mesh.Vertices, WorldMtx^, 1.0);
 FaceOrigins.AddTransform(Mesh.FaceOrigins, WorldMtx^, 1.0);
 ...
 Normals.AddTransform(Mesh.Normals, NormalMtx.RawMtx^, 0.0);
 FaceNormals.AddTransform(Mesh.FaceNormals, NormalMtx.RawMtx^, 0.0);
...
begin
 Texture:= nil;
 if (Assigned(Image)) then Texture:= Image.Texture[TextureNo];
...
begin
 for i:= 0 to TriangleCount - 1 do
  case Triangles[i].Culling of
   ...
   sctClockwise:
    Triangles[i].Visible:= Dot3(FaceNormals[i].GetXYZ(),
     FaceOrigins[i].GetXYZ()) > 0;
...
begin
 ...
 for i:= 0 to PostVertices.Count - 1 do
  if (DiffuseColors[i].a > 0) then
   begin
    FLights.Illuminate(PreVertices[i].GetXYZ(), Norm3(Normals[i].GetXYZ()),
     EyePos, Diffuse, Specular);
...
procedure TAsphyreScene.InsertPostTriangle(const v0, v1, v2: TVector4;
 ...
 ATexture: TAsphyreCustomTexture);
begin
 if (TriangleCount >= MaxTriangles) then Exit;
