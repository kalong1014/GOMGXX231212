unit GuiComboBoxes;
//---------------------------------------------------------------------------
// GuiComboBoxes.pas                                    Modified: 14-Sep-2012
// Combo Box controls for Asphyre GUI framework.                 Version 1.01
//---------------------------------------------------------------------------
// Important Notice:
//
...
// under the License.
//
// The Original Code is GuiComboBoxes.pas.
//
...
//---------------------------------------------------------------------------
uses
 Types, Classes, SysUtils, AsphyreDef, Vectors2px, GuiTypes, GuiControls,
 GuiListBoxes;
...
//---------------------------------------------------------------------------
uses
 AsphyreUtils, AsphyreImages, AsphyreFonts, GuiGlobals;

//---------------------------------------------------------------------------
constructor TGuiComboBox.Create(AOwner: TGuiControl);
begin
...
begin
 ...
 if (Assigned(FListBox)) then ArrowSize.y:= OrigHeight - 6;

 ArrowSize.x:= ArrowSize.y;
 if (ArrowSize.x > Width div 4) then ArrowSize.x:= Width div 4;
...
begin
 ...
 Height:= {Height + }FListBox.Top + FListBox.Height;
end;
 ...
 case Event of
  metDown:
   if (Button = mbtLeft) then
    begin
     if (not Assigned(FListBox)) then CreateListBox()
      else DestroyListBox();
    end;

  metWheelUp,
  metWheelDown:
   if (Assigned(FListBox)) then FListBox.AcceptMouse(MousePos, Event, Button,
    Shift);
 end;
end;
...
begin
 ...
 if (not Handled) then
  begin
   if (not Assigned(FListBox)) then DoKeyEvent(Key, Event, Shift)
...
begin
 ...
 if (Assigned(FListBox)) then DownAlpha:= 255 else DownAlpha:= 0;
...
begin
 ...
 if (Assigned(FListBox)) then DownAlpha:= Min2(DownAlpha + 48, 255)
...
begin
 FItemIndex:= Value;
 if (Assigned(FListBox)) then FListBox.ItemIndex:= FItemIndex;
end;
