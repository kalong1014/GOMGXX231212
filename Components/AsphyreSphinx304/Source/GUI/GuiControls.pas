unit GuiControls;
//---------------------------------------------------------------------------
// GuiControls.pas                                      Modified: 14-Sep-2012
// Base interactive control for Asphyre GUI framework.           Version 1.01
//---------------------------------------------------------------------------
// Important Notice:
//
...
//---------------------------------------------------------------------------
uses
 Types, Classes, SysUtils, AsphyreDef, Vectors2px, AsphyreXML, AbstractCanvas,
 GuiTypes, GuiComponents;
...
//---------------------------------------------------------------------------
 ...
 private
  ...
  procedure PasteFieldsFromStream(Stream: TStream; Master: TGuiComponent);

  procedure CustomLoadFromStream(Control: TGuiControl; Stream: TStream);
  procedure SaveChildrenToStream(Control: TGuiControl; Stream: TStream);

  procedure CustomPasteFromStream(Control: TGuiControl; Stream: TStream;
   Master: TGuiControl);
  function PasteFromClipboard(Stream: TStream): Boolean;
  function SaveThisToStream(Stream: TStream): Boolean;
 protected
  FocusIndex : Integer;
  FCtrlHolder: Boolean;
  FTabOrder  : Integer;
...
//---------------------------------------------------------------------------
uses
 AsphyreUtils, StreamUtils, AsphyreBase64, GuiRegistry, GuiForms;

//------------------------------------------------------------------------------
function MakeGuiName(Master: TGuiControl;
...
begin
 ...
 if (Assigned(FOwner)) then FOwner.Link(Self);
 FocusIndex:= -1;
end;
...
begin
 Result:= Self;
 while (Assigned(Result.Owner)) do Result:= Result.Owner;
end;
...
//---------------------------------------------------------------------------
procedure TGuiControl.PasteFieldsFromStream(Stream: TStream;
 Master: TGuiComponent);
var
 i, Count: Integer;
 FiName: UniString;
 FiText: UniString;
...
//---------------------------------------------------------------------------
function TGuiControl.SaveThisToStream(Stream: TStream): Boolean;
begin
 Result:= True;
