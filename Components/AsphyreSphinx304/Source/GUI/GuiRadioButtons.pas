unit GuiRadioButtons;
//---------------------------------------------------------------------------
// GuiRadioButtons.pas                                  Modified: 14-Sep-2012
// Radio Button controls for Asphyre GUI framework.              Version 1.01
//---------------------------------------------------------------------------
// Important Notice:
//
...
// under the License.
//
// The Original Code is GuiRadioButtons.pas.
//
...
//---------------------------------------------------------------------------
uses
 Types, AsphyreDef, Vectors2px, GuiTypes, GuiControls;
...
//---------------------------------------------------------------------------
uses
 AsphyreUtils, GuiGlobals, AsphyreFonts;
...
begin
 ...
 if (FChecked) then UncheckOtherButtons();
end;
...
begin
 ...
 for i:= 0 to Owner.ControlCount - 1 do
  begin
   ...
   Button:= TGuiRadioButton(Ctrl);
   if (Button.GroupIndex = FGroupIndex) then Button.Checked:= False;
  end;
end;
...
begin
 ...
 if (FChecked) then CheckedAlpha:= 255 else CheckedAlpha:= 0;
end;
...
begin
 ...
 if (not PressedOver) then
  begin
   if (Event = metDown)and(Button = mbtLeft) then ClickedOver:= True;
   if (Event = metUp)and(Button = mbtLeft) then
    begin
     if (ClickedOver)and(MouseOver) then SetChecked(True);
     ClickedOver:= False;
    end;
  end;
end;
...
begin
 case Event of
  ketDown:
   case Key of
    AVK_Space:
     if (not ClickedOver) then PressedOver:= True;

    AVK_Escape:
     if (PressedOver) then PressedOver:= False;
...
begin
 ...
 if (PropTag = ChangeGroupTag) then CheckInitStatus();
 if (PropTag = ChangeCheckTag)and(FChecked) then UncheckOtherButtons();
end;
