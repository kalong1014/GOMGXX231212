unit GuiTrackBars;
//---------------------------------------------------------------------------
// GuiTrackBars.pas                                     Modified: 14-Sep-2012
// Track Bar controls for Asphyre GUI framework.                  Version 1.0
//---------------------------------------------------------------------------
// Important Notice:
//
...
// under the License.
//
// The Original Code is GuiTrackBars.pas.
//
...
//---------------------------------------------------------------------------
uses
 Types, AsphyreDef, Vectors2px, GuiTypes, GuiControls, GuiButtons;
...
//---------------------------------------------------------------------------
constructor TGuiTrackBar.Create(AOwner: TGuiControl);
begin
 inherited;
...
begin
 ...
 if (Depth mod 2 = 0) then Dec(Depth);
...
begin
 ...
 if (Width > Height) then ScrollButton.SystemIcon:= 8;
end;
  ...
  metMove:
   if (ScrollClickAt <> InfPoint2px)and(ScrollArea > 0) then
    begin
     ...
     if (Width < Height) then ScrollButton.Top:= ScrollTop + NewTop
...
begin
 if (Key in [AVK_Left, AVK_Right, AVK_Up, AVK_Down]) then
  DoKeyEvent(Key, Event, Shift) else inherited;
end;
