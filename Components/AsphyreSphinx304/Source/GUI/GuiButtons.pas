unit GuiButtons;
//---------------------------------------------------------------------------
// GuiButtons.pas                                       Modified: 14-Sep-2012
// Button control for Asphyre GUI framework.                     Version 1.01
//---------------------------------------------------------------------------
// Important Notice:
//
...
// under the License.
//
// The Original Code is GuiButtons.pas.
//
...
//---------------------------------------------------------------------------
uses
 Types, Classes, AsphyreDef, Vectors2px, GuiTypes, GuiControls;
...
//---------------------------------------------------------------------------
uses
 AsphyreUtils, Vectors2, AsphyreFonts, AsphyreImages, GuiGlobals,
 AsphyreEvents;
...
begin
 ...
 FillAlpha:= CompAlpha;
 if (FButtonType = gbtTrackSvc) then FillAlpha:= 1.0;

 if (FShowBorder) then
  begin
   GuiDrawButtonFill(ShrinkRect(PaintRect, 1, 1), FillAlpha, EnabledAlpha,
    DownAlpha);
  end else GuiDrawButtonFill(PaintRect, CompAlpha, EnabledAlpha, DownAlpha);
 ...
 if (FGlyphImageName <> '')and(FGlyphPattern <> -1) then
  begin
   ...
   if (GlyphNo <> -1) then Glyph:= GuiImages[GlyphNo];
  end;
 ...
 if (Assigned(Glyph)) then
  begin
   ...
   GuiDrawGlyph(DrawAt, Glyph, FGlyphPattern, CompAlpha, EnabledAlpha,
    DownAlpha);
  end else GlyphSize:= ZeroPoint2px;
 ...
 if (not PressedOver) then
  begin
   ...
   if (Event = metUp)and(Button = mbtLeft) then
    begin
     if (ClickedOver)and(MouseOver) then DoButtonClick();
     ClickedOver:= False;
    end;
  end;
end;
