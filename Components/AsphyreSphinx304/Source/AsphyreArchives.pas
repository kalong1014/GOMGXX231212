...
//---------------------------------------------------------------------------
// AsphyreArchives.pas                                  Modified: 14-Sep-2012
// Archive format for storing compressed data and images.        Version 1.24
//---------------------------------------------------------------------------
// Important Notice:
//
...
// under the License.
//
// The Original Code is AsphyreArchives.pas.
//
...
//---------------------------------------------------------------------------
uses
 Types, SysUtils, Classes, Math, AsphyreDef;
...
{@exclude}
 TArchiveRecord = record
  Key       : UniString;  // record unique identifier
...
{ Asphyre archive implementation class that provides methods for reading,
 ...
 private
  ...
  function GetRecordKey(Index: Integer): UniString;
 ...
 public
  ...
  property RecordKey[Index: Integer]: UniString read GetRecordKey;
  ...
  function WriteRecord(const Key: UniString; Source: Pointer;
  ...
  { Writes a new record with the specified key from the given stream. The
    ...
    instead. The method returns @True if the record has been written
    successfully or @False if there was a problem.}
  function WriteStream(const Key: UniString; Stream: TStream;
  ...
  function ReadRecord(const Key: UniString; out Data: Pointer;
  ...
  function ReadMemStream(const Key: UniString;
...
//---------------------------------------------------------------------------
uses
 StreamUtils, UniStringUtils, AsphyreData, AsphyreXTEA;
...
//---------------------------------------------------------------------------
function TAsphyreArchive.GetRecordKey(Index: Integer): UniString;
begin
...
function TAsphyreArchive.FixPlatformFileName(
 const NewFileName: StdString): StdString;
var
 i: Integer;
begin
 Result:= NewFileName;
...
//---------------------------------------------------------------------------
procedure TAsphyreArchive.SetFileName(const Value: StdString);
var
...
//---------------------------------------------------------------------------
function TAsphyreArchive.OpenFile(const AFileName: StdString): Boolean;
var
...
begin
 ...
 OutSize:= AsphyreData.DecompressData(Source, DestStream.Memory, SourceSize,
  DestStream.Size);
...
//---------------------------------------------------------------------------
function TAsphyreArchive.WriteRecord(const Key: UniString; Source: Pointer;
 SourceSize: Integer; RecType: TArchiveRecordType = artFile;
 RecDate: TDateTime = 0.0): Boolean;
var
...
begin
 ...
 Checksum:= DataChecksum(Source, SourceSize);
...
//---------------------------------------------------------------------------
function TAsphyreArchive.WriteStream(const Key: UniString; Stream: TStream;
 RecType: TArchiveRecordType = artFile; RecDate: TDateTime = 0.0): Boolean;
var
 Data: Pointer;
...
//---------------------------------------------------------------------------
function TAsphyreArchive.ReadRecord(const Key: UniString; out Data: Pointer;
 out DataSize: Integer): Boolean;
var
 PhysBuf   : Pointer;
 PhysSize  : Cardinal;
...
begin
 ...
 if (Records[Index].Secure)and(Assigned(FPassword)) then
  begin
   DecipherDataXTEA(PhysBuf, PhysBuf, PhysSize, FPassword,
    @Records[Index].InitVec);
  end;
 ...
 Result:= DataChecksum(Data, DataSize) = Records[Index].Checksum;
end;

//---------------------------------------------------------------------------
function TAsphyreArchive.ReadStream(const Key: UniString;
...
//---------------------------------------------------------------------------
function TAsphyreArchive.ReadMemStream(const Key: UniString;
 MemStream: TMemoryStream): Boolean;
var
 PhysBuf   : Pointer;
 PhysSize  : Integer;
...
begin
 ...
 // (4) Calculate and verify the checksum to verify that the data is genuine.
 Result:= DataChecksum(MemStream.Memory, MemStream.Size) =
  Records[Index].Checksum;
end;

//---------------------------------------------------------------------------
function TAsphyreArchive.RenameRecord(const Key, NewKey: UniString): Boolean;
var
 Index: Integer;
begin
...
//---------------------------------------------------------------------------
function TAsphyreArchive.RemoveRecord(const Key: UniString): Boolean;
var
 NewRecords: array of TArchiveRecord;
 InStream, OutStream: TFileStream;
 TempFileName: UniString;
...
begin
 Result:= UniCompareText(Records[Value1].Key, Records[Value2].Key);
end;
...
//---------------------------------------------------------------------------
function TAsphyreArchive.IndexOf(const Key: UniString): Integer;
var
...
begin
 ...
 while (Lo <= Hi) do
  begin
   Mid:= (Lo + Hi) div 2;
   Res:= UniCompareText(Records[SearchList[Mid]].Key, Key);
